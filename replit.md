...
## Recent Changes
- Infographic Generator — Slack + Email sharing (Apr 18, 2026): Added one-click sharing directly from `/infographic`. Two new endpoints in `src/mastra/routes/infographicRoutes.ts`: (1) `POST /api/infographic/:section/share/slack` — body `{channel?, comment?}`, renders the SVG → PNG via the existing `svgToPng` helper and uploads to Slack via `@slack/web-api` `client.files.uploadV2({channel_id, file: png, filename, title, initial_comment})`; defaults to `SLACK_CHANNEL_ID` env if no channel override given; returns 400 with a clear message when `SLACK_BOT_TOKEN`/channel are missing. (2) `POST /api/infographic/:section/share/email` — body `{to, subject?, message?}`, recipients accepted as string ("a@b.com, c@d.com") or array, parsed/trimmed/dedup'd, validated with a strict email regex, capped at 20 per send; renders PNG and sends via Resend (`new Resend(RESEND_API_KEY).emails.send({attachments: [{filename, content: png}]})`) using `RESEND_FROM_EMAIL` (defaults to `onboarding@resend.dev`). Both endpoints reuse the section allow-list `Set` for input validation, return `{success:true, ...}` on OK, and include sanitized `escapeHtml()` for any user-supplied text rendered in the email body. UI: Two new buttons added to the action bar in `dashboard/infographic.html` — purple "Send to Slack" (Slack-mark icon, brand color #4A154B) and blue "Send via Email" (envelope icon). Each opens a small Tailwind modal with channel/recipients/subject/message fields, an inline status pill (info / ok / err) and a spinner-on-send. Modals close on backdrop click or Cancel and auto-close 1.8s after a successful send. Verified live: missing email → 400, invalid email → 400, too-many recipients → 400, unknown section → 404 on both /share/slack and /share/email, GET render still 200, page still gated 302→login.
- Infographic Generator (Apr 18, 2026): New in-platform tool under Support → Infographic Generator (`/infographic`) that lets the CCO/CEO pick any platform area and instantly generate a polished, shareable visual snapshot. Six sections shipped, each pulling live production data: Platform Health (pulse + dashboards + exports + RBAC + counts), KPI Engine (definitions/coverage/categories), Risk Management (heat-map distribution + treatment actions), Audit Readiness (totals + findings + statuses + latest audit), Duplicate Radar (clusters per CRM module + scan freshness + exports), AI Consultant (alerts by severity + HITL queue depth). Architecture: (1) `src/utils/infographicBuilder.ts` — single 1200x1500 dark-themed SVG template (`renderInfographicSvg`) with hero, 8 metric cards, 3 wide stat strip, status pills, footer; six per-section data builders all read via the existing `sharedPool` singleton (no per-request pg pools); all dynamic text passes through an `esc()` XSS-safe helper. (2) `src/mastra/routes/infographicRoutes.ts` — `GET /api/infographic/sections` (catalog), `GET /api/infographic/:section` (returns SVG, validates against allow-list `Set`, returns 404 for unknown sections), supports `?format=png` (ImageMagick stdin pipe with 15s SIGKILL timeout, ENOENT handling, AbortController-style settled-flag) and `?download=1`. (3) Picker UI `dashboard/infographic.html` — section grid → Generate → live SVG preview + Download SVG/PNG/Print buttons. (4) Wired in `src/mastra/index.ts` (import + spread + `/infographic` HTML page route) and `dashboard/js/navigation.js` Support menu (line 81). All endpoints sit behind global auth (verified NOT in `publicPaths` allowlist). Verified live: 6/6 SVG renders 200 (10–13KB each), PNG render 200 (995KB, 1200×1500). Sample at `exports/walaplus-platform-health-sample.png`.
- Native XLSX exports — phase 2 (Apr 18, 2026): Extended Excel exports to four more modules. New endpoints: (1) `GET /api/qms/nc/export-xlsx` (Summary + Nonconformances); (2) `GET /api/qms/capa-export-xlsx` (Summary + CAPAs — note hyphenated path, NOT `capa/export-xlsx`, because the GET `/api/qms/capa/:id` handler in `src/mastra/index.ts` shadows any `capa/<segment>` GET path and would 500 on `parseInt('export-xlsx')`); (3) `GET /api/vendors/export-xlsx` (Summary + Vendors + Assessments, joins `vendor_assessments` with vendor names); (4) `GET /api/risks/export-xlsx` in `riskRoutes.ts` (Summary + All Risks + one sheet per risk category + Treatment Actions, joining `risk_treatment_actions` with risk titles). All four cap at 50K rows, share the `excelExport.ts` utility (styled headers, frozen first row, auto-width, sheet-name dedup), and emit ISO date strings via a small `fmt(d)` helper. Emerald "Excel" buttons added on `dashboard/qms.html` (next to the existing CAPA CSV button AND a brand-new button next to the NC "New NC" button — NCs had no UI export before), `dashboard/vendors.html`, and `dashboard/risks.html`. Verified live: NC=7.9KB, CAPA=8KB, vendors=8.8KB, risks=9KB — all return correct XLSX MIME and parse cleanly with exceljs (Summary + data sheets present even with empty source tables, which is correct behaviour). Pulse remained 10/10 healthy throughout.
- Native XLSX exports — phase 1 (Apr 18, 2026): Put the previously-unused `exceljs` dependency to work with three multi-sheet Excel exports. New shared utility `src/utils/excelExport.ts` (`buildWorkbook(sheets, meta)` + `xlsxResponseHeaders`) handles styled headers, frozen first row, auto-width, and de-duplicated sheet names (≤31 chars, no `\/?*[]:`). Three new endpoints: (1) `GET /api/audits/:id/export-xlsx` in `auditRoutes.ts` — Summary + Findings sheets, accepts both UUID (GRC `audits` table) and numeric IDs; (2) `GET /api/duplicates/export-xlsx` in `duplicateRadarRoutes.ts` — Summary + per-record-type sheets (Leads/Deals/Contacts/Accounts), strips the heavy `raw_data` JSONB blob in-memory before serialisation, optional `?include_raw=1` adds an "All Records" sheet; (3) `GET /api/kpis/export-xlsx` in `qmsEnhancedRoutes.ts` — Summary + one sheet per category (ai/audit/compliance/governance/individual/process/quality/risk/training/vendor) + All Values sheet, capped at 50K values defensively. Dashboard buttons added: emerald "Excel" buttons on `dashboard/duplicates.html` and `dashboard/kpis.html` next to the existing CSV buttons; emerald "XLSX" link in the actions column of `dashboard/audits.html` next to the PDF link. Verified live: audit XLSX 8KB/0.3s, duplicates XLSX 3.2MB/6.7s (39,856 rows across 4 module sheets), KPI XLSX 20KB/0.03s — all return correct `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet` MIME and parse cleanly with exceljs.
