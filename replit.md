...
- `src/utils/` - Database utilities (audit, compliance, KPI, risk, policy, vendor, etc.) and security utilities:
  ...
  - `duplicateRadarDatabase.ts` - Multi-signal duplicate detection (email 40pts + domain 25pts + phone 30pts + company 20pts), cross-module clustering (Leads/Contacts/Deals/Accounts), upsert with ON CONFLICT (incremental scan via DUPLICATE_SCAN_MODE env), pg_trgm + GIN index for fuzzy company match, JOIN-based queries (no N+1), phone_normalized computed atomically, RAG owner accountability (green ≤2% / amber 2-5% / red >5%), auto-resolve engine, smart AI recommendations, enhanced summary with resolution rate + top signals + top clusters by inflation
