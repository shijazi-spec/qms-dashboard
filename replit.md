...
## Recent Changes
- Audit drill-down fairness fix (Apr 18, 2026): `directAuditRunner.ts` previously used a single global cap of 500 detailed per-record issues, populated in module order (Leads → Deals → Contacts → Tasks → Accounts). When the first 3 modules exhausted the cap, Tasks/Accounts had zero entries in `raw_audit_data.all_issues` and the drill-down modal showed a synthetic `summary_Tasks_1` row instead of real records. Replaced with a per-module cap (`MAX_DETAILED_PER_MODULE = 200`) tracked via a `Map<string, number>` passed into `analyzeRecordBatch`. All future audits will have real per-record drill-down data for every module. (Audits saved before the fix retain the old truncated data; re-run the audit to refresh.)
