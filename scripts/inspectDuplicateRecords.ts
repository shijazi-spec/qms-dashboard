 ...
 * directory came back empty. Run in the Replit shell:
 *
 *   npx tsx scripts/inspectDuplicateRecords.ts
 *
 * Prints: row counts by record_type and by zoho_module, a sample deal's
...
async function main() {
  ...
  console.log("\n=== row counts by zoho_module ===");
  const byMod = await pool.query(
    `SELECT zoho_module, COUNT(*) AS n FROM duplicate_records GROUP BY zoho_module ORDER BY n DESC`,
  );
  for (const r of byMod.rows) console.log(`  ${String(r.zoho_module ?? "(null)").padEnd(16)} ${r.n}`);

  console.log("\n=== cross-tab: record_type x zoho_module ===");
  const cross = await pool.query(
    `SELECT record_type, zoho_module, COUNT(*) AS n
       FROM duplicate_records GROUP BY record_type, zoho_module ORDER BY n DESC LIMIT 20`,
  );
  for (const r of cross.rows)
    console.log(`  rt=${String(r.record_type ?? "null").padEnd(10)} mod=${String(r.zoho_module ?? "null").padEnd(14)} ${r.n}`);

  console.log("\n=== sample deal raw_data keys (zoho_module='Deals') ===");
  const sample = await pool.query(
    `SELECT record_type, zoho_module, account_name, company_name, domain, raw_data
       FROM duplicate_records WHERE zoho_module = 'Deals' LIMIT 1`,
  );
  if (sample.rows.length) {
    const s = sample.rows[0];
    console.log(`  record_type=${s.record_type}  zoho_module=${s.zoho_module}`);
    ...
    console.log("  (no rows with zoho_module='Deals')");
  }
  ...
  for (const r of riyad.rows) {
    ...
    console.log(
      `  [${r.record_type}/${r.zoho_module}] name="${r.account_name || r.company_name}" domain=${r.domain} ` +
