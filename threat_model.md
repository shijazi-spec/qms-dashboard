...
- **High-value production surfaces for repeated scans:** `src/utils/rbacMiddleware.ts`, `src/mastra/routes/adminApiRoutes.ts`, `src/mastra/routes/policyRoutes.ts`, `src/mastra/routes/teamRoutes.ts`, `src/mastra/routes/managementReviewRoutes.ts`, `src/mastra/routes/reportRoutes.ts`, `src/mastra/routes/eventLogsRoutes.ts`, `src/mastra/routes/consultantRoutes.ts`, `src/mastra/tools/queryPlatformDataTool.ts`, `src/mastra/routes/knowledgeRoutes.ts`, `src/mastra/routes/onboardingRoutes.ts`, `src/mastra/routes/triggerRoutes.ts`, `src/mastra/routes/pmpRoutes.ts`, `src/utils/fileUpload.ts`, `src/utils/excelExport.ts`, `src/utils/reportGenerator.ts`, `dashboard/projects.html`, and pages with heavy `innerHTML` usage.
