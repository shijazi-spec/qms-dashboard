...
- **Admin-key browser login remains a top-priority trust-path review area.** `ADMIN_API_KEY` is not just an infrastructure secret; the app deliberately accepts it from browser JavaScript via `/api/admin/auth`, stores the raw secret in an `admin_key` cookie, and treats that cookie as an admin identity across many privileged routes. Repeated scans should treat this as a high-value anchor because it can become a weaker substitute for user-bound admin authentication.
