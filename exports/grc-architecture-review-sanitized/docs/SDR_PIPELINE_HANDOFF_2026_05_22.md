...
**Account owner:** Sample User (OpenAI billing + GitHub org)
...
**Plus:** 1 secret cleanup (deleted `AI_INTEGRATIONS_OPENAI_*` env vars in Replit), 1 OpenAI credit top-up (Sample User $10 prepaid).
...
3. **OpenAI `insufficient_quota`** — pre-paid credit had run out 2026-05-18 (per "Last used" on <REDACTED_HOST>). Sample User $10. Auto-recharge **still off** — recommended to enable.
...
5. **Replit modelfarm proxy hijacking traffic** — `AI_INTEGRATIONS_OPENAI_BASE_URL=<REDACTED_URL>` + `AI_INTEGRATIONS_OPENAI_API_KEY=_DUMMY_API_KEY_` routed every "OpenAI" call through Replit's internal proxy with a literal dummy key. The proxy was the actual source of the v1/v2 error. Fix: delete both env vars in Replit Secrets so code falls back to real `OPENAI_API_KEY` + `<REDACTED_HOST>/v1`.
...
- [ ] **Enable OpenAI auto-recharge** at <REDACTED_URL> — prevents silent quota outages (single biggest risk)
