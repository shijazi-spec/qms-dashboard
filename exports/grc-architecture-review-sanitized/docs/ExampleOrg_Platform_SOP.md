...
ExampleOrg is committed to delivering an enterprise-grade Quality Management System that ensures the accuracy, completeness, and governance of CRM data and business processes. We achieve this through:
...
**Document:** Sales SOP v1.1, Effective 2025-12-01, Prepared by Sample User
...
- Triggers appear in the ExampleOrg **Triggers** tab
...
| `qms.html` | `/qms` | ExampleOrg (Audit Reports) |
...
| 3 | CAPA status (open, overdue, effectiveness) | ExampleOrg (`/qms`) CAPA tab |
| 4 | NC status and resolution rates | ExampleOrg (`/qms`) NC tab |
...
### 18.1 Purpose
The internal audit program verifies that the ExampleOrg QMS platform itself operates in conformance with organizational requirements, ISO 9001 principles, and security policies. This is distinct from the CRM data audits performed by the AI agents.
...
- Course management is available in the ExampleOrg (`/qms`) Training tab
...
| CAPA effectiveness reviews | ExampleOrg (`/qms`) | Monthly |
| NC trend analysis | ExampleOrg (`/qms`) | Monthly |
...
ExampleOrg supports Arabic (RTL) in addition to English. The i18n system is a lightweight vanilla-JS module (`dashboard/js/i18n.js`) with no framework dependencies. It works on all static HTML dashboard pages and the navigation rail without a build step.
