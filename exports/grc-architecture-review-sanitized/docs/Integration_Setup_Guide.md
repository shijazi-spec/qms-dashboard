...
**Platform:** ExampleOrg QMS (https://<REDACTED_HOST>)

---

## Purpose

This document provides everything your **Zoho CRM Admin**, **Five9 Admin**, and **Google Workspace Admin** need to configure and connect their systems to the ExampleOrg QMS platform. Each section is self-contained — share the relevant section with the corresponding admin.
...
### What ExampleOrg Does with Zoho CRM

ExampleOrg connects to Zoho CRM to perform **read-only data quality audits** and **duplicate detection**. It reads records from the following modules:
...
**ExampleOrg does NOT write, update, or delete any Zoho CRM records.**

### What the Zoho Admin Needs to Do

#### Step 1: Create a Server-Side OAuth Client (Self Client)

1. Go to **Zoho API Console**: <REDACTED_URL>
...
| **Accounts URL** | `<REDACTED_URL>` (default) | `ZOHO_ACCOUNTS_URL` secret (optional) |
| **API Domain** | `<REDACTED_URL>` (default) | `ZOHO_API_DOMAIN` secret (optional) |
...
### What ExampleOrg Does with Five9

ExampleOrg ingests call recordings and metadata from Five9 for **SDR call quality evaluation**. The AI agents analyze calls for:
...
4. Note down:
   - **Domain** (your Five9 domain, e.g., `<REDACTED_HOST>` or your custom domain)
...
The Five9 integration is configured through the ExampleOrg admin panel:
...
ExampleOrg can ingest calls from multiple sources, not just Five9:
...
### What ExampleOrg Does with Google Calendar
...
ExampleOrg supports **Google OAuth login** so team members can sign in with their Google Workspace accounts instead of username/password.
...
6. Set **Authorized redirect URIs**:
   ```
   https://<REDACTED_HOST>/api/auth/google/callback
   ```
...
| `ZOHO_ACCOUNTS_URL` | No | Zoho accounts URL (defaults to US) | `<REDACTED_URL>` |
| `ZOHO_API_DOMAIN` | No | Zoho API domain (defaults to US) | `<REDACTED_URL>` |
...
2. **Zoho access is read-only** — ExampleOrg cannot modify CRM data
3. **Google Calendar access is read-only** — ExampleOrg cannot modify calendar events
...
**Distribution:** Zoho CRM Admin, Five9 Admin, Google Workspace Admin, ExampleOrg Platform Team
