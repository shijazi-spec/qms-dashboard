# Zoho OAuth Self Client — Setup for ExampleOrg ExampleOrg

**Audience:** Zoho admin (someone with admin rights at <REDACTED_HOST>)
...
1. Open **[<REDACTED_URL> in a new tab. Sign in with a Zoho admin account.
...
| HTML response instead of JSON | Wrong region URL | If your Zoho is `.eu`/`.in`/`.sa`, change `<REDACTED_HOST>` to `accounts.zoho.eu` etc. |

---

## Step 4 — Add the 3 secrets to Replit (~2 min)

1. In Replit, with **ExampleOrg** open, click **Tools → Secrets** in the left sidebar (lock-icon).
...
3. **Optional** — only if your Zoho org is in a non-US region:
   - `ZOHO_ACCOUNTS_URL` = `<REDACTED_URL>` (or `.in`, `.sa`, etc.)
   - `ZOHO_API_DOMAIN` = `<REDACTED_URL>` (or matching region)
...
1. Open `https://<REDACTED_HOST>/calls` in an **incognito window** (avoids cached state).
2. **Check the Overview tab.** The red banner *"Zoho CRM not connected"* should be **gone**.
3. **Open Replit → Tools → Shell** and run:
   ```bash
   curl -s https://<REDACTED_HOST>/api/integrations/status | jq
   ```
   Expected:
   ```json
...
- **Revoke if leaked.** Log into the Self Client app at <REDACTED_HOST> → "Generate Code" → repeat Step 2-3 to mint a fresh refresh_token, then update Replit Secrets and Republish. The old token stops working as soon as you regenerate.
- **Audit log.** Zoho logs every API call made with this credential under your admin account, traceable to `<REDACTED_HOST>`.
