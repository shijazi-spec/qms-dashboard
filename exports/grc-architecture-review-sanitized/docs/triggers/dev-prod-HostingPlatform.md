...
2. **Creates an Inngest function** with ID `api-linear` that:
   ...
   - Forwards the request to `<REDACTED_URL>`
...
ChatProvider and Telegram use paths like `/webhooks/ChatProvider/action` and `/webhooks/telegram/action`. Because `registerApiRoute` extracts the connector name from the first path segment, both resolve to the connector name "webhooks" instead of "ChatProvider" or "telegram". This means:

- Both listen for: `event/api.webhooks.webhooks.action` (shared event name)
- Function IDs: `api-webhooks` (same for both)
- Test scripts must send `event/api.webhooks.webhooks.action` for ChatProvider/Telegram
...
POST /linear/webhook ──► Handler receives
                         ...
                         Calls workflow.start() ────────────────────────────────┐
                                                  ...
                                                  Orchestrator (RAM)
                                                       ...
                                                       │
                                                       ▼
                                                  View in UI: HostingPlatform Playground
...
**Important:** You **never** deploy yourself. The HostingPlatform Publish wizard handles all deployment configuration automatically. This section explains what happens behind the scenes so you can understand the production architecture.
...
# HostingPlatform automatically registers your app with Inngest Cloud (<REDACTED_HOST>)
```
...
1. External webhook arrives
   Linear servers → HostingPlatform Webhook Service (HostingPlatform infrastructure)
2. HostingPlatform transforms to Inngest event
   HostingPlatform Webhook Service converts webhook to:
   event/api.webhooks.linear.action → sends to <REDACTED_HOST>
...
5. Container forwards to webhook handler
   ...
   fetch(`<REDACTED_URL>`) with original payload
...
8. Inngest orchestrates Step 1
   Inngest Cloud → Your Container
   HTTP POST to <REDACTED_URL> requesting Step 1
...
External Webhook     HostingPlatform Service        Inngest Cloud         Your Container
────────────────     ──────────────        ─────────────         ──────────────

POST /webhook    ──► Transforms to
                     inngest event     ──► <REDACTED_HOST>
                                           Finds function
                                           "api-linear"
                                                │
...
/**
 * This sends the EXACT event that HostingPlatform Webhook Service sends to Inngest Cloud.
 ...
 */
// Configuration
const provider = "linear";  // The webhook provider (e.g., "linear", "SourceControlProvider", "PaymentProvider")
 ...
 * It exercises the complete production flow.
 * 
 * SPECIAL CASE: ChatProvider and Telegram use /webhooks/{provider}/action paths,
 * so they both listen for "event/api.webhooks.webhooks.action" instead.
 */
// Determine the correct event name
let eventName: string;
if (provider === "ChatProvider" || provider === "telegram") {
...
console.log(`✅ Event sent for ${provider}! Check execution at: <REDACTED_URL>`);
```
...
- Webhooks flow: External → HostingPlatform Service → Inngest Cloud → Your Container
