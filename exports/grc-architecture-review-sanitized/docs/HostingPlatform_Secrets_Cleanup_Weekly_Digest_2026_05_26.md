...
**Linked code commit:** `9a11b3e` — *fix: decommission Weekly ChatProvider Digest per 3rd + 4th scope amendments*
...
The ChatProvider spam will stop **as soon as HostingPlatform republishes the latest commit**.

The env vars below are now dead weight — they're read by nothing. Removing them keeps HostingPlatform Secrets clean and prevents a future engineer from thinking the digest is still live based on the env-var presence.

---

## Pre-flight check (do this first)

Confirm HostingPlatform has pulled commit `9a11b3e` or later:

```
1. Open HostingPlatform → ExampleOrg
...
6. Open ChatProvider → #automatic-audits → confirm no new digest messages
```
...
Open HostingPlatform → **Secrets** (left sidebar, lock icon) and delete each of the following keys.
...
### ☐ 4. `ChatProvider_DIGEST_CHANNEL_ID`

- **What it did:** The ChatProvider channel id where the digest was posted (the `#automatic-audits` channel).
- **Caveat:** This secret is also referenced as a fallback for some other ChatProvider notification paths. **Only delete if you've confirmed nothing else uses it.**
...
2. Confirm in HostingPlatform → Secrets that the three rows are gone.
3. Confirm in ChatProvider `#automatic-audits` that no new digest messages arrive over the next 24h.
...
1. Set `DIGEST_DECOMMISSIONED_OVERRIDE=true` in HostingPlatform Secrets
...
- [ ] `WEEKLY_DIGEST` removed from HostingPlatform Secrets
- [ ] `WEEKLY_DIGEST_CRON` removed (or confirmed not present)
- [ ] `WEEKLY_DIGEST_RECIPIENTS` removed
- [ ] (Optional) `ChatProvider_DIGEST_CHANNEL_ID` evaluated for removal
