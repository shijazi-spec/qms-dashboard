...
| Schema | `src/utils/complianceDatabase.ts` | `seedDefaultRegulations` gains a `SACS-002` (Example Organization Cybersecurity Standard) entry — 9th framework, zero obligations by design. |
...
- **Its frequency changes from "Per Certificate" to "quarterly"**, which changes how Sample User's North Star composite is computed (this KPI now contributes on the same cadence as the other quarterly inputs, not per-certificate events).
