...
2. If the field is empty, domain TLD is inspected. Any pattern in
   `DUPLICATE_RADAR_GOV_DOMAIN_PATTERNS` (defaults to `.<REDACTED_HOST>`, `.gov`,
   `.<REDACTED_HOST>`) → government.
3. Default → private.
...
real web domain like `<REDACTED_HOST>`). The radar reads it out of `raw_data`
...
DUPLICATE_RADAR_GOV_DOMAIN_PATTERNS=.<REDACTED_HOST>,.gov,.<REDACTED_HOST>
...
Phase 3 was verified operationally healthy on <REDACTED_PHONE> BLOCK / 64 REVIEW
