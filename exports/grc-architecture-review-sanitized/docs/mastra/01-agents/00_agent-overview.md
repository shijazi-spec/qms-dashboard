...
> **📹 Watch**:  → An introduction to agents, and how they compare to workflows [YouTube (7 minutes)](<REDACTED_URL>
...
Agents can return structured, type-safe data by defining the expected output using either [Zod](<REDACTED_URL> or [JSON Schema](<REDACTED_URL> We recommend Zod for better TypeScript support and developer experience. The parsed result is available on `response.object`, allowing you to work directly with validated and typed data.

### Using Zod

Define the `output` shape using [Zod](<REDACTED_URL>
