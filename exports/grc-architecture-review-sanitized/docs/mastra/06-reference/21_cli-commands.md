...
Starts a server which exposes a [local dev playground](/docs/server-db/local-dev-playground) and REST endpoints for your agents, tools, and workflows. You can visit [<REDACTED_URL> for an overview of all available endpoints once `mastra dev` is running.
...
The `mastra build` command bundles your Mastra project into a production-ready Hono server. [Hono](<REDACTED_URL> is a lightweight, type-safe web framework that makes it easy to deploy Mastra agents as HTTP endpoints with middleware support.
...
Custom environment variables file to include. By default, includes `.env.development`, `.<REDACTED_HOST>`, and `.env`.
...
By default, Mastra collects anonymous information about your project like your OS, Mastra version or Node.js version. You can read the [source code](<REDACTED_URL> to check what's collected.
...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      description: "Credentials mode for requests. See <REDACTED_URL> for more info.",
      isOptional: true,
    },
  ]}
/>
