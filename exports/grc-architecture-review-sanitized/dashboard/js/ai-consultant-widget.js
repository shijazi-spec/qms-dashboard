(function() {
    ...
    function applyWidgetTranslations() {
        ...
        if (expandLink) {
            ...
            // Open the full conversation in a NEW TAB (Sample User 2026-06-18) so the
    ...
    function widgetAttachFeedback(bubble, messageId, promptText, responseText, promptVersion) {
        ...
        bar.innerHTML =
            '<button class="widget-feedback-thumb" title="Helpful" aria-label="Helpful">' +
              '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 <REDACTED_PHONE>.894l-3.5 7A2 <REDACTED_PHONE> 21h-4.017c-.<REDACTED_PHONE>.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.<REDACTED_PHONE>.905C11 <REDACTED_PHONE> 8v12m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5"/></svg>' +
            '</button>' +
            '<button class="widget-feedback-thumb" title="Not helpful" aria-label="Not helpful">' +
              '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14H5.236a2 <REDACTED_PHONE>.894l3.5-7A2 2 0 018.736 3h4.018c.<REDACTED_PHONE>.06L17 4m-7 10v2a2 2 0 002 2h.095c.<REDACTED_PHONE>.905C13 <REDACTED_PHONE> 13V1m-7 13h2M17 4h2a2 2 0 012 2v6a2 2 0 01-2 2h-2.5"/></svg>' +
            '</button>' +
    ...
    async function widgetSendMessage(text) {
        ...
        try {
            ...
            while (true) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    var line = lines[i];
                    if (line.startsWith('<REDACTED_SCHEME> ')) {
            ...
            if (buffer.trim() && buffer.startsWith('<REDACTED_SCHEME> ') && buffer.substring(6) !== '[DONE]') {
