              ...
              const blocks = [
                ...
                { type: "divider" },
                {
                  ...
                  elements: [
                    {
                      type: "mrkdwn",
                      text: `🕒 Generated ${new Date().toISOString().slice(0, 16).replace("T", " ")} UTC · ExampleOrg QMS`,
                    },
                  ],
                },
              ];
        ...
        try {
          ...
          const html = `
            <div style="font-family:-apple-system,'Segoe UI',Roboto,sans-serif;max-width:640px;margin:0 auto;padding:24px;color:#1e293b">
              ...
              <p style="margin:0;color:#64748b;font-size:12px">Generated from live ExampleOrg production data on ${stamp}.</p>
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        ...
        try {
          ...
          const svg = await buildSectionInfographic(section);
          const stamp = new Date().toISOString().slice(0, 10);
          const filename = `ExampleOrg-${section}-${stamp}.${wantPng ? "png" : "svg"}`;
