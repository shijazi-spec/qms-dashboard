...
import { z } from "zod";
import { fetchCalendarEvents, getCalendarList, CalendarEvent } from "../../utils/IdentityProviderCalendar";

export const fetchCalendarEventsTool = createTool({
  id: "fetch-calendar-events",

  description:
    "Fetches calendar events from IdentityProvider Calendar for a specified date range. Use this to audit meeting activities and validate CRM logging.",
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      ...
      logger?.info("📅 [fetchCalendarEventsTool] Fetching events from IdentityProvider Calendar API...");
...
export const listCalendarsTool = createTool({
  ...
  description:
    "Lists all available calendars from the connected IdentityProvider Calendar account.",
