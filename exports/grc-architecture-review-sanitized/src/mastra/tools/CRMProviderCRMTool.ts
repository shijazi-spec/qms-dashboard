...
export const auditCRMHygieneTool = createTool({
  ...
  description:
    "Performs a comprehensive data hygiene audit on CRMProvider CRM records. Checks for missing fields, invalid formats, and governance violations across Leads, Deals, Contacts, and Accounts modules.",
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      for (const moduleName of modules) {
        ...
        let records: CRMProviderCRMRecord[] = [];
        
        try {
          records = await fetchAllCRMProviderRecords(moduleName, { maxRecords: 50000 });
...
export const checkCRMActivityTool = createTool({
  ...
  execute: async ({ context, mastra }) => {
    ...
    try {
      const records = await fetchCRMProviderRecords(context.module, { perPage: 200 });
