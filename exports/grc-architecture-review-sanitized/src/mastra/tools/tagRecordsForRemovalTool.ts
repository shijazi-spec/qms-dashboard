...
import { z } from "zod";
import { addCRMProviderTags, CRMProviderWritesAllowedInEnv } from "../../utils/CRMProviderCRM";
...
export const tagRecordsForRemovalTool = createTool({
  ...
  description:
    "Flag CRMProvider CRM records for removal by applying the agreed removal tag (default \"Duplicate-Delete\") so the CRM admin can delete them. Use AFTER finding the records (e.g. via lookup-entity by phone/email/company) and confirming the list with the user. Provide the module and the CRMProvider record ids. This NEVER deletes records — it only tags them — and it is gated by the AI Approvals queue, so report the approval ticket and do NOT claim the tag was applied until it is approved.",
  ...
  inputSchema: z.object({
    ...
    recordIds: z
      .array(z.string())
      .min(1)
      .describe("CRMProvider record ids to tag (get these from lookup-entity first)"),
    tag: z
      .string()
      .optional()
  ...
  execute: async ({ context }) => {
    ...
    if (!CRMProviderWritesAllowedInEnv()) {
      return {
        ...
        tagged: 0,
        message:
          "Tagging is blocked outside production (dev shares production's CRMProvider credentials). Run it from the deployed app.",
        error: "live CRMProvider writes disabled outside production",
      };
    }
    try {
      await withTimeout(
        addCRMProviderTags(context.module, ids, [tag]),
      ...
      return {
        ...
        tag,
        tagged: 0,
        message: "Failed to apply the tag in CRMProvider.",
