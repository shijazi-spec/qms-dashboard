 ...
 * When Sample User, the approval route calls executeApprovedAction(),
...
export const duplicateResolutionExecuteTool = createTool({
  ...
  description:
    "Apply an approved duplicate-merge plan: migrate fields onto the survivor, tag duplicates 'Duplicate-Delete', optionally link to the account. Never deletes. Acts on behalf of Sample User.",
  inputSchema: z
    .object({
...
// executeApprovedAction() / isToolGated() can find it when Sample User.
