...
import { z } from "zod";
import { updateCRMProviderRecord, CRMProviderWritesAllowedInEnv } from "../../utils/CRMProviderCRM";
 ...
 * rule. CRMProvider can't merge across modules, so the correct fix for a Contact/Deal
 * that belongs under an existing Account is to set its Account_Name lookup.
 *
 * This IS a write tool (it sets Account_Name in CRMProvider via updateCRMProviderRecord). It
...
export const linkRecordToAccountTool = createTool({
  ...
  description:
    "Link CRMProvider Contacts or Deals to an existing Account by setting their Account_Name lookup (the cross-module LINK fix — CRMProvider can't merge across modules). Use when a contact/deal should sit under a different/existing account. Provide the module (Contacts or Deals), the CRMProvider record ids to relink, and the target Account's CRMProvider id (get all ids from lookup-entity first, and confirm with the user). This is a gated write — it enqueues an AI Approval, so report the approval ticket and do NOT claim the link was made until it is approved. You DO have write capability; you are not read-only — you just route writes through approval.",
  ...
  inputSchema: z.object({
    ...
    recordIds: z
      .array(z.string())
      .min(1)
      .describe("CRMProvider record ids of the contacts/deals to relink (from lookup-entity)"),
    accountCRMProviderId: z
      .string()
      .min(1)
      .describe("CRMProvider id of the target Account to link them under"),
    reason: z
      .string()
      .optional()
  ...
  execute: async ({ context }) => {
    ...
    const accountId = String(context.accountCRMProviderId || "").trim();
    if (!ids.length || !accountId) {
      return {
        success: false,
        module: context.module,
        accountCRMProviderId: accountId,
        linked: 0,
        failed: 0,
        message: "Provide both record ids and a target accountCRMProviderId.",
        error: "missing recordIds or accountCRMProviderId",
      };
    }
    if (!CRMProviderWritesAllowedInEnv()) {
      return {
        ...
        failed: 0,
        message:
          "Linking is blocked outside production (dev shares production's CRMProvider credentials). Run it from the deployed app.",
    ...
    for (const id of ids) {
      try {
        // Account_Name is a lookup field — CRMProvider expects the related record id.
        await withTimeout(
          updateCRMProviderRecord(context.module, id, {
