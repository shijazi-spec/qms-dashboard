        ...
        for (const moduleName of modules) {
          ...
          try {
            ...
            for (const record of records) {
              ...
              if (issues.length > 0) moduleRecordsWithIssues++;
            ...
            logger?.info(`✅ [Step 2] Completed ${moduleName} audit: ${moduleIssues.length} issues across ${moduleRecordsWithIssues} records`);
          } catch (error) {
...
`;
      ...
      const [sdrResponse, salesResponse] = await Promise.all([
        withAiTelemetry(
          { agentName: 'ExampleOrg SDR Quality Specialist', model: 'gpt-4o',
        ...
        withAiTelemetry(
          { agentName: 'ExampleOrg Sales Quality Specialist', model: 'gpt-4o',
...
`;
      ...
      const prompt = `
Based on the following quality audit data for ExampleOrg CRM and Calendar systems, provide:
...
`;
      ...
      const { result: response } = await withAiTelemetry<{ text: string }>(
        { agentName: 'ExampleOrg Quality Specialist', model: 'gpt-4o',
...
<body>
  <div class="header">
    <h1>🎯 ExampleOrg Weekly Quality Report</h1>
    <p>${reportDate}</p>
  </div>
  ...
  <div class="footer">
    <p>This report was automatically generated by the ExampleOrg Agentic AI Quality Specialist</p>
    <p>Powered by Mastra AI • ${new Date().toISOString()}</p>
      ...
      const textContent = `
ExampleOrg Weekly Quality Report
...
---
This report was automatically generated by the ExampleOrg Agentic AI Quality Specialist
`;
      ...
      const emailSubject = `${subjectPrefix} ExampleOrg Weekly Quality Report - ${subjectScore}`;
      ...
      try {
        ...
        const savedAudit = await saveAuditResult({
          ...
          dimension_details: {
            people: { score: qualityScores.peopleScore, weight: 0.30 },
            process: { score: qualityScores.processScore, weight: 0.30 },
          ...
          raw_audit_data: {
            ...
            // Truthful per-module unique-records-with-issues counts. Drives
            // the dashboard compliance % and the Data Quality Trend tile.
            // Source: per-module loop in step 2 (sdr/sales/ops sub-workflows
            // also feed this via combinedModuleBreakdown's recordsWithIssues).
            recordCountsByModule: (crmAudit.moduleBreakdown || []).reduce(
              (acc: Record<string, number>, m: any) => {
                if (m && m.module && typeof m.recordsWithIssues === 'number') {
                  acc[m.module] = m.recordsWithIssues;
                }
                return acc;
              },
