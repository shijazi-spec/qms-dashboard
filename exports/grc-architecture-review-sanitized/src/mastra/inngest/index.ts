...
const resolutionDigestFunction = inngest.createFunction(
  ...
  async ({ step }) => {
    return await step.run("post-resolution-digest", async () => {
      ...
      // can't post twice (Sample User 2026-06-22).
      ...
      try {
        ...
        if (capaResult.created > 0) {
          ...
          await createNotification({
            ...
            message: `${capaResult.created} CAPA(s) created (${capaResult.<REDACTED_TOKEN>} skipped — already open). Threshold: SAR ${Number(capaResult.threshold_sar).toLocaleString()}. Numbers: ${capaResult.capa_numbers.join(", ") || "—"}.`,
            action_url: "/duplicates",
            priority: "high",
          });
        }
      ...
      try {
        ...
        if (capaResult.created > 0) {
          ...
          await createNotification({
            ...
            message: `${capaResult.created} CAPA(s) created (${capaResult.<REDACTED_TOKEN>} skipped — already open). Severities: ${capaResult.severities.join(", ")}. Numbers: ${capaResult.capa_numbers.join(", ") || "—"}.`,
            action_url: "/duplicates",
            priority: "high",
          });
        }
