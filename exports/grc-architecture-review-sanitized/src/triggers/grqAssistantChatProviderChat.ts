/**
 * GRQ Assistant — two-way ChatProvider chat.
 *
 * Lets anyone in the resolution channel (Sample User, her manager) talk to the agent
 * in ChatProvider: @mention the bot, address it by name ("GRQ" / "hey GRQ"), or DM it,
 * and it replies in-thread using the same GRQ Assistant brain (qmsConsultantAgent
 * — full QMS/compliance/CRM tools). Conversation memory is per ChatProvider channel/
 * thread so it's an actual back-and-forth.
 *
 * Built on the existing registerChatProviderTrigger framework (handles ChatProvider's URL
 * verification, dedup, and bot-loop guard). Registered at /webhooks/ChatProvider/action
 * — set that as the Event Subscriptions Request URL in the ChatProvider app.
 *
 * ChatProvider app setup (one-time): Event Subscriptions → Request URL
 * <REDACTED_URL_SCHEME><host>/webhooks/ChatProvider/action → subscribe to bot events `app_mention`
...
import { registerChatProviderTrigger, getClient } from "./ChatProviderTriggers";
...
// Training). EVERYONE in the (private) ChatProvider channel inherits this access level
// — Sample User's governance decision (2026-06-11). Override via ChatProvider_ADAM_ROLE.
// Write tools stay approval-gated (autoApproveTier "never") so nothing executes
// from ChatProvider without landing in the platform AI Approvals queue.
 ...
 * it wasn't permissions (ChatProvider runs at the SAME role, above) or a shorter
 ...
 * extra steps cost latency/tokens, not a ChatProvider timeout.
 */
const ChatProvider_AGENT_MAX_STEPS = Number(process.env.ChatProvider_AGENT_MAX_STEPS) || 14;

/** ChatProvider truncates very long messages; split on line boundaries and post in
 *  order so a long answer arrives whole instead of being cut mid-list. */
const ChatProvider_MAX_CHARS = 3500;
export function splitForChatProvider(text: string, limit = ChatProvider_MAX_CHARS): string[] {
 ...
 * carry the LLMProvider org id / account internals into a chat channel.
 */
...
export function registerGrqAssistantChatProviderRoutes(): ApiRoute[] {
  return registerChatProviderTrigger({
    triggerType: "ChatProvider/grq-assistant",
    handler: (async (mastra: any, triggerInfo: any) => {
      try {
        ...
        const { ChatProvider } = await getClient();
        ...
        // (`ChatProvider-<channel>`) got stamped with the first speaker's resource;
        ...
        const ChatProviderUser = event.user || channel;
        const convThreadId = `ChatProvider-${channel}-${ChatProviderUser}`;
        const convResourceId = `ChatProvider-user-${ChatProviderUser}`;
        ...
        try {
          ...
          void import("../utils/adamTopicLog").then(({ recordQuestionSection }) =>
            recordQuestionSection(q, { surface: "ChatProvider", askedBy: `ChatProvider-${ChatProviderUser}` }),
          ).catch(() => {});
          const res = await generateWithRateLimitRetry(async () => {
            ...
            //
            // NO promptText is passed. ChatProvider questions carry client company
            ...
            // LLMProviderCallMetric/finalizeAiCallMetric, so this can never break
            // a chat turn.
            const { result } = await withAiTelemetry(
              {
                agentName: "ExampleOrg QMS Consultant",
                model: "gpt-4o",
                userId: `ChatProvider-${ChatProviderUser}`,
                ...
                withAgentUserContext(
                  {
                    user: {
                      userId: null,
                      email: `ChatProvider-${ChatProviderUser}`,
                      role: ChatProvider_ADAM_ROLE,
                  ...
                  () =>
                    agent.generate([{ role: "user", content: q }], {
                      ...
                      maxSteps: ChatProvider_AGENT_MAX_STEPS,
                    }),
                ),
            );
            return result;
        ...
        for (const part of splitForChatProvider(reply)) {
          await ChatProvider.chat.postMessage({ channel, thread_ts: threadTs, text: part });
        }
        return null;
      } catch {
        // Never throw out of the webhook (it would 500 to ChatProvider and trigger retries).
        return null;
      }
    }) as any,
  } as any);
