/**
 * ChatProvider Consultant Rating Handler — Task #763
 *
 * Helper used by the ChatProvider thumbs-up/down bot (and the pattern any future
 ...
 *
 * Other surfaces (ChatProvider bot, mobile app, embedded widget) typically only
 ...
 * making it impossible to tell whether a regression is a ChatProvider-only
 ...
 *      - `surface`    — short literal: 'ChatProvider' | 'mobile' | 'embedded'
 *      - `userHash`   — anonymous-safe identifier (ChatProvider user id, etc.)
...
export interface RecordConsultantRatingInput {
   ...
   * Use a short literal — `'ChatProvider'`, `'mobile'`, `'embedded'` — to keep
   * the dashboard breakdown stable.
   */
  surface: ConsultantRatingSurface;
  /**
   * Stable identifier for the rater. ChatProvider passes the user id; mobile
 ...
 * (ChatProvider action handler, mobile API route, etc.) can decide whether to
...
export async function recordConsultantRatingFromSurface(
  ...
  if (!Number.isFinite(callId) || callId <= 0) {
    logger.warn("[ChatProviderRatingHandler] invalid callId", { callId });
    return { success: false, promptVersion: null };
  }
  if (rating !== "thumbs_up" && rating !== "thumbs_down") {
    logger.warn("[ChatProviderRatingHandler] invalid rating", { rating });
    return { success: false, promptVersion: null };
  }
