...
export async function getEmptyDeals(): Promise<EmptyRecordRow[]> {
  const q = await pool.query(
    `SELECT CRMProvider_record_id, record_name, owner_name,
      ...
      WHERE record_type='deal'
        ...
        -- (Sample User 2026-06-30). These two stages mean a live merchant relationship.
  ...
  for (const r of q.rows) {
    ...
    // Operator decision (Sample User 2026-06-26): a deal that has REAL data but is just
...
export async function reconcileDeletedRecords(
  ...
  // activeClustersOnly (Sample User 2026-07-13): restrict the live-verify to records
  ...
  // clusterIds (Sample User 2026-07-15): scope the verify to SPECIFIC clusters — the
    ...
    await Promise.all(chunk.map(async (row) => {
      ...
      try {
        ...
        if (!rec) {
          ...
          // Converted lead (Sample User 2026-07-13): it has ALREADY become an
...
//   'test'     — structurally empty AND a test/placeholder/ExampleOrg name
