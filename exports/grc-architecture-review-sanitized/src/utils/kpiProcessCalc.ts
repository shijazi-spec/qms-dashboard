...
// Per Sample User (2026-06-16): all SDR calls are captured in the platform's Call
 ...
 *
 * 1. CORPORATE (ExampleOrg layout) ONLY. The Deals table also holds Marketplace
 ...
 *
 * 3. REVENUE and ASP are CALENDAR YEAR-TO-DATE (Sample User, 2026-08-18), windowed on
...
export async function adhocSalesAggregates(): Promise<AdhocSalesAggregates> {
  ...
  adhocInFlight = (async () => {
    const seg = buildSegmentPredicate("ExampleOrg", 1);
...
export async function calcAdhocSalesQualifiedPipeline(): Promise<ProcessKpiValue> {
  ...
  return {
    ...
    dataAvailable: true,
    details: { open_deals: a.openCount, segment: "ExampleOrg" },
  };
}
 ...
 * "Coverage by Framework" data (per Sample User, 2026-06-16). Each certification /
 ...
 * per Sample User (2026-06-16). NOTE: this is the document REVIEW CYCLE (Draft → Review
