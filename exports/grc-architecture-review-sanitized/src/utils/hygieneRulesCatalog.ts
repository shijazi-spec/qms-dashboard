 ...
 * Adam — Scope of Work screen (GET /api/duplicates/hygiene-rules) so Sample User
...
export const HYGIENE_RULES_CATALOG: HygieneRuleGroup[] = [
  // ── Cross-cutting (always shown) ───────────────────────────────────────────
  {
    ...
    rules: [
      ...
      { id: "gen-hitl", rule: "Every CRMProvider write is gated (Human-in-the-Loop): it queues for approval (admin password) before applying. Attribution: 'Adam — GRQ Assistant (on behalf of Sample User)'. Shadow default = no autonomous writes; kill switch always live." },
      ...
      { id: "gen-grade2", rule: "Grade 2 (Assistant) for the auto-merge rules. Grades: G1 Trainee → G2 Assistant → G3 Trusted → G4 Autonomous Specialist (computed from agreement/override metrics; promotion is a human decision, never self-promotion). For these high-confidence, admin-gated auto-merges Adam acts as an ASSISTANT — he surfaces, previews and assists the merges; he NEVER claims they run unattended, and every CRMProvider write still requires the admin password (HITL).", ref: "Sample User 2026-06-22" },
    ],
  },
