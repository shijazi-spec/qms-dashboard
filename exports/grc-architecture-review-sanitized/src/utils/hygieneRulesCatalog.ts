 ...
 * AssistantPersona — Scope of Work screen (GET /api/duplicates/hygiene-rules) so Sample User
...
export const HYGIENE_RULES_CATALOG: HygieneRuleGroup[] = [
  // ── Cross-cutting (always shown) ───────────────────────────────────────────
  {
    ...
    general: true,
    rules: [
      { id: "gen-scope", rule: "CORPORATE / B2B company records only. Marketplace & Merchant records are out of scope. A record is corporate unless it carries an explicit merchant layout (Marketplace, Partner Accounts).", ref: "Sample User 2026-06-17" },
      { id: "gen-freemail", rule: "Free-mail domains (gmail, yahoo, ymail, hotmail, outlook…) and ExampleOrg house domains are NOT a clustering signal. Placeholder names (N/A, Not Provided…) go to a quarantine bucket, not a real duplicate." },
      ...
      { id: "gen-resolved-merged", rule: "'Resolved' ≠ 'Merged'. A cluster is only truly resolved once tagged records are confirmed deleted in CRMProvider. Report the real applied/merged figure, never the cluster 'resolved' count." },
      { id: "gen-hitl", rule: "Every CRMProvider write is gated (Human-in-the-Loop): it queues for approval (admin password) before applying. Attribution: 'AssistantPersona — GRQ Assistant (on behalf of Sample User)'. Shadow default = no autonomous writes; kill switch always live." },
      { id: "gen-safe-tier", rule: "Safe-tier auto-apply requires ALL clear: confidence ≥85, <2 domains, <2 phones, no CS block/review, pipeline & ARR ≤ cap, no active deal stage, 0 field conflicts, all CRMProvider ids present, not cross-module, <2 layouts, ≤1 owner, nothing modified <7 days, no failed prior verification, no custom-field assumption, survivor ≥50% complete, no attachments. Any one missing → escalate." },
      { id: "gen-automerge-lifecycle", rule: "A bulk auto-merge marks each touched cluster 'AI-Applied · pending CRMProvider admin delete'. It flips to 'Resolved' ONLY after the admin actually deletes the tagged duplicates in CRMProvider (a per-sync reconcile detects the deletion, then records the merge). A failed tag (e.g. CRMProvider rate-limited because a sync is running) leaves the cluster Untouched and re-runnable — it is never marked pending on a failed write. Run auto-merges when no sync is in progress. Large runs apply in BATCHES so they can't hit the proxy timeout.", ref: "Sample User 2026-06-22" },
      { id: "gen-apply-all-safe", rule: "ONE-CLICK 'Apply ALL safe merges (accounts + contacts)' button — runs every safe matcher in batches behind a SINGLE admin password: Accounts (same domain+name) first so contacts re-parent onto the merged survivor, then Contacts (exact email+phone, then same name+phone). Same conservative rules + dry-run→tag→admin-deletes safety as the individual buttons; loops until nothing's left. Use it to clear the safe backlog fast. Endpoint POST /api/duplicates/apply-all-safe.", ref: "Sample User 2026-06-25" },
      { id: "gen-grade2", rule: "Grade 2 (Assistant) for the auto-merge rules. Grades: G1 Trainee → G2 Assistant → G3 Trusted → G4 Autonomous Specialist (computed from agreement/override metrics; promotion is a human decision, never self-promotion). For these high-confidence, admin-gated auto-merges AssistantPersona acts as an ASSISTANT — he surfaces, previews and assists the merges; he NEVER claims they run unattended, and every CRMProvider write still requires the admin password (HITL).", ref: "Sample User 2026-06-22" },
    ],
  },
  ...
  },
  {
    ...
    rules: [
      ...
      { id: "con-genuine", rule: "The Contacts tab shows a cluster ONLY when ≥2 of its contacts DIRECTLY share an identity signal (email, phone, or name). Same-account colleagues who share only the company domain (a 'chained match') are NOT duplicates — they are a LINK-to-account job (Account-merge cascade / LLMProvider Plan → Link), never tagged. Within a cluster only the truly matching pair is shown; the rest drop off.", ref: "Sample User 2026-06-22" },
      { id: "con-automerge", rule: "TWO contact auto-merge buttons (dry-run preview → admin password → batched apply, never delete): (1) EXACT email+phone — same person → keep the most-complete survivor, preserve a duplicate's second NUMBER onto the survivor (gap-fill Mobile, migrate-then-tag), then tag the duplicates; (2) SAME name+phone — same person even when emails or numbers differ or one is missing: keep BOTH emails (primary Email + the other in Secondary_Email; a lone email becomes the primary) AND BOTH numbers (primary Phone + the other in Mobile) on the survivor, then tag the duplicates. Phone preservation is GAP-FILL only — it never overwrites a populated Mobile. CRMProvider Contacts holds two of each (Email + Secondary_Email, Phone + Mobile); a 3rd+ distinct value is surfaced as a warning, not dropped. Matching is Arabic-aware (fold ة/ه, ى/ي, آأإ→ا, strip invisible marks/tatweel) and Saudi-phone-canonical (drop 0 / +966, compare the last 9 digits).", ref: "Sample User 2026-06-24" },
    ],
  },
  {
    key: "<REDACTED_SECRET>",
