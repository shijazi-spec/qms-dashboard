...
export const HYGIENE_RULES_CATALOG: HygieneRuleGroup[] = [
  // ── Cross-cutting (always shown) ───────────────────────────────────────────
  {
    ...
    rules: [
      ...
      { id: "gen-hitl", rule: "Every Zoho write is gated (Human-in-the-Loop): it queues for approval (admin password) before applying. Attribution: 'Adam — GRQ Assistant (on behalf of Sample User)'. Shadow default = no autonomous writes; kill switch always live." },
  ...
  // ── Cross-module & lifecycle tabs ──────────────────────────────────────────
  {
    ...
    rules: [
      { id: "xm-lead", rule: "Lead ↔ anything: Zoho can't link a Lead — fix is CLOSE the Lead (convert into the existing Account or close as duplicate). Pure Lead↔Contact / Lead↔Account clusters are hidden here (handled on the Leads tab).", ref: "Sample User 2026-06-16" },
