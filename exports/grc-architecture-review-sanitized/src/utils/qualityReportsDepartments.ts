...
export interface QualityReportBUSeed {
   ...
   * Sample User 2026-08-16 the SDR KPIs belong to **SDR B2B only** — B2C must not
...
export const SEED_BUS: QualityReportBUSeed[] = [
  // Catalog-KPI owners are seeded on the B2B rows ONLY (Sample User 2026-08-16):
...
export async function ensureQualityReportTables(): Promise<void> {
  ...
  for (const b of SEED_BUS) {
    ...
    // kpi_bu_name is never clobbered (Sample User 2026-08-09).
    await pool.query(
