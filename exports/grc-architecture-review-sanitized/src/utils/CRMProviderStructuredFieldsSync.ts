/**
 * CRMProvider structured-fields sync — promotes call-evaluation results from
 * a free-form Note (the existing /sync-CRMProvider behavior) into actual
 * filterable fields on the CRMProvider Lead/Deal record.
 *
 ...
 * /api/calls/:callId/sync-CRMProvider. Notes cannot be filtered, reported on,
 * or used in CRMProvider workflow triggers." Promoting to structured fields
 * unlocks native CRMProvider dashboards filtered by QA_Score and lets the
 * sales manager build workflow triggers like "QA_Score < 60 → alert".
 *
 * --------------------------------------------------------------------
 *  PREREQUISITE — CRMProvider admin must create these custom fields FIRST
 * --------------------------------------------------------------------
 *
 *  On BOTH the Leads and Deals modules, the CRMProvider admin needs to add:
 *
 ...
 *
 *  The actual API field names CRMProvider assigns depend on the field labels
 *  the admin types in. Override the defaults via env vars in HostingPlatform
 *  Secrets if the admin chose different names:
 *
 *    CRMProvider_FIELD_QA_SCORE         (default: QA_Score)
 *    CRMProvider_FIELD_COMPLIANCE_PASS  (default: Compliance_Pass)
 *    CRMProvider_FIELD_LAST_EVAL_DATE   (default: Last_Evaluation_Date)
 *
 * --------------------------------------------------------------------
 *  Behavior
 * --------------------------------------------------------------------
 *
 *  Feature-flagged on CRMProvider_STRUCTURED_FIELDS. Ships disabled; flip on
 *  AFTER the CRMProvider admin has created the fields, otherwise the PATCH
 *  will 400 with "Invalid field" and leave a noisy log trail.
 *
 *  The existing Note-write at /api/calls/:callId/sync-CRMProvider is NOT
...
export interface CRMProviderStructuredFieldNames {
  qaScore: string;
  compliancePass: string;
  lastEvalDate: string;
}

export function readFieldNames(): CRMProviderStructuredFieldNames {
  return {
    qaScore: process.env.CRMProvider_FIELD_QA_SCORE || "QA_Score",
    compliancePass:
      process.env.CRMProvider_FIELD_COMPLIANCE_PASS || "Compliance_Pass",
    lastEvalDate:
      process.env.CRMProvider_FIELD_LAST_EVAL_DATE || "Last_Evaluation_Date",
  };
}
...
export interface BuildPatchResult {
  /** Patch object to send as CRMProvider PUT body — empty {} if nothing to patch. */
...
/**
 * Build the CRMProvider PATCH payload from an evaluation row. Pure function —
 * no I/O. Skips fields whose source values are null/undefined so we
 * never overwrite a CRMProvider cell with empty when the platform doesn't
 * have a value to write.
 */
export function buildEvaluationPatch(
  ev: EvaluationPatchInput,
  fields: CRMProviderStructuredFieldNames = readFieldNames(),
): BuildPatchResult {
  ...
  //       anything else to write, stamp "now" so CRMProvider's
  ...
  if (evalDate) {
    // CRMProvider expects YYYY-MM-DD for Date fields, YYYY-MM-DDTHH:MM:SS+0000
...
export interface SyncStructuredFieldsResult {
  ...
  fields?: string[];
  /** CRMProvider module the patch went to. */
  module?: "Leads" | "Deals";
  /** CRMProvider record ID the patch went to. */
  record_id?: string;
}

/**
 * Promote an evaluation to CRMProvider structured fields. Best-effort —
 ...
 *   4. PUT runs; on failure → <REDACTED_TOKEN>='CRMProvider_error' (and logged)
 */
export async function syncEvaluationToCRMProviderStructuredFields(
  ...
  if (!isFlagEnabled("CRMProvider_structured_fields", identity)) {
  ...
  try {
    const { updateCRMProviderRecord } = await import("./CRMProviderCRM");
    await updateCRMProviderRecord(module, recordId, patch);

    logger?.info("✅ [CRMProviderStructuredFields] PATCH succeeded", {
    ...
    safeLogger.warn("[CRMProviderStructuredFields] PATCH failed", {
      callId: callRecord.id,
      module,
      recordId,
      fields: fieldsIncluded,
    ...
    return {
      synced: false,
      <REDACTED_TOKEN>: "CRMProvider_error",
      fields: fieldsIncluded,
      module,
      record_id: recordId,
    };
