import { WebClient, type ChatPostMessageResponse } from '@ChatProvider/web-api';
import { logger } from './logger';

let ChatProviderClient: WebClient | null = null;

function getChatProviderClient(): WebClient | null {
  if (ChatProviderClient) return ChatProviderClient;
  const token = process.env.ChatProvider_BOT_TOKEN || process.env.ChatProvider_API_TOKEN;
  if (!token) {
    logger.info('[ChatProvider Notifications] No bot token configured');
    return null;
  }
  ChatProviderClient = new WebClient(token);
  return ChatProviderClient;
}

function getChannel(): string | null {
  return process.env.ChatProvider_CHANNEL_ID || process.env.ChatProvider_QMS_CHANNEL || null;
}
...
/**
 * Post a ChatProvider message and return both the delivery status and the message
 * `ts` (timestamp id) the ChatProvider API returned. The `ts` is needed by callers
 ...
 */
export async function postChatProviderMessage(
  channel: string,
  text: string,
  blocks?: any[],
  thread_ts?: string,
): Promise<{ ok: boolean; ts: string | null }> {
  const client = getChatProviderClient();
  ...
  try {
    ...
    logger.info(`[ChatProvider Notifications] Message sent to ${channel}`);
    const ts = typeof res.ts === "string" ? res.ts : null;
    return { ok: true, ts };
  } catch (error: any) {
    logger.error('[ChatProvider Notifications] Failed to send', { error: error?.message || String(error) });
 ...
 * threading without switching to `postChatProviderMessage`.
 */
export async function sendChatProviderNotification(
  channel: string,
  text: string,
  blocks?: any[],
  thread_ts?: string,
): Promise<boolean> {
  const r = await postChatProviderMessage(channel, text, blocks, thread_ts);
  return r.ok;
}
...
export async function sendAuditCompletedNotification(
  ...
  return sendChatProviderNotification(channel, fallback, blocks);
}
