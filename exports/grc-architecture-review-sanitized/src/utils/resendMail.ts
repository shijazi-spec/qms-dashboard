...
const FROM_EMAIL =
  process.env.RESEND_FROM_EMAIL || "ExampleOrg QMS <user@example.invalid>";
...
export async function sendResendEmail(options: ResendEmailOptions): Promise<{
  ...
  if (!resend) {
    logger.warn(
      `⚠️ [ResendMail] Email service not configured - email will not be sent`,
    );
    return {
      success: false,
      error:
  ...
  try {
    ...
    logger.info(`📧 [ResendMail] Sending email to: ${recipients.join(", ")}`);
    logger.info(`📧 [ResendMail] From: ${fromEmail}`);
    logger.info(`📧 [ResendMail] Subject: ${options.subject}`);
    logger.info(`📧 [ResendMail] API Key length: ${currentApiKey?.length}`);
    ...
    if (error) {
      logger.error("❌ [ResendMail] Failed to send email:", error);
      return {
        success: false,
        error: error.message,
      };
    ...
    logger.info(`✅ [ResendMail] Email sent successfully. ID: ${data?.id}`);
    return {
      success: true,
      id: data?.id,
    };
    ...
    logger.error("❌ [ResendMail] Exception sending email:", errorMessage);
    return {
      success: false,
      error: errorMessage,
    };
