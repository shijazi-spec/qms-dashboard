 ...
 *
 * The classifier was rewritten on 2026-06-11 (Sample User): BLOCK now
...
test("BLOCK: open Proposal deal + Paid handoff deal + Adoption phase", () => {
  const deals: ClusterDealInfo[] = [
    {
      stage: "Proposal",
      cs: { phase: null, gov_type: "Private", domain: "<REDACTED_HOST>" },
      arr_value: 0,
    },
    {
      stage: "Paid",
...
test("BLOCK: open Awaiting PO deal + Agreement Signed handoff in Renewal", () => {
  const deals: ClusterDealInfo[] = [
    {
      stage: "Awaiting PO",
      cs: { phase: null, gov_type: "Government", domain: "<REDACTED_HOST>" },
      arr_value: 0,
    },
    {
      stage: "Agreement Signed",
      cs: { phase: "Renewal", gov_type: "Government", domain: "<REDACTED_HOST>" },
      arr_value: 500_000,
    },
  ];
  const out = classifyClusterOverlap(deals, NOW);
...
test("null: only handoff Paid deal exists, no open sales deal", () => {
  const deals: ClusterDealInfo[] = [
    {
      stage: "Paid",
      cs: { phase: "Adoption", gov_type: "Private", domain: "<REDACTED_HOST>" },
      arr_value: 1_000_000,
    },
  ];
  const out = classifyClusterOverlap(deals, NOW);
