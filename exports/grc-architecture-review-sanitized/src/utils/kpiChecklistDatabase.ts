...
/**
 * Standard framework-build action plan applied to every BU — Sample User's actual
 * methodology (generalized from the CS Action Plan, <REDACTED_PHONE> phases ending
  ...
  for (const bu of markAll) {
    ...
    const r = await pool.query(
      `UPDATE kpi_checklist_items SET is_done = true, updated_at = NOW()
        WHERE kpi_id = $1 AND section = $2
          AND (updated_by IS NULL OR updated_by = 'Sample User')`,
      [kpi.id, bu],
    );
