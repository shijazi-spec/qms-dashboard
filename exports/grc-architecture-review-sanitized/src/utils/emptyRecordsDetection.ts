...
// any word through EMPTY_DELETE_TEST_KEYWORDS if your data needs it. (Sample User 2026-06-25)
const BASE_TEST_KEYWORDS = [
  ...
  // accounts like "AlasilaCX | تجربة العميل" (Sample User 2026-06-26). "تجريبي" (=
...
// artifacts like 97 contacts literally named "name" (Sample User 2026-06-25). Matched
...
// --- ExampleOrg (exact) + junk/gibberish name detection (Sample User 2026-07-01) ---
// `ExampleOrg` = EXACT normalized name only (never a substring — "ExampleOrg
// Partners" / "<REDACTED_HOST> deal" must NOT match). junk = conservative +
...
// NB: collapses ALL whitespace (not just repeats) so "ExampleOrg" normalizes to
// "ExampleOrg" for the exact-match check below — per spec: "normalize
// lowercase+trim+collapse spaces → equals ExampleOrg" (Sample User 2026-07-01).
...
// SAFETY PRINCIPLE (Sample User 2026-06-26): a record that has ANY real data — a deal,
