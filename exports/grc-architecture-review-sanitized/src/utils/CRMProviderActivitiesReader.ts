import { fetchAllCRMProviderRecords, type CRMProviderCRMRecord } from './CRMProviderCRM';
...
function toItem(kind: ActivityKind, record: CRMProviderCRMRecord): ActivityItem {
...
function isTaskOpen(record: CRMProviderCRMRecord): boolean {
...
function isMeetingOpen(record: CRMProviderCRMRecord, now: number): boolean {
  const d: any = record.data || {};
...
function isCallOpen(record: CRMProviderCRMRecord, now: number): boolean {
  const d: any = record.data || {};
...
async function safeFetch(
  ...
  try {
    return await fetchAllCRMProviderRecords(module, {
      criteria,
      fields: fields.split(','),
      maxRecords: 500,
    });
    ...
    logger.warn(`[CRMProviderActivitiesReader] ${label} fetch failed: ${msg}`);
    errors.push(`${label}: ${msg}`);
    return [];
  }
}

export async function getCRMProviderActivitiesForRecord(
  parentModule: ParentModule,
  parentId: string,
