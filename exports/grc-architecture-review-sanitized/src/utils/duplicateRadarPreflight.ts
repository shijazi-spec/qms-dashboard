 ...
 * single corporate layout string ("Corporate Sales (ExampleOrg Layout)") and so
...
/**
 * Preflight rule mode (Sample User 2026-06-18).
 *
 ...
 * means the company is a live customer. Sample User 2026-06-18: STRICTLY these two
...
export interface PreflightResponse {
   ...
   * "Preflight check — generated <REDACTED_PHONE>:00 UTC".
   */
  generated_at: string;
...
export function buildExecutiveAction(input: {
   ...
   * Qualified, Disqualified, Converted}. Sample User 2026-06-17 — a
  ...
  if (input.verdict === "duplicate") {
    ...
    // Sample User 2026-06-17 — active-lead REJECT rule. If the cluster has at
    ...
    // the routing-call mess Sample User.
    ...
    // Sample User 2026-06-17 — Closed-Lost-only clusters are NOT a hard block:
 ...
 * masquerade as a real phone/email and slip a row through (Sample User 2026-07-22).
 */
 ...
 * Egyptian mobile from a 10-digit Saudi one. Sample User 2026-07-22/23: "it shall
...
/**
 * KSA-scope phone check (Sample User 2026-07-14). Preflight screens Saudi-market
...
// ─── DOAM client directory (Sample User 2026-07-14) ───────────────────────────────
// DOAM = government entities subscribed to ExampleOrg THROUGH the HR ministry
...
// <REDACTED_HOST> → <REDACTED_HOST>) or exact gov NAME (EN or AR). Source list
...
export function matchDoamClient(input: {
  ...
  if (dom) {
    ...
    // sub-domain of a DOAM gov domain (e.g. a hospital under <REDACTED_HOST>)
 ...
 * email provider, not the company. Sample User 2026-06-17 — even if every
...
/**
 * Strategic-domain suffixes (Saudi government and education). Sample User
...
const PROTECTED_ACCOUNTS: ReadonlyArray<ProtectedAccount> = [
  ...
  },
  {
    ...
    // verified from the CRM record (NOT <REDACTED_HOST>).
...
export interface PreflightClusterRow {
   ...
   * Lead_Status is NOT in {Junk Lead, Bogus Lead, Lost Lead, Not
   * Qualified, Disqualified, Converted}. Sample User 2026-06-17 — a vendor
...
// is therefore pursuable, not a hard reject. 2026-06-18 (Sample User): added "new" and
...
export function classifyPreflightRows(input: {
  ...
  for (let i = 0; i < examineCount; i++) {
    ...
    // Sample User 2026-06-17 corporate-scope rule — a cluster whose
    ...
    if (!matched || isOutOfScope) {
      // Sample User 2026-06-17 safety shields — before letting a row PASS,
      ...
      //
      // (2) Strategic Saudi domains (.<REDACTED_HOST>, .<REDACTED_HOST>). These are
      ...
      if (isStrategic) {
        out.push({
          ...
          suggested_action:
            "Strategic Saudi Government / Education entity (.<REDACTED_HOST> / .<REDACTED_HOST>). Route through Head of Sales before any SDR contact.",
          executive_action:
    ...
    // ── Sample User / Sample User 2026-06-17 — unreliable non-domain match guard ──
    ...
    // Sample User 2026-06-17 — when corporate-only counts are available
...
/**
 * RULE 3 — ACTIVE OPEN SALES DEAL (Sample User 2026-07-21).
 *
...
/**
 * RULE 2 (v2) — existing-client verdict (Sample User 2026-06-22).
 *
...
// (Sample User 2026-06-24: SAJA≠Hekma, Kasab≠Tarsheed, Rawabi Offshore≠Rawabi Holding,
...
const CS_DIR_STOP = new Set([
  ...
  // collided with an unrelated single-token client entry (Sample User 2026-06-24).
...
/**
 * OPEN-DEAL DIRECTORY (Sample User 2026-07-21).
 *
...
export async function getCsClientDirectory(todayMs: number): Promise<CsClientDirectory> {
  ...
  // NOT the deal owner (Sample User 2026-06-24). It's a user-lookup, so it can be an
  ...
  // ── DOAM clients synced into the platform's client directory (Sample User
  // 2026-07-14) ──────────────────────────────────────────────────────────────
  // DOAM = government entities subscribed to ExampleOrg via the HR ministry;
  ...
  // caller) treat them as existing ExampleOrg clients, so they never surface as
...
// safely infer (Sample User 2026-07-26) — e.g. a missing dot. Keyed by the cleaned
...
//   sa.eycom → <REDACTED_HOST> (EY Saudi; reduces to <REDACTED_HOST> and merges with it).
//   <REDACTED_HOST> → <REDACTED_HOST> (Alwaleed
//     Philanthropies; "phialn" is a misspelling of "philan").
// NB: <REDACTED_HOST> is NOT corrected — it is a real, separate company (Saey Steel,
// <REDACTED_HOST> → <REDACTED_HOST>), not an EY typo. Verified 2026-07-26.
const CLIENT_DOMAIN_CORRECTIONS: Record<string, string> = (() => {
  const m: Record<string, string> = {
    "sa.eycom": "<REDACTED_HOST>",
...
// CentralNic domain-hack suffixes like <REDACTED_HOST>). Longest-match wins so e.g.
// <REDACTED_HOST> stays a 3-label registrable domain, NOT collapsed to "<REDACTED_HOST>",
// and <REDACTED_HOST> is kept distinct from <REDACTED_HOST>. Extend as needed.
const CLIENT_DOMAIN_MULTI_SUFFIX = new Set([
  "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>",
  "com.eg", "com.qa", "com.kw", "com.bh", "com.om", "com.ae", "gov.ae", "co.ae",
  "net.ae", "org.ae", "co.uk", "org.uk", "gov.uk", "ac.uk", "com.au", "com.tr",
  "co.in", "com.pk", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>", "<REDACTED_HOST>",
]);

/**
 * Clean + normalise a client domain to its registrable form, or null if it is
 * junk (Sample User 2026-07-26 hygiene pass). Steps: lowercase; strip scheme /
 ...
 * the registrable domain (so `<REDACTED_HOST>` and `<REDACTED_HOST>`
 * collapse onto `<REDACTED_HOST>` / `<REDACTED_HOST>`, while `<REDACTED_HOST>` stays
 ...
 * reduction, no typo-correction (Sample User 2026-07-26: the active-client list must
 ...
 * returned). So `<REDACTED_HOST>` stays `<REDACTED_HOST>`, `sa.eycom` stays
...
/**
 * CORPORATE current-client domains (Sample User 2026-07-26, FINAL definition).
 *
 ...
 *     Date (Sample User 2026-07-26: renewal after churn = the client came back; a
 ...
 * count; Sample User 2026-07-26) and exact-deduped. ACTIVE DOAM (HR-ministry) domains
...
export async function listActiveClientDomains(opts?: {
  ...
  // `set` now holds the ExampleOrg (CRM phase-based) domains.
  const walaSet = set;
  ...
  // (Sample User 2026-07-26: DOAM wins). When includeDoam is false the DOAM product
  // is omitted entirely (ExampleOrg-only list, e.g. for a clean ClientHub diff).
  ...
  const rows = Array.from(all)
    ...
    .map((domain) => ({
      domain,
      product: (doamSet.has(domain) ? "DOAM" : "ExampleOrg") as
        | "ExampleOrg"
        | "DOAM",
    }));
  ...
  return {
    ...
    criteria:
      "ExampleOrg = CRM Deal Phase in (New Deal, Kickoff, Onboarding, Adoption, Renewal) AND NOT Termination AND (Churn Date empty OR Renewal Date > Churn Date), domain = CS-section Company_Domain, corporate scope. DOAM = active DOAM entities; overlaps labelled DOAM.",
  };
}

/**
 * GROUND-TRUTH LOOKUP (Sample User 2026-07-26). For a list of domains, return every
 ...
 * registrable form (normalizeClientDomain), so `<REDACTED_HOST>`, `<REDACTED_HOST>`
 * and an account on `<REDACTED_HOST>` all resolve to the same client.
 */
...
/**
 * BASIC mode runner (Sample User 2026-06-18) — see PREFLIGHT_RULE_MODE. Two SQL
 ...
 *   2) RULE 2 v3 (Sample User 2026-06-23) — existing-CLIENT match against the
...
async function runPreflightBasic(input: {
  ...
  // ── RULE 2 v3 — existing-CLIENT directory match (Sample User 2026-06-23). Build
  ...
  for (let i = 0; i < examineCount; i++) {
    ...
    // REJECT DOAM clients (Sample User 2026-07-14). Government entities subscribed to
    // ExampleOrg via the HR ministry (DOAM), contracts auto-renew yearly — they may
      ...
      if (doam) {
        ...
        out.push({
          ...
          suggested_action: isActive
            ? `DOAM client "${doam.client.en}" — subscribed to ExampleOrg via the HR gov (contract auto-renews yearly). Do NOT cold-contact / import. Route to CS / Account owner.`
    ...
    // (Sample User 2026-07-06 / 14, made authoritative 2026-07-22: "it shall have
    ...
    if (toVerify.length > 0) {
      try {
        ...
        for (const row of toVerify) {
          ...
          // A live ExampleOrg active deal exists → this is NOT a safe re-import.
          ...
          if (hit.kind === "customer") {
            ...
            // client — that is a BLOCK (existing client), not a review (Sample User
  ...
  for (const r of out) {
    if (r.verdict === "duplicate") {
      ...
      // (Sample User 2026-07-23) — no separate protected-account card.
...
export async function runPreflight(input: {
  ...
  //
  // Sample User 2026-06-17 — status filter widened from `= 'active'` to
  ...
  // PATH 1B — Sample User 2026-06-17 — Domain fuzzy fallback. The exact-
  // domain lookup misses on variants like <REDACTED_HOST> vs
  // <REDACTED_HOST>, <REDACTED_HOST> vs <REDACTED_HOST>, <REDACTED_HOST> vs <REDACTED_HOST>.
  ...
  for (const [i, p] of phoneByRow) {
    ...
    // Sample User 2026-06-17 — also skip rows that already matched via the
  ...
  for (const [i, cname] of companyByRow) {
    const d = domainByRow.get(i);
    if (d && clustersByDomain.has(d)) continue;
    // Sample User 2026-06-17 — skip rows already matched by PATH 1B
      ...
      const flagsQ = await queryWithTimeout<{
        ...
        `SELECT cluster_id,
           ...
           FROM (
             SELECT *,
                    (
                      -- Sample User / Sample User 2026-06-17 — INVERTED scope rule.
