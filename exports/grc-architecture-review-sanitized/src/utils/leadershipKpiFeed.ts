...
/**
 * Leadership-platform status taxonomy (7 states, matching the ExampleOrg
...
/**
 * Period basis per KPI code. ALL GRQ KPIs are reported QUARTERLY (per Sample User,
...
async function calcAuditExecutionRate() {
  // Audit Execution Rate — QUARTERLY (per Sample User, 2026-06-17): of the audits
...
const NORTH_STAR_DEFS: Array<{
...
}> = [
  {
    ...
    detail: {
      description:
        "Single weighted score combining Sample User's quality KPIs for the current quarter (the headline of the Quality scorecard).",
      methodology:
 ...
 */
/**
 * AUTO GRQ North-Star KPIs: mirror the /kpis dashboard exactly — send the same
 * stored KPI-engine value (kpi_values) the dashboard shows — instead of the feed's
 * own independent recompute, so leadership never drifts from what the managers see
 * (Sample User 2026-08-16).
 *
