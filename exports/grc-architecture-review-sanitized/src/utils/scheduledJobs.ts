...
// DELETION-FEED SWEEP — sync-INDEPENDENT schedule (Sample User 2026-07-23).
...
// DEAL DOCUMENT COMPLIANCE — rolling background sweep (Sample User 2026-08-25:
...
// MONTHLY MISSING-DOCUMENTS REPORT (Sample User 2026-08-25, shipped 2026-09-02).
//
...
export async function runResolutionDigestIfDue(): Promise<{ ran: boolean; ageHours: number }> {
  ...
  await pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS idx_scanner_run_log_name_time ON scanner_run_log(scanner_name, ran_at DESC);
  `);
  // Single shared once-per-slot claim (Sample User 2026-06-22): the cron AND this
 ...
 * THURSDAY morning (Sample User 2026-09-02: changed from daily to weekly-Thursday).
 *
