...
ok(pickAccountForContact("<REDACTED_HOST>", [{ id: "A1", domain: "<REDACTED_HOST>", name: "Acme" }])?.id === "A1", "contact->account by domain");
ok(pickAccountForContact("<REDACTED_HOST>", []) === null, "no account candidate -> null");
ok(pickContactForDeal("<REDACTED_HOST>", [{ id: "C1", domain: "<REDACTED_HOST>", name: "Sara" }])?.id === "C1", "deal->contact single under account");
