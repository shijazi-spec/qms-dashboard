...
function loadConfig(): Config {
  ...
  cachedConfig = {
    ...
    // where Kickoff is deliberately EXCLUDED (Sample User 2026-09-03: exempt from the
    ...
    // 365-day window, not the 180-day private one (Sample User 2026-09-01: Qassim
    // University came back PASS at 189 days because .<REDACTED_HOST> was treated as
    // private). Matched EXACTLY or as a sub-domain (<REDACTED_HOST> → <REDACTED_HOST>),
    // never by bare endsWith — "<REDACTED_HOST>" as a suffix would also swallow
    // "<REDACTED_HOST>". A curated list on purpose: .<REDACTED_HOST> as a whole is a MIX,
    ...
    govEduDomains: list(process.env.DUPLICATE_RADAR_GOV_EDU_DOMAINS, [
      ...
      "<REDACTED_HOST>", // Qassim University
      "<REDACTED_HOST>", // University of Ha'il
 ...
 *   2. Domain TLD — ends with any configured pattern (.<REDACTED_HOST>, .gov, etc.)
 *   3. Default: private
 *
...
export function detectSector(input: {
  ...
  if (d) {
    ...
    // a bare endsWith — that would let "<REDACTED_HOST>" swallow "<REDACTED_HOST>".
    if (
...
//
// Sample User (2026-06-11) rewrote the BLOCK rule:
//
...
export function classifyClusterOverlap(
  ...
  for (const d of deals) {
    if (stageInSet(d.stage, cfg.handoffStages)) {
      ...
      // NOT manufacture an open-sales-vs-handoff overlap on its own (Sample User
