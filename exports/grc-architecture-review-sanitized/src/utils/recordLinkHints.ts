...
export async function inferAccountForContact(
  ...
  // domain column is literally "<REDACTED_HOST>" can't be linked to a bogus account.
  const domain =
...
export async function scanStaleDeals(opts: { limit?: number } = {}): Promise<StaleDealHint[]> {
  ...
  // Does the SAME company already have a signed/paid deal? (Sample User 2026-07-14:
...
export async function applyStaleDealDisposition(
  ...
  // Record the disposition (Sample User 2026-07-16) so the operator can review how many
 ...
 * §4 can show "how many closed / resolved" at a glance (Sample User 2026-07-16). */
