 ...
 * lives in CRMProvider's Tasks module. Nothing synced Tasks, so all three could only
 * ever render "--".
 *
 * The per-record reader (CRMProviderActivitiesReader) cannot power a KPI: it costs one
 * CRMProvider request per parent record, which is exactly why the Sales cycle times
 ...
 * -----------------------------------------------
 * CRMProvider honours `criteria` only on /search, which cannot sort and rejects
 ...
 * works, and it is what CRMProviderCRM.ts documents. Same lesson the Calls import
 * learned the hard way.
 *
 * LINKAGE
 * -------
 * CRMProvider links an activity through two lookups, and which one holds the parent
...
import { fetchAllCRMProviderRecords, getCRMProviderConnectionStatus } from "./CRMProviderCRM";
...
/** CRMProvider's only "this task is finished" status. Everything else is open. */
export const TASK_CLOSED_STATUSES = new Set(["Completed"]);

let tablesReady = false;

export async function ensureCRMProviderTaskTables(): Promise<void> {
  ...
  await pool.query(`
    CREATE TABLE IF NOT EXISTS CRMProvider_tasks (
      id SERIAL PRIMARY KEY,
      CRMProvider_task_id VARCHAR(100) NOT NULL UNIQUE,
      subject TEXT,
      status VARCHAR(100),
      priority VARCHAR(50),
      due_date DATE,
  ...
  for (const sql of [
    `CREATE INDEX IF NOT EXISTS idx_CRMProvider_tasks_who ON CRMProvider_tasks(who_id) WHERE who_id IS NOT NULL`,
    `CREATE INDEX IF NOT EXISTS idx_CRMProvider_tasks_what ON CRMProvider_tasks(what_id) WHERE what_id IS NOT NULL`,
    `CREATE INDEX IF NOT EXISTS idx_CRMProvider_tasks_status ON CRMProvider_tasks(status)`,
    `CREATE INDEX IF NOT EXISTS idx_CRMProvider_tasks_due ON CRMProvider_tasks(due_date)`,
  ]) {
...
export interface CRMProviderTasksSyncResult {
  scanned: number;
  imported_new: number;
...
/** CRMProvider lookups arrive as {id,name} objects, or occasionally a bare string. */
...
export async function runCRMProviderTasksSync(
  options: { sinceIso?: string; maxRecords?: number } = {},
): Promise<CRMProviderTasksSyncResult> {
  const t0 = Date.now();
  ...
  const result: CRMProviderTasksSyncResult = {
    scanned: 0,
    imported_new: 0,
    updated_existing: 0,
    <REDACTED_TOKEN>: 0,
  ...
  if (!conn.configured) {
    ...
    result.error_samples.push(
      "CRMProvider is not configured — set the CRMProvider OAuth credentials (or CRMProvider_ACCESS_TOKEN) before syncing tasks.",
    );
  ...
  await ensureCRMProviderTaskTables();

  let tasks: any[] = [];
  try {
    tasks = await fetchAllCRMProviderRecords("Tasks", {
      fields: [
    ...
    result.errors++;
    result.error_samples.push(`CRMProvider fetch failed: ${err?.message || String(err)}`);
    result.duration_ms = Date.now() - t0;
    return result;
  }
  ...
  for (const rec of tasks) {
    ...
    const CRMProviderId = String((rec as any).id || d.id || "").trim();
    ...
    try {
      const r = await pool.query(
        `INSERT INTO CRMProvider_tasks (
           CRMProvider_task_id, subject, status, priority, due_date, closed_time,
      ...
      if (result.error_samples.length < 5) {
        result.error_samples.push(`task ${CRMProviderId}: ${e?.message || String(e)}`);
      }
    }
  }

  result.duration_ms = Date.now() - t0;
  logger.info("[CRMProviderTasksSync] complete", {
...
export async function getCRMProviderTaskStats(): Promise<{
  total: number;
