<#
.SYNOPSIS
    ExampleOrg Documentation Live Tracker - file-server collector.
...
function Get-DocxText {
    param([string]$Path)
    try {
        if ([<REDACTED_HOST>.Path]::GetExtension($Path).ToLowerInvariant() -ne '.docx') { return '' }
        Add-Type -AssemblyName <REDACTED_HOST>.Compression.FileSystem -ErrorAction SilentlyContinue
        $zip = [<REDACTED_HOST>.Compression.ZipFile]::OpenRead($Path)
        try {
            ...
            $reader = New-Object <REDACTED_HOST>.StreamReader($entry.Open())
...
function Invoke-LibraryScan {
    ...
    foreach ($folder in $matched) {
        ...
        foreach ($f in $files) {
            ...
            $title = [<REDACTED_HOST>.Path]::GetFileNameWithoutExtension($f.Name)
            if ($null -ne $code) {
...
function Save-QueuedSnapshot {
    ...
    try {
        ...
        [<REDACTED_HOST>.File]::WriteAllText($path, $Json, [System.Text.Encoding]::UTF8)
...
function Invoke-QueuedRetry {
    ...
    try {
        $json = [<REDACTED_HOST>.File]::ReadAllText($path, [System.Text.Encoding]::UTF8)
        $bytes = [System.Text.Encoding]::UTF8.GetBytes($json)
        $headers = @{ 'X-Tracker-Key' = $Config.Key; 'Content-Type' = 'application/json; charset=utf-8' }
...
function Start-LiveWatch {
    ...
    $watcher = New-Object <REDACTED_HOST>.FileSystemWatcher
    $watcher.Path = $Config.LibraryRoot
    $watcher.IncludeSubdirectories = $true
    $watcher.NotifyFilter = [<REDACTED_HOST>.NotifyFilters]::LastWrite -bor `
                            [<REDACTED_HOST>.NotifyFilters]::FileName -bor `
                            [<REDACTED_HOST>.NotifyFilters]::Size
...
$config = Get-Config
Write-Log "ExampleOrg Documentation Collector v$($Script:CollectorVersion) (hash spec $($Script:HashSpecVersion))"
