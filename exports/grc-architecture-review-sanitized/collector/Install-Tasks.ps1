<#
.SYNOPSIS
    Register the ExampleOrg documentation collector with Task Scheduler.

.DESCRIPTION
    Creates two scheduled tasks:

      "ExampleOrg Doc Tracker - Daily Push"  06:00 daily, full reconcile.
      "ExampleOrg Doc Tracker - Live Watch"  at startup, resident file watcher.
...
Register-CollectorTask `
    -Name 'ExampleOrg Doc Tracker - Daily Push' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$script`" -Mode Daily" `
    -Trigger (New-ScheduledTaskTrigger -Daily -At $DailyTime)

if ($LiveWatch) {
    Register-CollectorTask `
        -Name 'ExampleOrg Doc Tracker - Live Watch' `
...
Write-Host '  Get-ScheduledTask -TaskName "ExampleOrg Doc Tracker*"'
Write-Host ''
Write-Host 'Test the daily task immediately with:'
Write-Host '  Start-ScheduledTask -TaskName "ExampleOrg Doc Tracker - Daily Push"'
