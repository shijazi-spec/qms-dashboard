 ...
 *
 *   "resolved": "<REDACTED_URL><pkg>/-/<pkg>-<ver>.tgz"
 *
...
const replitCount = offenders.filter((o) =>
  o.resolvedUrl.includes("<REDACTED_HOST>"),
).length;
...
if (replitCount > 0) {
  console.error(
    `${replitCount} of these are Replit's internal package proxy — this happens when\n` +
      ...
      `  node -e "const f='package-lock.json',fs=require('fs');fs.writeFileSync(f,fs.readFileSync(f,'utf8').split('<REDACTED_URL>').join('<REDACTED_URL>'))"\n\n` +
