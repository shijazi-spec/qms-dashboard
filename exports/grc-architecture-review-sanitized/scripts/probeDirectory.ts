 ...
 *
 *   npx tsx scripts/probeDirectory.ts "<REDACTED_HOST>" "هيئة الخبراء" "Aon"
 */
...
async function main() {
  ...
  if (!terms.length) {
    console.error('Pass terms: npx tsx scripts/probeDirectory.ts "<REDACTED_HOST>" "هيئة الخبراء" "Aon"');
    process.exit(2);
  }
