...
const UNOWNED = new Set(["", "unassigned", "ExampleOrg", "ExampleOrg"]);
...
describe("a deal nobody owns cannot be the keeper", () => {
  // Sample User 2026-08-26. Five conflict deals are owned by "ExampleOrg" — the
  ...
  it("keeps the owned On Hold over the ownerless New Deal", () => {
    ...
    expect(
      keeper([
        { stage: "New Deal", owner: "ExampleOrg" },
  ...
  it("falls back to the normal rules when NEITHER deal has an owner", () => {
    ...
    expect(
      keeper([
        { stage: "Unaccounted", owner: "ExampleOrg" },
        { stage: "Contacted", owner: "Unassigned" },
      ]),
    ).toBe(1);
  });
  ...
  it("does not let a big amount rescue an ownerless deal", () => {
    ...
    expect(
      keeper([
        { stage: "Unaccounted", owner: "ExampleOrg", amount: 89800 },
