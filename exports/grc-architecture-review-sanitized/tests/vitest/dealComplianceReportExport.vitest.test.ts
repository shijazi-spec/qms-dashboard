 ...
 *
 * Sample User's brief (2026-08-25): the missing-document PERCENTAGE, WHICH OWNER the
...
describe("workbook shape", () => {
  ...
  const sheets = buildDealComplianceReportSheets(rows, { segment: "ExampleOrg", inScope: 10 });
...
describe("filename", () => {
  it("names the segment", () => {
    expect(dealComplianceReportFilename("ExampleOrg")).toBe(
      "deal-document-compliance-ExampleOrg.xlsx",
    );
  });
