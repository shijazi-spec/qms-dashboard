...
describe("detectSector", () => {
  ...
  test("empty gov_type, .<REDACTED_HOST> domain → government", () => {
    expect(detectSector({ gov_type: null, domain: "<REDACTED_HOST>" })).toBe(
      "government",
    );
  });
  test("empty gov_type, .<REDACTED_HOST> domain → private", () => {
    expect(detectSector({ gov_type: null, domain: "<REDACTED_HOST>" })).toBe(
      "private",
    );
  });
...
describe("extractCsFieldsFromRawData — normalised key fallback", () => {
  test("matches Company_Domain via exact key", () => {
    ...
    expect(out.company_domain).toBe("<REDACTED_HOST>");
  });
...
describe("real-world fixture (docs/Mawsool Deals - Final Report.xlsx)", () => {
  ...
  test("government-domain churn uses 12-month cool-off", () => {
    if (!rows) return;
    // Find a .<REDACTED_HOST> termination row where the churn is between 180 and 365 days
    ...
    const candidate = rows.find((r) => {
      ...
      if (!r.domain.endsWith(".<REDACTED_HOST>")) return false;
