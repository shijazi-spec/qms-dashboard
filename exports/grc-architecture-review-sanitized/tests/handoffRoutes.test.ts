/**
 * Integration tests for src/mastra/routes/handoffRoutes.ts
 *
 * Coverage matrix:
 *   - structural      → every route exposes path/method/createHandler.
 *   - 200/4xx         → handlers return a numeric status.
 *
 * Run:  npx tsx tests/handoffRoutes.test.ts
 */

import { handoffRoutes } from "../src/mastra/routes/handoffRoutes";
import { TestSuite } from "./_helpers/runner";
import { buildHandler, makeContext, type FakeContext } from "./_helpers/fakeContext";

const suite = new TestSuite("handoffRoutes");

console.log("\n=== handoffRoutes integration tests ===\n");

await suite.test("every route exposes path, method and createHandler", async () => {
  for (const r of handoffRoutes) {
  ...
  suite.expect(handoffRoutes.length >= 1, "at least 1 route registered");
});

const apiRoutes = handoffRoutes.filter((r) => r.path.startsWith("/api/"));

for (const route of apiRoutes) {
  const path = route.path;
  const method = route.method as string;
  await suite.test(`${method} ${path} — handler returns numeric status`, async () => {
    const handler = await buildHandler(handoffRoutes, path, method, { mastra: null });
    const ctx = makeContext({
      method,
      params: { id: "1", handoffId: "1" },
