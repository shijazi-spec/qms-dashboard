...
function testParseClientIp(): boolean {
  ...
  if (HOPS === 0) {
    ...
    assert(
      ...
      parseClientIp('::ffff:<REDACTED_IP>', undefined),
      '<REDACTED_IP>',
    );
  } else if (HOPS === 1) {
    assert(
      'HOPS=1: second-from-right XFF entry (real client before intermediate proxy)',
      parseClientIp('<REDACTED_IP>, <REDACTED_IP>', undefined),
      '<REDACTED_IP>',
    );
    assert(
      'HOPS=1: X-Real-IP IS trusted when TRUST_PROXY_HOPS > 0',
      parseClientIp(undefined, '::ffff:<REDACTED_IP>'),
      '<REDACTED_IP>',
    );
    assert(
