  ...
  await suite.test(
    ...
    async () => {
      ...
      try {
        ...
        suite.expect(
          names.includes("ExampleOrg QMS Consultant"),
          "ExampleOrg QMS Consultant still in /active",
        );
      } finally {
        if (original === undefined) delete process.env.ADMIN_API_KEY;
        else process.env.ADMIN_API_KEY = original;
