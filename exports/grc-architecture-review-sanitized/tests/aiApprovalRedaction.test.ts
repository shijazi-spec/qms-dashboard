...
async function run(): Promise<void> {
  ...
  const recorded = await recordExecutionResult(enqueued.action_code, {
    ...
    data: {
      ...
      curl_example: "curl -H 'Authorization: Bearer <REDACTED_TOKEN>' <REDACTED_URL>",
    },
  });
  ...
  assert(
    !executionResultJson.includes("<REDACTED_TOKEN>"),
  ...
  const INLINE_GHP = "<REDACTED_TOKEN>";
  const INLINE_SK  = "<REDACTED_TOKEN>";
