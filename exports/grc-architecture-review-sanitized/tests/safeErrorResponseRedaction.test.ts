...
(async function main() {
      ...
      makeJsonResponse(
        {
          ...
          details: 'Invalid token <REDACTED_TOKEN> supplied',
        },
        500,
      ),
    );
  ...
  // 2. ghp_… in 4xx error response is redacted ---------------------------------
  {
    ...
    assert(
      typeof body.details === 'string' &&
        !body.details.includes('<REDACTED_TOKEN>'),
  ...
  {
    const html = '<html><body>500 error: token <REDACTED_TOKEN></body></html>';
    const ctx = makeFakeContext(
      new Response(html, {
        status: 500,
