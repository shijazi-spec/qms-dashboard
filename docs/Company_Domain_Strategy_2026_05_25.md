...
**Audience:** WalaPlus product + GRQ leadership
**Date:** 2026-05-25
...
For a B2B SaaS like WalaPlus, **a company's email domain is its canonical identity** — more stable and useful than name, phone, or CRM record ID.
...
Mature platforms (Gong, Chorus, Salesforce ABM, ZoomInfo) all use domain as the primary key for company-level aggregation. WalaPlus should too.
...
{contact.email} ──────────┴────────────┘                      "all touches to aramco.com
...
- Shows `Domain: aramco.com` chip in the Call Details modal right column, below Phone
...
Concretely: a customer calls from a non-Zoho-registered cell phone but identifies as `khalid@stc.com.sa` during the verification step. Phone match: fails (cell isn't on file). Email domain match: hits Account = STC. Call links automatically.
...
- New view: *"this agent fails Objection Handling on 4 calls — 3 of them are to banking domains (`@alrajhi.com`, `@snb.com.sa`, `@bsf.com.sa`)"*
...
- Industries inferred from domain via a lookup table — `@bank.com`/`@al-rajhi.com` → Banking, `@stc.com.sa` → Telecom, etc.
...
- **Domain blacklist/competitive intelligence** — flag calls to competitor domains (`@competitor.com`). Niche use-case.
...
| **P2** — Coaching + clusters by domain | 1 day | Large — enables industry-level analysis | Differentiates WalaPlus from generic CI tools | Build when call volume justifies (≥ 500/month per agent) |
...
4. **Domain canonicalisation** — `aramco.com` vs `aramco.com.sa` vs `aramco.net` — same company, different domains. Need a "primary domain" concept maintained somewhere. Cheapest: a small column on the Account record in Zoho marked `is_primary_domain`. More complete: a separate `company_domains` table mapping all variants to a canonical entity.
