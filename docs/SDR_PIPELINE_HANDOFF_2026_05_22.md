...
**Primary stakeholder:** Ahmad Amashah (GRQ Quality & Operations)
**Account owner:** Sarah Hijazi (OpenAI billing + GitHub org)
**Deployed at:** `qms-dashboard.replit.app`

---

## 1. Executive Summary

**What it became.** A working end-to-end SDR call evaluation pipeline: drop a `.wav` → transcribe (Arabic, Whisper) → analyze (sentiment + QA via gpt-4o-mini) → **auto-score against the 18-attribute WalaPlus Sales Quality Scorecard v1.0** → display in SDR Evaluation tab → roll up per-agent in Analytics → manager Approves or Disagrees → exportable as multi-sheet Excel.

**What it was at session start.** A pipeline that silently failed at every stage. Every error was generic ("Failed to analyze call"), every diagnostic was wrong (Replit modelfarm proxy / dummy key), and no scorecard had ever been applied to any call.
...
| #22 | `fix/dashboard-ghost-audits-and-history-layout` | `6dc97cf` | Hide ghost audits + widen audit history table (v1) |
...
| (Deps fix) | direct to main | `187887d` → `a131ee8` | `@ai-sdk/openai 1.3.24 → 3.0.65` + remove unused `openai` pkg |
...
**Plus:** 1 secret cleanup (deleted `AI_INTEGRATIONS_OPENAI_*` env vars in Replit), 1 OpenAI credit top-up (Sarah added $10 prepaid).
...
3. **OpenAI `insufficient_quota`** — pre-paid credit had run out 2026-05-18 (per "Last used" on platform.openai.com). Sarah Hijazi added $10. Auto-recharge **still off** — recommended to enable.
4. **AI SDK version mismatch** — `ai@5.0.190` requires v2 model spec but `@ai-sdk/openai@1.3.24` only provided v1. Every `gpt-4o` call died inside the JS layer with `"Unsupported model version v1 for provider openai.chat"` before ever reaching OpenAI. Fix: upgrade `@ai-sdk/openai 1.3.24 → 3.0.65`.
5. **Replit modelfarm proxy hijacking traffic** — `AI_INTEGRATIONS_OPENAI_BASE_URL=http://localhost:1106/modelfarm/openai` + `AI_INTEGRATIONS_OPENAI_API_KEY=_DUMMY_API_KEY_` routed every "OpenAI" call through Replit's internal proxy with a literal dummy key. The proxy was the actual source of the v1/v2 error. Fix: delete both env vars in Replit Secrets so code falls back to real `OPENAI_API_KEY` + `api.openai.com/v1`.
...
| File | What changed |
|---|---|
| `dashboard/calls.html` | Scorecards tab removed; "Active Scorecard" collapsible panel inside SDR Evaluation; CRM link → Zoho phone search; Analyze All Pending button; score badges in eval list; Manager Review panel (Approve/Disagree + history); Excel export button; loadScorecards on tab open |
...
| `openai` | `^6.22.0` | **removed** (unused) |

### Replit Secrets
| Secret | Action |
|---|---|
| `AI_INTEGRATIONS_OPENAI_BASE_URL` | **DELETED** (was pointing at modelfarm proxy) |
| `AI_INTEGRATIONS_OPENAI_API_KEY` | **DELETED** (was a dummy key) |
| `OPENAI_API_KEY` | Untouched — real `sk-proj-…` key, $10 credit loaded |
...
- Transcription (Whisper via real OpenAI, gpt-4o-mini-transcribe, Arabic)
...
- **Medium #9** — Switch nightly cron-fired analysis to OpenAI Batch API (50% cost discount, 24h SLA)
...
- **Deferred** — Phase 6 CS Lifecycle stage-history audit (needs Zoho Stage History API access)
...
```bash
# Pipeline health (in Replit shell)
...
cd ~/workspace && npm ls @ai-sdk/openai && npm ls openai
```
...
- [ ] **Enable OpenAI auto-recharge** at https://platform.openai.com/billing — prevents silent quota outages (single biggest risk)
...
- [ ] **`git push origin main`** from Replit shell if Replit Agent has unpushed local commits

### Cost profile
- Per call (transcribe + analyze + scorecard): ~$0.0025 with gpt-4o-mini
- $10 credit covers ~4,000 calls
- For nightly batch when #9 ships: drops to ~$0.00125/call via OpenAI Batch API
