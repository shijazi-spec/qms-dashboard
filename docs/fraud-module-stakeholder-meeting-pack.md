...
| 4 | §15 Q2 — SMS for P1 | "Open question — SMS in v1?" | **NO** — `NotificationChannel = "in_app" \| "email" \| "slack"`. SMS is out of scope per §3.2. | No SMS provider exists in the platform; adding Twilio/Unifonic is a 3-5 day Phase 5 work item. Closes Q2. |
...
| 2 | SMS notifications for P1 in v1? | **NO** — defer to Phase 5. | No SMS infra. Email already covers the P1 channel. CEO/Head of GRQ can configure email-to-SMS via their carrier as interim. | New Twilio/Unifonic integration + secrets + billing + rate limits. ~3-5 days. |
