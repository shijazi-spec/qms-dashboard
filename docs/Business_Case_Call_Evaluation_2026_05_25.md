...
A complete in-house Call Evaluation pipeline structured in five layers, each built on top of what already exists in `qms-dashboard.replit.app/calls`:
...
Khalid (or an ops admin) uploads the recording to `/calls` → Data Sources. The platform's strict filename parser extracts every field automatically:
...
3. **(Phase 1, Week 4–5) Speaker diarization** — identifies Khalid's voice vs customer's voice
...
| COPC v2 Attribute | Khalid's score | Why |
|---|---|---|
| Greeting & introduction | PASS (10/10) | Said the WalaPlus opening + introduced himself |
...
| **Objection handling** | **FAIL** | Customer said "too expensive" → Khalid offered discount instead of probing |
...
Ahmed opens `qms-dashboard.replit.app/calls` directly. Lands on the **Weekly Report**.
...
Khalid    | 10 calls | 62% ↓ | 78% comp. | 0 critical | 1 plan pending
...
Ahmad     | 11 calls | 73%   | 88% comp.  | 0 critical | 1 plan pending
  ↳ [Deliver Coaching Plan]

Mona      | 8 calls  | 78%   | 87% comp.  | 1 critical (consent line missed) | 1 plan pending
  ↳ [Deliver Coaching Plan]   [Review Critical Fail]

Layla     | 12 calls | 91% ↑ | 100% comp. | 0 critical | 0 plans
  ↳ Top performer this week

═══ Coaching actions this week ═══

• Khalid — Objection Handling (4 failed-call evidence) → [Deliver]
• Ahmad — Closing plan (3 failed-call evidence) — awaiting delivery for 5 days → [Nudge]
...
Ahmed clicks **Deliver →** next to Khalid's plan. A modal opens:
...
Ahmed clicks **Mark Delivered**. The plan moves to `awaiting_verification`.
...
The next time Ahmed opens the Weekly Report, Khalid's row shows:
- Active plans: **0**
...
| Step | Owner | When |
|---|---|---|
| CTO approval to proceed with the 5-week plan | Ahmed Eldaly | This week |
