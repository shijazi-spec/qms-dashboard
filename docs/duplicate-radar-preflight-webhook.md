...
| Field | Type | Notes |
|---|---|---|
| `domain` | string | Authoritative signal. Pass `acme.com` (without protocol). |
...
preflight_url = "https://<your-replit-deployment>.replit.app/api/duplicates/preflight/check";
...
```bash
curl -X POST https://<your-deployment>.replit.app/api/duplicates/preflight/check \
...
async function shouldCreateLead(input: {
  ...
  const res = await fetch(
    "https://<your-deployment>.replit.app/api/duplicates/preflight/check",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
