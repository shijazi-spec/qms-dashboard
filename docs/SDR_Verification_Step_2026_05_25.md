...
- **Verified:** ends with the company's domain (e.g. `name@walaplus.com`), OR a domain that obviously belongs to a real business (e.g. `aramco.com`, `stc.com.sa`, `pif.gov.sa`)
...
If the prospect's email domain doesn't match the company name they gave (e.g. *"شركة الصحراء"* but `@example.com`), the SDR clarifies — there's often a parent company, holding entity, or rebranding. Note both in the CRM.
