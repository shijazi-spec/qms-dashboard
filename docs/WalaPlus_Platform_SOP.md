...
| 4.5.1 | Apr 18, 2026 | Engineering | **Audit drill-down fairness fix.** `directAuditRunner.ts` previously capped detailed (per-record) audit issues at a single global limit of 500 entries, populated in module-iteration order (Leads → Deals → Contacts → Tasks → Accounts). When the first three modules saturated the cap, Tasks and Accounts received zero per-record entries in `raw_audit_data.all_issues` even though they were counted in `issues_by_category`. The drill-down modal then fell back to a synthetic `summary_Tasks_1` / `summary_Accounts_1` row instead of real records. Replaced the global cap with a **per-module cap** (`MAX_DETAILED_PER_MODULE = 200`, total ceiling 1000 for 5 modules) tracked via a `Map<string, number>` passed into `analyzeRecordBatch`. All audits run after this change have real per-record entries for every module, and the drill-down modal renders actual records for Tasks/Accounts. *(Audits saved before this fix retain the old truncated `all_issues` and will continue to show the synthetic summary row until re-run.)* |
