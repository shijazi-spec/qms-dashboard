...
**Reviewing for:** a.amashah@walaplus.com (Operations & Quality HOD)
**Date:** 2026-05-25
...
More critically: the expand-row pattern works for "I want to drill in once" but breaks for "I want to compare Layla and Khalid side-by-side." If Sarah is doing 1:1 prep, she needs both agents loaded at once. Inline expansion fails this workflow because expanding row 2 collapses row 1 (typical UX pattern), or expanding both means a 4-screen scroll.
...
The Khalid case study is the spine of the document. But:

| Fiction | Why it matters |
|---|---|
| **Cohen's κ = 0.51 on Objection Handling "moderate, tune rubric"** | κ requires two independent coders. Sarah is the only reviewer. The κ shown is between AI and Sarah, which is a "model agreement with sole human grader" metric — useful, but not Cohen's κ in the inter-rater sense Velents proposed and not what the rubric tuning literature uses. |
...
|---|---|
| "≥70% reduction in manual QA effort" | **Vibes.** Self-reported, "pre-launch baseline." The baseline is "Sarah's vague recollection of how long she spent listening last quarter." |
...
Sarah may be reviewing from a desktop now, but the Coaching board, Critical Issues banner, and Inline drill-down patterns will be screenshot-shared in WhatsApp executive groups. A manager who can't pinch-zoom into a screenshot and read it loses the proactive sharing pattern.
...
Before computing AI-vs-Sarah κ, you need to establish:
- Does Sarah score Khalid's call the same way today as 3 weeks ago? (intra-rater reliability)
- Would Sarah and a second reviewer agree on whether Khalid passed Objection Handling? (inter-rater reliability)

If Sarah's own scoring drifts, the κ between AI and Sarah is measuring Sarah's noise, not the rubric's quality.
...
### 2.9 Coaching delivery accountability — what if Sarah never delivers a pending plan?
...
PDPL Article 22 requires controllers to "demonstrate compliance." For a single random call, can WalaPlus produce:
...
**Required:** measure Sarah's actual time-on-QA *next week* (timesheet for 5 days). Without that, the metric is unmeasurable.

### 3.4 Three managers blurred together

Sarah Hijazi (Quality Manager), Ahmed Amashah (HOD sponsor), Ahmed Eldaly (CTO approver) — somewhat interchangeably called "the manager" across the docs.

**Required:** explicit RACI:
- Who opens the dashboard daily? Sarah.
- Who delivers coaching plans? Sarah.
- Who approves deferred PII redaction risk? Ahmed Amashah.
...
When CTO Ahmed Eldaly reads three same-day decision records each cutting scope, the question he'll ask is "what's *next* to be cut?"
...
**WalaPlus is heavy on Mode 1 and light on Mode 2.** A manager scoring 50 calls with AI-rubric output still needs to *read* 50 rubrics to triage which are worth deep review. The flagged-call inbox (Mode 2) is the actual time-saver.
...
WalaPlus's auto-trigger (3 fails in 14 days) is unusual. Real risks:
...
Why: the inline-drill leaderboard *alone* delivers 60% of the case-study value with 30% of the engineering risk. Shipping 1a in 4 weeks gives Sarah measurable benefit while diarization spike runs in parallel.
...
- Sarah keeps a 5-day timesheet of QA hours (baseline for "70% reduction")
...
Every value-claim in the Business Case rests on Sarah doing this. The Decision Record amendment explicitly accepts "if manager doesn't check dashboard weekly" as a risk. But there is no week-1 test for this.

**2-week behavioral pilot (free, before any new code):**
- Sarah commits to opening `/calls` daily for 10 working days
- Telemetry captures: time-in-app, tabs visited, calls reviewed, coaching plans actioned
- At end of pilot, ask Sarah: "If the Overview tab had X, Y, Z, would you have stayed longer / done more?"

If Sarah doesn't sustain daily access in week 2, *no amount of dashboard polish in week 7 will change that*.
...
After 3 weeks, Sarah has a working cockpit. Phase 1b adds diarization + κ + IA cleanup as a second tranche.
...
End of Week 1: Sarah opens the dashboard, sees the leaderboard, sorts it, clicks a row → today's modal opens. The baseline of "manager dashboard opens" metric starts populating.
...
| # | Action | Time | Why |
|---|---|---|---|
| 1 | Run a 5-day timesheet of Sarah's actual QA hours | 5 days, ~5 min/day | The "70% reduction" metric needs a real baseline. |
| 2 | Schedule a 90-min inter-rater calibration session | 90 min one session | Establishes whether rubric ambiguity is the actual problem before measuring AI-vs-Sarah κ. |
...
### 6.2 Questions to ask Sarah before starting Week 2
...
### 6.3 Questions to ask CTO Ahmed Eldaly before requesting approval
...
3. **What's the team's actual definition of "1 FTE"?** Is Ahmed Amashah on this 100%, 50%, or evenings/weekends?
...
Then ask:
  Can I sort this output to identify the "Khalid equivalent" — the SDR
...
Do five concrete things this week before any new feature code. Ask Sarah seven specific questions to anchor team-size, cadence, and prioritization. Ask the CTO five questions to align on PDPL posture, budget, FTE definition, and Phase 1b commitment. Run one acid-test SQL query on real data — that single query is more informative than another week of planning.
