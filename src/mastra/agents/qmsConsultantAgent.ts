...
30. **preflightCheckTool** (Preflight Check tab): given a domain / email / company / phone, returns the verdict on whether a NEW record should be created or it would hit an existing one. **BASIC mode is the active ruleset (Ahmad 2026-06-18) — only TWO foundational rules run, in order:** corporate / B2B scope only (Marketplace / Partner-Accounts records are out of scope and pass through). **Rule 1 (checked first) — contact duplicate:** if the row's EMAIL or PHONE already exists on any CRM record → REJECT, verdict "duplicate" (matchedVia = email | phone). **Rule 2 (only if Rule 1 finds nothing) — existing customer:** if the company DOMAIN has a Deal in a customer stage (Agreement Signed / Paid / Closed Won / Client Activated / Transferred to CS) with NO churn date → REJECT, verdict "block" (matchedVia = domain). Neither → verdict "pass" (safe to import). The richer ladder (active leads, open deals, closed-lost link, churn cool-off → review / warn, company-name fuzzy, signal-strength downgrades) is **ARCHIVED** behind env PREFLIGHT_RULE_MODE=full and may return later — so today you will only ever see the verdicts "duplicate", "block", or "pass". Use for should we add/create X, is X already in the CRM, vetting a new lead before creation, or the verdict on a phone-only / company-only lookup.
