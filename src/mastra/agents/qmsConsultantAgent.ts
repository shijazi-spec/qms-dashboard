...
- **Per-module duplicate definition:** **Accounts** = the same company (same domain / legal name / close fuzzy-name). **Account PRODUCT (Sarah 2026-08-11):** corporate accounts all share the "Corporate Accounts" layout, so WalaPlus vs WalaOne is NOT in the layout — it lives in the account's Zoho multi-select **Products** field (e.g. WalaOne), with a populated "WalaPlus Products" sub-field as a secondary WalaPlus signal. **The WalaPlus FAMILY (Sarah 2026-08-12): WalaPlus, Wala Offer / WalaOffer and WalaBravo all count as WalaPlus; WalaOne is the only WalaOne signal.** The token lists are env-configurable (no code change to add a new sub-product): RADAR_WALAPLUS_PRODUCT_TOKENS / RADAR_WALAONE_PRODUCT_TOKENS, served to the dashboard via GET /api/duplicates/config. The Account Duplicates tab shows each account's product chip (WalaPlus / WalaOne / both) read live from raw_data, and flags a cluster with a **⚡ cross-product** chip when its accounts span more than one product. Sarah's rule: two accounts for the SAME company but on DIFFERENT products (one WalaPlus, one WalaOne) ARE accepted as a duplicate and must surface here — cross-product is not a reason to treat them as separate. (The top segment chip still filters by LAYOUT, not this product field.) **Contacts** = the same person — only a duplicate when **≥2 of {email, phone, full name}** match (sharing an Account is NOT duplicate evidence). **Leads** = duplicate leads are ACCEPTED; a lead is only flagged bad when the SAME lead (same phone + email) is **submitted again** (a re-submission). **Deals** = the problem is **2 ACTIVE deals** for the same thing (won/closed deals are not duplicates to fix).
