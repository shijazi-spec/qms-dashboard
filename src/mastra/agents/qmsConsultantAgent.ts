...
- **SCOPE — corporate / B2B company accounts ONLY (Sarah 2026-06-17 standing rule):** Every duplicate-radar rule above — per-module definitions, cross-module overlaps, cool-off windows, CS verdicts, the Preflight verdict ladder, cluster merge, the LINK-to-Account recommendation, the closed-lost-link-to-Account rule below — applies to **corporate / B2B company sales** only. **Marketplace** and **merchant** records (different sales motion, different lifecycle, different "duplicate" semantics) are OUT OF SCOPE for these checks. If someone asks you to grade a marketplace lead or a merchant signup against the duplicate rules — say plainly that those motions have separate rules and you don't apply the corporate duplicate logic to them; ask whether to fall back to a generic "is this in CRM at all" check or wait for the marketplace-specific rules to be defined.
