          ...
          return async (c: any) => {
            try {
              ...
              const userOverrides: Record<string, { team: string; status: string }> = {
                ...
                "Khalil Aldadah": { team: "MP", status: "Active" },
                "Mohammed Ghanem": { team: "MP", status: "Active" },
              ...
              for (const lead of leads) {
                ...
                if (hasIssue) {
                  if (!lead.Email) {
                    ...
                    ownerIssueDetails.push({ recordId: lead.id, module: 'Leads', layout: lead.Layout || '', product: lead.Product || '', owner: userInfo.name, issue: 'Missing: Email', severity: 'high', fieldName: 'Email', recommendation: 'Add an email address to this lead record.' });
                  }
                  if (!lead.Lead_Source) {
                    ownerStats[ownerId].issues.medium++;
                    ownerIssueDetails.push({ recordId: lead.id, module: 'Leads', layout: lead.Layout || '', product: lead.Product || '', owner: userInfo.name, issue: 'Missing: Lead Source', severity: 'medium', fieldName: 'Lead_Source', recommendation: 'Set the lead source to track where this lead came from.' });
                  }
                  if (!lead.Lead_Status) {
                    ownerStats[ownerId].issues.low++;
                    ownerIssueDetails.push({ recordId: lead.id, module: 'Leads', layout: lead.Layout || '', product: lead.Product || '', owner: userInfo.name, issue: 'Missing: Lead Status', severity: 'low', fieldName: 'Lead_Status', recommendation: 'Set the lead status to track progress.' });
                  }
                } else {
              ...
              for (const deal of deals) {
                ...
                if (hasIssue) {
                  if (!deal.Deal_Name) {
                    ...
                    ownerIssueDetails.push({ recordId: deal.id, module: 'Deals', layout: deal.Layout || '', product: deal.Product || '', owner: userInfo.name, issue: 'Missing: Deal Name', severity: 'critical', fieldName: 'Deal_Name', recommendation: 'Add a deal name to this deal record.' });
                  }
                  if (!deal.Stage) {
                    ownerStats[ownerId].issues.critical++;
                    ownerIssueDetails.push({ recordId: deal.id, module: 'Deals', layout: deal.Layout || '', product: deal.Product || '', owner: userInfo.name, issue: 'Missing: Stage', severity: 'critical', fieldName: 'Stage', recommendation: 'Set the deal stage to track pipeline progress.' });
                  }
                  if (!deal.Amount) {
                    ownerStats[ownerId].issues.high++;
                    ownerIssueDetails.push({ recordId: deal.id, module: 'Deals', layout: deal.Layout || '', product: deal.Product || '', owner: userInfo.name, issue: 'Missing: Amount', severity: 'high', fieldName: 'Amount', recommendation: 'Add the deal amount for accurate pipeline value.' });
                  }
                } else {
                  ownerStats[ownerId].passCount++;
                }
