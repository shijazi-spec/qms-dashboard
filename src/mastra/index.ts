...
export const mastra = new Mastra({
  ...
  server: {
    ...
    apiRoutes: [
      ...
      // ======================================================================
      // Admin & Tools pages (moved out of hidden / Analytics / Support).
      //   /logs           — System Logs   (was under Analytics)
      //   /ai-approvals   — HITL queue    (was hidden)
      //   /external-audits — External/Surveillance audits (new, GRC)
      // RBAC is enforced by the back-end API routes these pages call;
      // the page shells are plain HTML served by the same pattern as /migration.
      // ======================================================================
      {
        path: "/logs",
        method: "GET",
        createHandler: async () => {
          return async (c: any) => {
            try {
              ...
              return c.text("System Logs page not found", 404);
            } catch (error) {
              console.error("Error serving Logs page:", error);
              return c.text("Error loading System Logs", 500);
            }
          };
        },
      },
        ...
        createHandler: async () => {
          return async (c: any) => {
            try {
              ...
              return c.text("AI Approvals page not found", 404);
            } catch (error) {
              console.error("Error serving AI Approvals page:", error);
              return c.text("Error loading AI Approvals", 500);
            }
          };
        },
      },
      {
        ...
        createHandler: async () => {
          return async (c: any) => {
            try {
              const possiblePaths = [
                join(process.cwd(), "dashboard", "intake.html"),
                join(process.cwd(), "..", "dashboard", "intake.html"),
                "/home/runner/workspace/dashboard/intake.html",
              ];
              for (const p of possiblePaths) {
                if (existsSync(p)) {
                  return c.html(readFileSync(p, "utf-8"));
              ...
              return c.text("Manual Intake page not found", 404);
            } catch (error) {
              console.error("Error serving Manual Intake page:", error);
              return c.text("Error loading Manual Intake", 500);
            }
          };
        },
      },
      {
        ...
        createHandler: async () => {
          return async (c: any) => {
            try {
              const possiblePaths = [
                join(process.cwd(), "dashboard", "external-audits.html"),
                join(process.cwd(), "..", "dashboard", "external-audits.html"),
                "/home/runner/workspace/dashboard/external-audits.html",
              ];
              for (const p of possiblePaths) {
                if (existsSync(p)) {
                  return c.html(readFileSync(p, "utf-8"));
              ...
              return c.text("External Audits page not found", 404);
            } catch (error) {
              console.error("Error serving External Audits page:", error);
              return c.text("Error loading External Audits", 500);
            }
          };
        },
      },
        ...
        createHandler: async () => {
          return async (c: any) => {
            try {
              ...
              return c.text("/* tailwind.css not found */", 404, { "Content-Type": "text/css" });
            } catch (error) {
