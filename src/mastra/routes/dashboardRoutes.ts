...
export function createDashboardRoutes(deps: DashboardDeps = defaultDeps) {
  ...
  app.get("/audit/latest", async (c) => {
    try {
      ...
      logger.error("Error fetching latest audit:", error);
  ...
  app.get("/governance", async (c) => {
    try {
      ...
      logger.error("Error fetching governance document:", error);
  ...
  app.get("/scorecard", async (c) => {
    try {
      ...
      logger.error("Error fetching scorecard:", error);
  ...
  app.post("/audit/trigger", async (c) => {
    try {
      const workflowBaseUrl =
        process.env.MASTRA_WORKFLOW_BASE_URL ?? "http://localhost:5000";
      ...
      if (response.ok) {
        ...
        return c.json(
          { success: false, message: "Failed to trigger audit" },
          500,
        );
      }
    } catch (error) {
