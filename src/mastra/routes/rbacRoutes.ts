    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("👤 [RBAC API] GET /api/rbac/users", {
            role,
            department,
          });
          const users = await rbacDb.getSystemUsers({
            role,
            department,
            active_only,
          });
          ...
          safeLogger.error("❌ [RBAC API] Error fetching users:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error("❌ [RBAC API] Error fetching user:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("👤 [RBAC API] POST /api/rbac/users", {
            email: body.email,
          });
          ...
          safeLogger.error("❌ [RBAC API] Error creating user:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error("❌ [RBAC API] Error updating user:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🔐 [RBAC API] GET /api/rbac/permissions/:role", {
            role,
          });
          ...
          safeLogger.error("❌ [RBAC API] Error fetching permissions:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🔐 [RBAC API] POST /api/rbac/check-permission", {
            email,
            permission,
          });
          ...
          safeLogger.error("❌ [RBAC API] Error checking permission:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("📋 [RBAC API] GET /api/rbac/bu-processes", {
            department,
          });
          const processes = await rbacDb.getBuProcesses({
            department,
            active_only,
          });
          return c.json({ processes, total: processes.length });
        } catch (error: any) {
          safeLogger.error("❌ [RBAC API] Error fetching BU processes:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const logger = mastra?.getLogger();
          const body = await c.req.json();
          logger?.info("📋 [RBAC API] POST /api/rbac/bu-processes", {
          ...
          safeLogger.error("❌ [RBAC API] Error creating BU process:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error("❌ [RBAC API] Error updating BU process:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const logger = mastra?.getLogger();
          logger?.info(
            "📊 [RBAC API] POST /api/rbac/calculate-control-readiness",
          );
          ...
          safeLogger.error(
            "❌ [RBAC API] Error calculating control readiness:",
            error,
          );
          return c.json({ error: "An internal error occurred" }, 500);
        }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error(
            "❌ [RBAC API] Error escalating overdue actions:",
            error,
          );
          return c.json({ error: "An internal error occurred" }, 500);
        }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("⚠️ [RBAC API] GET /api/rbac/escalations", {
            status,
            source_type,
          });
          const escalations = await rbacDb.getEscalationLog({
            status,
            source_type,
          });
          return c.json({ escalations, total: escalations.length });
        } catch (error: any) {
          safeLogger.error("❌ [RBAC API] Error fetching escalations:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("⚠️ [RBAC API] PUT /api/rbac/escalations/:id/resolve", {
            id,
          });
          ...
          safeLogger.error("❌ [RBAC API] Error resolving escalation:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
          ...
          const roles = [
            {
              ...
              description: "Creates/edits CAPA, audits, findings, training",
            },
            {
              id: "grc_manager",
              name: "GRC Manager",
              description:
                "Accepts risks, approves policies, compliance sign-off",
            },
            {
              id: "ai_specialist",
              name: "AI Specialist",
              description:
                "View dashboards, configure AI settings (no approvals)",
            },
            {
              id: "bu_owner",
              name: "BU Owner",
              description: "Submit evidence, update action status",
            },
            {
              id: "executive",
              name: "Executive",
              description: "Read-only strategic views",
            },
            { id: "admin", name: "Admin", description: "Full system access" },
          ];
          return c.json({ roles });
        } catch (error: any) {
          safeLogger.error("❌ [RBAC API] Error fetching roles:", error);
          return c.json({ error: "An internal error occurred" }, 500);
        }
      };
    },
