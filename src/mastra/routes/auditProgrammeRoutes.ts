 ...
 * Role helpers
 * ------------------------------------------------------------------------- */
const EDITOR_ROLES   = new Set(['admin', 'head_of_operations_quality', 'quality_manager']);
...
function buildPayloadPreview(p: any): string {
  ...
  if (p.scope_summary) lines.push(`Scope: ${String(p.scope_summary).slice(0, 180)}`);
  ...
  /* ---------------- GET /api/audit-programme ---------------------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        console.error('[AuditProgramme] list error', err);
        return c.json({ error: 'Failed to list programmes', details: err.message }, 500);
      }
    },
  },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        return c.json({
          ...
          programme,
          history,
          can_edit:    canEdit(user.role)    && (programme.status === 'draft' || programme.status === 'cancelled'),
          can_submit:  canEdit(user.role)    &&  programme.status === 'draft',
          can_approve: canApprove(user.role) &&  programme.status === 'pending_signoff',
        });
      } catch (err: any) {
        console.error('[AuditProgramme] detail error', err);
        return c.json({ error: 'Failed to fetch programme', details: err.message }, 500);
      }
    },
  },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        if (!canEdit(user.role)) {
          return forbiddenResponse(c, 'Only Quality Manager or Head of Operations & Quality can draft audit programmes.');
        }
        const body = await c.req.json().catch(() => ({}));
        if (!body?.title || !body?.programme_year) {
          return c.json({ error: 'title and programme_year are required' }, 400);
        }
        const programme = await createProgramme({
          ...
          planned_audits:       Array.isArray(body.planned_audits) ? body.planned_audits : [],
          prepared_by_email:    user.email,
          prepared_by_name:     user.name,
        });
        await logEvent({
          userId: user.userId, userEmail: user.email, userRole: user.role,
          actionType: 'CREATE', entityType: 'audit_programme',
          entityId: String(programme.id), entityName: programme.programme_code,
        ...
        console.error('[AuditProgramme] create error', err);
        return c.json({ error: 'Failed to create programme', details: err.message }, 500);
      }
    },
  },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        const updated = await updateProgramme(id, {
          ...
          planned_audits:       Array.isArray(body.planned_audits) ? body.planned_audits : undefined,
        });
        return c.json({ success: true, programme: updated });
      } catch (err: any) {
        console.error('[AuditProgramme] update error', err);
        return c.json({ error: 'Failed to update programme', details: err.message }, 500);
      }
    },
  },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        if (programme.status !== 'draft') {
          return c.json({ error: `Cannot submit from status "${programme.status}"` }, 409);
        }
        const planned = Array.isArray(programme.planned_audits) ? programme.planned_audits.length : 0;
        if (planned === 0) {
          return c.json({
            error: 'Programme has no planned audits. Add at least one before submitting for sign-off.',
          }, 400);
        }
        ...
        const ticket = await enqueuePendingAction({
          ...
          payload:          { programme_id: programme.id, programme_code: programme.programme_code },
        ...
        await logEvent({
          userId: user.userId, userEmail: user.email, userRole: user.role,
          actionType: 'AI_ACTION', entityType: 'audit_programme',
          entityId: String(programme.id), entityName: programme.programme_code,
          description: `Submitted programme ${programme.programme_code} for sign-off`,
          module: 'audits', severity: 'INFO',
          aiInvolved: false, correlationId: ticket.action_code,
        }).catch(() => { /* non-fatal */ });

        return c.json({ success: true, action_code: ticket.action_code });
      } catch (err: any) {
        console.error('[AuditProgramme] submit error', err);
        return c.json({ error: 'Failed to submit programme', details: err.message }, 500);
      }
    },
  },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        if (!canApprove(user.role)) {
          return forbiddenResponse(c, 'Only Head of Operations & Quality (or admin) can approve.');
        }
        const id = parseInt(c.req.param('id'), 10);
        if (!Number.isFinite(id)) return c.json({ error: 'Invalid id' }, 400);
        const programme = await getProgrammeById(id);
        ...
        if (programme.status !== 'pending_signoff') {
          return c.json({ error: `Cannot approve from status "${programme.status}"` }, 409);
        }
        if (!programme.approval_action_code) {
          ...
          const sub = hist.reverse().find((h: any) => h.action === 'submitted' && h.action_code);
          if (!sub?.action_code) {
            return c.json({ error: 'No HITL ticket found; please re-submit the programme.' }, 409);
          }
          programme.approval_action_code = sub.action_code;
        }

        // Segregation of duties — prevent self-approval unless admin break-glass.
        if (user.role !== 'admin' && programme.prepared_by_email && programme.prepared_by_email === user.email) {
        ...
        if (!ticketCode) return c.json({ error: 'No HITL ticket associated with this programme' }, 409);

        const ticket = await getPendingActionByCode(ticketCode);
        if (!ticket) return c.json({ error: 'HITL ticket missing or expired' }, 409);
        if (ticket.status !== 'pending') {
          return c.json({ error: `HITL ticket is ${ticket.status}, cannot approve` }, 409);
        }
        ...
        if (!claimed) {
          return c.json({ error: 'Could not claim ticket — may be expired or already handled.' }, 409);
        }
        ...
        await logEvent({
          userId: user.userId, userEmail: user.email, userRole: user.role,
          actionType: 'AI_ACTION', entityType: 'audit_programme',
          entityId: String(id), entityName: programme.programme_code,
          description: `Approved programme ${programme.programme_code}`,
          module: 'audits', severity: 'INFO',
          aiInvolved: false, correlationId: ticketCode,
        }).catch(() => { /* non-fatal */ });

        return c.json({ success: true, programme: updated });
      } catch (err: any) {
        console.error('[AuditProgramme] approve error', err);
        return c.json({ error: 'Failed to approve programme', details: err.message }, 500);
      }
    },
  },
  ...
  /* ---------------- POST /api/audit-programme/:id/reject ---------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        if (reason.length < 5) {
          return c.json({ error: 'Rejection reason (min 5 chars) is required.' }, 400);
        }
        const programme = await getProgrammeById(id);
        if (!programme) return c.json({ error: 'Not found' }, 404);
        if (programme.status !== 'pending_signoff') {
          return c.json({ error: `Cannot reject from status "${programme.status}"` }, 409);
        }

        const ticketCode = programme.approval_action_code;
        if (ticketCode) {
          await rejectHitl(ticketCode, {
            userId: user.userId, email: user.email, name: user.name,
          }, reason);
        }
        ...
        await logEvent({
          ...
          module: 'audits', severity: 'WARNING',
          aiInvolved: false, correlationId: ticketCode || undefined,
        }).catch(() => { /* non-fatal */ });

        return c.json({ success: true, programme: updated });
      } catch (err: any) {
        console.error('[AuditProgramme] reject error', err);
        return c.json({ error: 'Failed to reject programme', details: err.message }, 500);
      }
    },
  },
];
