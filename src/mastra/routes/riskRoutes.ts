 ...
 */
const RISK_READ_ROLES: UserRole[] = [
  "admin",
...
async function requireRiskReadAuth(
  c: any,
...
export const riskRoutes = [
      ...
      return async (c: any) => {
        ...
        try {
          ...
          const r = await pool.query(
            `SELECT COUNT(*)::int AS total FROM enterprise_risks`,
          );
          return estimateResponse(estimateFromCount(r.rows[0]?.total, "csv"));
        } catch (error) {
          safeLogger.error("❌ [RiskAPI] Error estimating CSV export:", error);
      ...
      return async (c: any) => {
        ...
        try {
          ...
          const [rRes, aRes] = await Promise.all([
            ...
            pool.query(
              `SELECT COUNT(*)::int AS total FROM risk_treatment_actions`,
            ),
          ]);
          const totalRows =
            (rRes.rows[0]?.total ?? 0) + (aRes.rows[0]?.total ?? 0);
          ...
          safeLogger.error("❌ [RiskAPI] Error estimating XLSX export:", error);
          return c.json({ error: "Failed to estimate export size" }, 500);
        } finally {
          await pool.end();
        }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const pg = await import("pg");
          const localPool = new pg.default.Pool({
            connectionString: process.env.DATABASE_URL,
          });
          ...
          return await stageStreamingExportFromHono(c, () =>
            streamCsv(`risks_${Date.now()}.csv`, headers, mappedRows),
          );
        } catch (error) {
          safeLogger.error("❌ [RiskAPI] Error exporting risks:", error);
      ...
      return async (c: any) => {
        ...
        try {
          ...
          const [rTotR, rLevR, aTotR, aOvR, catsR] = await Promise.all([
            pool.query(`SELECT COUNT(*)::int AS total FROM enterprise_risks`),
            pool.query(
              `SELECT risk_level, COUNT(*)::int AS cnt FROM enterprise_risks GROUP BY risk_level`,
            ),
            pool.query(
              `SELECT COUNT(*)::int AS total FROM risk_treatment_actions`,
            ),
            pool.query(
              `SELECT COUNT(*)::int AS cnt FROM risk_treatment_actions WHERE due_date IS NOT NULL AND status != 'completed' AND due_date < NOW()`,
            ),
            pool.query(
              `SELECT DISTINCT COALESCE(risk_category, 'Uncategorised') AS cat FROM enterprise_risks ORDER BY cat`,
            ),
          ]);
          const rTotal = rTotR.rows[0]?.total ?? 0;
          const rByLev = (l: string) =>
            rLevR.rows.find((r) => r.risk_level === l)?.cnt ?? 0;
          ...
          const actRows = (async function* () {
            try {
              for await (const r of actSource)
                yield r as Record<string, unknown>;
            } finally {
              await pool.end();
            }
          ...
          return await stageStreamingExportFromHono(c, async () =>
            streamXlsx(sheets, `risks_${Date.now()}.xlsx`, {
              title: "Enterprise Risk Register Export",
            }),
          );
        } catch (error) {
          safeLogger.error("❌ [RiskAPI] Error exporting risks XLSX:", error);
          await pool.end();
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!auth.ok) return auth.res;
          const logger = mastra?.getLogger();
          const { getAllRisks, initRiskTables } =
          ...
          logger?.info("📊 [RiskAPI] GET /api/risks", {
            status,
            category,
            risk_level,
            owner_department,
          ...
          const { obfuscateResourceIdsList } =
            await import("../../utils/riskDatabase");
          return c.json({
            risks: obfuscateResourceIdsList(result.risks),
            total: result.total,
          });
        } catch (error) {
          safeLogger.error("❌ [RiskAPI] Error fetching risks:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!auth.ok) return auth.res;
          const logger = mastra?.getLogger();
          const { getRiskHeatmapData, initRiskTables } =
            await import("../../utils/riskDatabase");
          await initRiskTables();
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching heatmap:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!auth.ok) return auth.res;
          const logger = mastra?.getLogger();
          const { getRiskSummaryStats, initRiskTables } =
            await import("../../utils/riskDatabase");
          await initRiskTables();
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching summary:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!auth.ok) return auth.res;
          const logger = mastra?.getLogger();
          const { getRiskTrends, initRiskTables } =
            await import("../../utils/riskDatabase");
          await initRiskTables();
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching trends:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching overdue items:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!auth.ok) return auth.res;
          const logger = mastra?.getLogger();
          const { getRiskCategories, initRiskTables } =
            await import("../../utils/riskDatabase");
          await initRiskTables();
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching categories:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { obfuscateResourceIds, obfuscateResourceIdsList } =
            await import("../../utils/riskDatabase");
          return c.json({
          ...
          safeLogger.error("❌ [RiskAPI] Error fetching risk:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const logger = mastra?.getLogger();
          const { createRisk, initRiskTables } =
            await import("../../utils/riskDatabase");
          const { logEvent } = await import("../../utils/eventLogsDatabase");
          await initRiskTables();

          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] POST /api/risks", {
            title: body.risk_title,
          ...
          const risk = await createRisk({
            ...body,
            identified_by: sessionUser.email,
          });
          ...
          const { obfuscateResourceIds } =
            await import("../../utils/riskDatabase");
          logger?.info("✅ [RiskAPI] Risk created", {
            id: risk.public_id,
            level: risk.risk_level,
          });
          return c.json({ success: true, risk: obfuscateResourceIds(risk) });
        } catch (error) {
          safeLogger.error("❌ [RiskAPI] Error creating risk:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const resolved = resolveRiskId(idParam);
          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] PUT /api/risks/:id", {
            idParam,
            by: sessionUser.email,
          });
          ...
          if (body.status && body.status !== existingRisk.status) {
            return c.json(
              {
                error:
                  "Status changes are not allowed via generic update. Use the dedicated /close, /accept, or /escalate endpoints.",
              },
              400,
            );
          }
          ...
          logger?.info("✅ [RiskAPI] Risk updated", {
            id: existingRisk.public_id,
          });
          return c.json({
            success: true,
          ...
          safeLogger.error("❌ [RiskAPI] Error updating risk:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const resolved = resolveRiskId(idParam);
          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] POST /api/risks/:id/treatment", {
            idParam,
            by: sessionUser.email,
          });
          ...
          logger?.info("✅ [RiskAPI] Treatment action created", {
            actionId: action.public_id,
          });
          return c.json({
            success: true,
            action: obfuscateResourceIds(action),
          });
        } catch (error) {
          safeLogger.error(
            "❌ [RiskAPI] Error creating treatment action:",
            error,
          );
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] PUT /api/risks/treatment/:actionId", {
            actionIdParam,
            by: sessionUser.email,
          });
          ...
          let actionId: number;
          if (resolved.isUUID) {
            const { getTreatmentActionByPublicId } =
              await import("../../utils/riskDatabase");
          ...
          logger?.info("✅ [RiskAPI] Treatment action updated", {
            actionId: updatedAction.public_id,
          });
          return c.json({
            success: true,
            action: obfuscateResourceIds(updatedAction),
          });
        } catch (error) {
          safeLogger.error(
            "❌ [RiskAPI] Error updating treatment action:",
            error,
          );
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const resolved = resolveRiskId(idParam);
          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] POST /api/risks/:id/escalate", {
            idParam,
            by: sessionUser.email,
          });
          ...
          safeLogger.error("❌ [RiskAPI] Error escalating risk:", error);
  ...
  },
  {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          const { requireWriteRole, unauthorizedResponse, forbiddenResponse } =
            await import("../../utils/rbacMiddleware");
          const sessionUser = requireWriteRole(c);
          if (!sessionUser) return unauthorizedResponse(c);
          ...
          const resolved = resolveRiskId(idParam);
          const body = await c.req.json();
          logger?.info("📝 [RiskAPI] POST /api/risks/:id/close", {
            idParam,
            by: sessionUser.email,
          });
          ...
          if (!hasPermission) {
            return forbiddenResponse(
              c,
              "Permission denied: cannot close risks",
            );
          }
          ...
          if (!treatments || treatments.length === 0) {
            return c.json(
              {
                error:
                  "Cannot close risk: at least one treatment action must exist before closure",
              },
              400,
            );
          }
          ...
          safeLogger.error("❌ [RiskAPI] Error closing risk:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info(
            "🔐 [RiskAPI] POST /api/risks/:id/accept (RBAC enforcement)",
            { idParam, userEmail },
          );
          ...
          if (!user) {
            logger?.warn(
              "🚫 [RiskAPI] Risk acceptance blocked - user not registered in system",
              { userEmail },
            );
            await logEvent({
              entityType: "SYSTEM",
          ...
          if (!user.is_active) {
            logger?.warn(
              "🚫 [RiskAPI] Risk acceptance blocked - user account inactive",
              { userEmail },
            );
            return c.json(
              {
          ...
          if (!hasPermission) {
            logger?.warn(
              "🚫 [RiskAPI] Risk acceptance blocked - user lacks GRC permission",
          ...
          if (risk.status === "closed") {
            return c.json(
              { error: "Cannot accept a closed risk. Reopen it first." },
              400,
            );
          }
          ...
          logger?.info("✅ [RiskAPI] Risk accepted by GRC Manager", {
            id: risk.public_id,
          ...
          safeLogger.error("❌ [RiskAPI] Error accepting risk:", error);
