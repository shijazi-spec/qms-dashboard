...
const TRIGGER_REVIEWER_ROLES = new Set([
  ...
  'auditor', 'team_lead', 'executive', 'bu_owner', 'ai_specialist',
]);
      ...
      return async (c: any) => {
        try {
          ...
          if (!TRIGGER_REVIEWER_ROLES.has(user.role)) {
            return c.json({ error: 'Insufficient permissions to view triggers' }, 403);
          }
          ...
          logger?.info('🔔 [TriggerAPI] GET /api/triggers', { userRole: user.role, effectiveRole });

          const triggers = await getPendingTriggers({ type: type as any, role: effectiveRole });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!user) return c.json({ error: 'Authentication required' }, 401);
          if (!TRIGGER_REVIEWER_ROLES.has(user.role)) {
            return c.json({ error: 'Insufficient permissions to view trigger stats' }, 403);
          }

          logger?.info('📊 [TriggerAPI] GET /api/triggers/stats', { userRole: user.role });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info('📝 [TriggerAPI] POST /api/triggers/:id/action', { id, action, userRole: user.role });
          ...
          if (!TRIGGER_ADMIN_ROLES.has(user.role)) {
            if (!current.assigned_role || user.role !== current.assigned_role) {
              return c.json({
                error: 'Forbidden: you are not the assigned reviewer for this trigger',
              }, 403);
            }
          }
          ...
          if (action === 'acknowledge') {
            ...
            if (!decision || !user.email) {
              return c.json({ error: 'decision is required for decide action' }, 400);
            }
            status = 'actioned';
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!user) return c.json({ error: 'Authentication required' }, 401);
          if (!TRIGGER_REVIEWER_ROLES.has(user.role)) {
            return c.json({ error: 'Insufficient permissions to view audit triggers' }, 403);
          }
          ...
          const effectiveRole = TRIGGER_ADMIN_ROLES.has(user.role) ? undefined : user.role;

          logger?.info('🔔 [TriggerAPI] GET /api/triggers/audit/:auditId', { auditId, userRole: user.role, effectiveRole });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!user) return c.json({ error: 'Authentication required' }, 401);
          if (!TRIGGER_REVIEWER_ROLES.has(user.role)) {
            return c.json({ error: 'Insufficient permissions to view notifications' }, 403);
          }
          ...
          logger?.info('📧 [TriggerAPI] GET /api/notifications', { userRole: user.role, effectiveRole });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const id = parseInt(c.req.param('id'));
          logger?.info('📧 [TriggerAPI] POST /api/notifications/:id/read', { id, userRole: user.role });
          ...
          if (
            ...
            return c.json({
              error: 'Forbidden: this notification does not belong to your role',
            }, 403);
          }
