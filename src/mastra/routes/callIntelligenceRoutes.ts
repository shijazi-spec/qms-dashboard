import { requireAdminOrKey, requireRoleOrKey, getSessionUser, unauthorizedResponse, type SessionUser } from '../../utils/rbacMiddleware';

const CALL_READ_ROLES = ['admin', 'ai_specialist', 'head_of_operations_quality', 'quality_manager', 'team_lead', 'grc_manager'] as const;
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getCallRecords, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          ...
          logger?.info("✅ [API] Call records fetched", { count: result.records.length });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getCallAnalyticsSummary, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          ...
          logger?.info("✅ [API] Analytics fetched", { totalCalls: analytics.totalCalls });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getComplianceRecords, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          ...
          logger?.info("✅ [API] Compliance records fetched", { count: result.records.length });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getCallRecordById, updateCallRecord } = await import("../../utils/callIntelligenceDb");
          ...
          const { saveTranscript, saveCallAnalysis, saveQAScore } = await import("../../utils/callIntelligenceDb");
          const { createOpenAI } = await import("@ai-sdk/openai");
          const { generateText } = await import("ai");

          const transcript = data.transcript || "Transcript from recording (placeholder)";
...
Respond with JSON only:
{
  ...
  "ai_insights": "<recommendations>"
          ...
          logger?.info("✅ [API] Call analysis completed", { callId, success: analysisResult.success });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getCallRecordById } = await import("../../utils/callIntelligenceDb");
          ...
          const expectedActions = data.expected_actions || ["notes_updated", "call_logged", "task_created"];
          ...
          const complianceResult = {
            ...
            message: overallCompliance ? "CRM compliance passed" : `${missingActions.length} missing actions`
          };

          logger?.info("✅ [API] Compliance check completed", { callId, success: complianceResult.success });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getComplianceByCallId } = await import("../../utils/callIntelligenceDb");
          
          const compliance = await getComplianceByCallId(callId);

          if (!compliance) {
            return c.json({ error: "Compliance data not found for this call" }, 404);
          }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getTranscriptByCallId } = await import("../../utils/callIntelligenceDb");
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getCallRecords } = await import("../../utils/callIntelligenceDb");
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          if (!data.calendar_event_id || !data.meeting_title || !data.transcript) {
            return c.json({ 
              error: "calendar_event_id, meeting_title, and transcript are required" 
            }, 400);
          }
...
Respond with JSON only:
{
  ...
  "notes": "<additional notes>"
          ...
          logger?.info("✅ [API] MoM generated", { success: momResult.success });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getMOMByEventId } = await import("../../utils/callIntelligenceDb");
    ...
    createHandler: async () => {
      ...
      return async (c: any) => {
        try {
          ...
          console.error("Error serving calls dashboard:", error);
      ...
      return async (c: any) => {
        try {
          ...
          if (file && file.size > 0) {
            ...
            logger?.info("📁 [API] File saved", { fileName, size: file.size, path: audioFilePath });
          }
          ...
          const callRecord = await createCallRecord({
            ...
            status: 'pending',
            metadata: { uploaded_at: new Date().toISOString(), original_filename: file?.name || '' }
          } as any);
          
          // Update the audio_file_path in the database
          if (audioFilePath && callRecord.id) {
            const { pool } = await import("../../utils/database");
            await pool.query('UPDATE call_records SET audio_file_path = $1 WHERE id = $2', [audioFilePath, callRecord.id]);
          }

          logger?.info("✅ [API] Manual call record created", { id: callRecord.id });
      ...
      return async (c: any) => {
        try {
          ...
          const callRecord = await createCallRecord({
            ...
            call_date: new Date(),
            status: 'pending',
            metadata: { uploaded_at: new Date().toISOString(), original_filename: file.name, file_size: file.size }
          });
          ...
          if (autoAnalyze) {
            try {
              ...
              logger?.info("🎙️ [API] Starting audio transcription (Arabic supported)");

              const arrayBuffer = await file.arrayBuffer();
              const audioFile = new File([arrayBuffer], file.name, { type: file.type });
              ...
              logger?.info("📝 [API] Transcription completed", { length: transcriptText.length });
...
قدم تحليلك بصيغة JSON التالية (باللغة العربية):
{
  ...
  "ai_insights": "رؤى وتحليلات إضافية من الذكاء الاصطناعي"
              ...
              try {
                ...
                logger?.warn("⚠️ [API] Failed to parse analysis JSON, using defaults");
                analysisData = {
              ...
              logger?.error("❌ [API] Analysis failed", { error: analysisError });
          ...
          return c.json({
            ...
            message: autoAnalyze ? 'Call uploaded and analysis completed' : 'Call uploaded successfully'
          });
        } catch (error) {
      ...
      return async (c: any) => {
        try {
          ...
          if (!Array.isArray(calls) || calls.length === 0) {
            return c.json({ success: false, error: 'No calls provided. Expected an array of call records.' }, 400);
          }

          if (calls.length > 100) {
            return c.json({ success: false, error: 'Maximum 100 calls per bulk upload' }, 400);
          }
          ...
          for (let i = 0; i < calls.length; i++) {
            ...
            try {
              ...
              const callRecord = await createCallRecord({
                ...
                direction: (call.direction || 'outbound') as 'inbound' | 'outbound',
                recording_url: call.recording_url || '',
                call_date: call.call_date ? new Date(call.call_date) : new Date(),
              ...
              results.push({ row: i + 1, success: true, call_record_id: callRecord.id });
            } catch (err) {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🔌 [API] Testing Five9 connection", { domain: body.domain });

          if (!body.domain || !body.username || !body.password) {
            return c.json({ success: false, error: 'Domain, username, and password are required' }, 400);
          }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("⚙️ [API] Configuring Five9 integration", { domain: body.domain });
          ...
          `, [JSON.stringify({ domain: body.domain, username: body.username, configured_at: new Date().toISOString() })]);
      ...
      return async (c: any) => {
        try {
          ...
          if (configResult.rows.length === 0) {
            return c.json({ success: false, error: 'Five9 not configured. Please configure first.' }, 400);
          }
          ...
          logger?.info("✅ [API] Five9 sync completed (placeholder - actual Five9 API integration pending)");

          return c.json({
            success: true,
            synced_count: 0,
            message: 'Five9 sync completed. Configure Five9 API credentials in secrets for full integration.'
          });
        } catch (error) {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("📝 [API] Saving SDR evaluation", { callId, totalScore: body.total_score });

          const { createOrUpdateQAScore, updateCallStatus, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          await initCallIntelligenceTables();
          ...
          return c.json({
            ...
            message: body.complete ? 'Evaluation completed and saved' : 'Evaluation saved as draft'
          });
        } catch (error) {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🤖 [API] Starting AI-powered SDR evaluation", { callId, teamName });
          ...
          if (!transcript?.transcript_text && audioFilePath) {
            logger?.info("🎙️ [API] No transcript found, transcribing audio first...", { audioPath: audioFilePath });
            ...
            try {
              ...
              if (!fs.default.existsSync(audioPath)) {
                return c.json({ success: false, error: "Audio file not found on server" }, 404);
              }
              ...
              if (!transcribeRes.ok) {
                ...
                logger?.error("❌ [API] Transcription failed", { status: transcribeRes.status, error: errorText });
                return c.json({ success: false, error: "Failed to transcribe audio: " + errorText }, 500);
              }
              
              const transcriptText = await transcribeRes.text();
              logger?.info("✅ [API] Audio transcribed successfully", { length: transcriptText.length });
              
              // Save the transcript
              const { saveTranscript } = await import("../../utils/callIntelligenceDb");
              await saveTranscript({
                call_record_id: callId,
                transcript_text: transcriptText,
                language: 'ar'
              ...
              transcript = { call_record_id: callId, transcript_text: transcriptText } as any;
            } catch (transcribeError: any) {
              logger?.error("❌ [API] Transcription error", { error: transcribeError?.message || transcribeError });
              return c.json({ success: false, error: "Failed to transcribe audio: " + (transcribeError?.message || 'Unknown error') }, 500);
            }
          }
          
          if (!transcript?.transcript_text) {
            return c.json({ success: false, error: "No audio file or transcript available. Please upload an audio recording." }, 400);
          }

          const scorecard = await getActiveSDRScorecard(teamName);
          if (!scorecard) {
            return c.json({ success: false, error: "No active SDR scorecard found" }, 400);
          }

          logger?.info("📊 [API] Using scorecard", { name: scorecard.name, attributes: scorecard.attributes.length });

          const evaluationPrompt = buildSDREvaluationPrompt(transcript.transcript_text, scorecard);
          ...
          try {
            ...
            logger?.error("❌ [API] Failed to parse AI response", { error: parseError });
            return c.json({ success: false, error: "Failed to parse AI evaluation response" }, 500);
          }
          ...
          const evaluation = {
            ...
            dimension_scores: evaluationData.overall_summary?.dimension_scores || { people: 0, process: 0, governance: 0 },
            ...
            coaching_actions: evaluationData.overall_summary?.coaching_actions || [],
            critical_risks: evaluationData.overall_summary?.critical_risks || [],
            coaching_message_ar: evaluationData.coaching_recommendation?.message_ar || '',
            coaching_message_en: evaluationData.coaching_recommendation?.message_en,
            micro_training_topics: evaluationData.coaching_recommendation?.micro_training_topics || [],
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getSDREvaluation, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          await initCallIntelligenceTables();

          const evaluation = await getSDREvaluation(callId);
          
          if (!evaluation) {
            return c.json({ success: false, error: "No SDR evaluation found for this call" }, 404);
          }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getActiveSDRScorecard, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          await initCallIntelligenceTables();

          const scorecard = await getActiveSDRScorecard(teamName);
          
          if (!scorecard) {
            return c.json({ success: false, error: "No active scorecard found" }, 404);
          }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("✅ [API] AI feedback submitted successfully", { feedbackId });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getAITrainingStats, initCallIntelligenceTables } = await import("../../utils/callIntelligenceDb");
          await initCallIntelligenceTables();

          const stats = await getAITrainingStats();

          logger?.info("✅ [API] AI training stats fetched successfully", stats);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("✅ [API] Quality scorecards fetched", { count: scorecards.length });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("✅ [API] Quality scorecard created", { id: scorecard.id });
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("📝 [API] Updating quality scorecard", { id, updates: Object.keys(updates) });

          const { updateScorecard } = await import("../../utils/database");
          const scorecard = await updateScorecard(id, updates);

          if (!scorecard) {
            return c.json({ success: false, error: "Scorecard not found" }, 404);
          }
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🔄 [API] Syncing call evaluation to Zoho CRM", { callId });

          const { getSDREvaluation, getCallRecordById } = await import("../../utils/callIntelligenceDb");
          ...
          const evaluation = await getSDREvaluation(callId);
          if (!evaluation) {
            return c.json({ success: false, error: "No evaluation found for this call" }, 404);
          }
          ...
          if (callRecord.lead_id) {
            await updateZohoRecordNotes('Leads', callRecord.lead_id, noteContent);
            synced = true;
            syncTarget = `Lead ${callRecord.lead_id}`;
          } else if (callRecord.deal_id) {
            await updateZohoRecordNotes('Deals', callRecord.deal_id, noteContent);
            synced = true;
          ...
          logger?.info("✅ [API] Call evaluation synced to Zoho", { callId, syncTarget });
