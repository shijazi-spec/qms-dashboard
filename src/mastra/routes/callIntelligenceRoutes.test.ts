  ...
  const res = await handler(
    makeContext({
      ...
      body: {
        ...
        domain: `acme.${value}.five9.com`,
        username: `agent-${value}`,
        password: "literal-password-not-persisted",
      },
    }),
  );
  assert(
...
for (const [keyLabel, rawSecret] of Object.entries(SECRETS)) {
  ...
  const res = await handler(
    makeContext({
      ...
      headers: { "X-Admin-Key": ADMIN_KEY },
      body: {
        domain: `acme.${rawSecret}.five9.com`,
...
// Anti-tautology — clean configuration preserved verbatim.
{
  ...
  if (params) {
    const flat = JSON.stringify(params);
    assert(
      flat.includes("acme.five9.com") && flat.includes("agent-clean"),
