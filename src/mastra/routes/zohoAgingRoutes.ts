 ...
 *   GET /api/zoho/leads/:id/status-aging      single-record helper
 *   GET /api/zoho/deals/:id/stage-aging       single-record helper
 *   GET /api/zoho/leads/aging                 paginated list (sorted desc)
 *   GET /api/zoho/deals/aging                 paginated list (sorted desc)
 *
 * The list endpoints page Zoho themselves — the caller never has to supply
 * record ids — and accept `limit` (default 50, max 200), `cursor` (Zoho
...
/**
 * Allow tests / future automation to swap the Zoho fetchers without going
 * through the live Zoho client. Defaults to the production fetchers in
 * `src/utils/zohoAging.ts`.
 */
...
export const zohoAgingRoutes = [
  ...
  {
    path: "/api/zoho/deals/:id/stage-aging",
    method: "GET" as const,
    createHandler: async () => {
      return async (c: any) => {
        try {
  ...
  {
    path: "/api/zoho/leads/:id/status-aging",
    method: "GET" as const,
    createHandler: async () => {
      return async (c: any) => {
        try {
