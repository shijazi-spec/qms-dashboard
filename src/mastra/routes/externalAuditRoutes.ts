    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        const rows = await listExternalAudits({
          ...
          kind:
            (url.searchParams.get("kind") as ExternalAuditKind) || undefined,
          upcoming_only: url.searchParams.get("upcoming_only") === "true",
        });
        return c.json({ success: true, rows });
      } catch (err: any) {
        logger.error("[ExternalAudits] list error", err);
  ...
  /* ---------------- GET /api/external-audits/summary -------------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ExternalAudits] summary error", err);
        return c.json(
          { error: "Failed to build summary", details: err.message },
          500,
        );
      }
    },
  ...
  /* ---------------- GET /api/external-audits/certificates --------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ExternalAudits] certs list error", err);
        return c.json(
          { error: "Failed to list certificates", details: err.message },
          500,
        );
      }
    },
  ...
  /* ---------------- POST /api/external-audits --------------------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ExternalAudits] create error", err);
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        const [checklist, certs] = await Promise.all([
          listChecklist(id),
          listCertificates({}).then((rows) =>
            rows.filter((r) => r.external_audit_id === id),
          ),
        ]);
        return c.json({ success: true, audit, checklist, certificates: certs });
      } catch (err: any) {
        logger.error("[ExternalAudits] detail error", err);
  ...
  /* ---------------- PUT /api/external-audits/:id ------------------------ */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ExternalAudits] update error", err);
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        if (!body?.category || !body?.requirement) {
          return c.json(
            { error: "category and requirement are required" },
            400,
          );
        }
        ...
        logger.error("[ExternalAudits] checklist add error", err);
        return c.json(
  ...
  /* ---------------- PUT /api/external-audits/checklist/:itemId ---------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ExternalAudits] checklist update error", err);
        return c.json(
      ...
      try {
        ...
        if (
          ...
          return c.json(
            {
              error:
                "certificate_number, standard, and certification_body are required",
            },
            400,
          );
        }
        ...
        await logEvent({
          ...
          userRole: user.role,
          actionType: "CREATE",
          entityType: "external_audit_certificate",
        ...
        logger.error("[ExternalAudits] cert create error", err);
        return c.json(
          { error: "Failed to create certificate", details: err.message },
          500,
        );
      }
    },
