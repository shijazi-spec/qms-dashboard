      ...
      return async (c: any) => {
        ...
        try {
          const [auditScoreRes, ncResolutionRes, capaEffRes, kpiRes, complianceRes] = await Promise.all([
            ...
            pool.query(`SELECT COUNT(*) FILTER (WHERE effectiveness_result = 'effective') as effective, COUNT(*) FILTER (WHERE effectiveness_result IS NOT NULL) as reviewed FROM capa_records WHERE created_at >= NOW() - INTERVAL '90 days'`).catch(() => ({ rows: [{ effective: 0, reviewed: 0 }] })),
