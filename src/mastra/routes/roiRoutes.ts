    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { listROIInitiatives, initROITables } =
          ...
          logger?.info("✅ [ROI API] Initiatives fetched", {
            count: result.initiatives.length,
          });
          return c.json(result);
        } catch (error) {
          safeLogger.error("Error fetching ROI initiatives:", error);
          return c.json(
            {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getROIAnalytics, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error fetching ROI analytics:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          safeLogger.error("Error calculating ROI:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const logger = mastra?.getLogger();
          const data = await c.req.json();
          logger?.info("💰 [ROI API] Creating initiative", {
          ...
          const {
            createROIInitiative,
            initROITables,
            createManpowerBreakdown,
            createPlatformCost,
            createErrorCosts,
            createRevenueImpact,
            createImplementationBreakdown,
            createRiskInputs,
          } = await import("../../utils/roiDatabase");
          await initROITables();
          ...
          logger?.info("✅ [ROI API] Initiative created", {
            id: initiative.id,
          });
          ...
          if (
            data.platformCosts &&
            Array.isArray(data.platformCosts) &&
            initiative.id
          ) {
            logger?.info("📊 [ROI API] Creating platform costs", {
              count: data.platformCosts.length,
            });
          ...
          if (data.implementation && initiative.id) {
            ...
            breakdownResults.implementation =
              await createImplementationBreakdown({
                ...data.implementation,
                initiative_id: initiative.id,
              });
          }
          ...
          return c.json({
            ...
            initiative,
            breakdowns:
              Object.keys(breakdownResults).length > 0
                ? breakdownResults
                : undefined,
          });
        } catch (error) {
          safeLogger.error("Error creating ROI initiative:", error);
          return c.json(
            {
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getFullInitiativeDetails, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error fetching full initiative details:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info("📊 [ROI API] Creating/updating manpower breakdown", {
            initiativeId: id,
          });

          const {
            createManpowerBreakdown,
            updateManpowerBreakdown,
            getManpowerBreakdown,
            initROITables,
            getROIInitiativeById,
          } = await import("../../utils/roiDatabase");
          await initROITables();
          ...
          if (existing) {
            logger?.info("📝 [ROI API] Updating existing manpower breakdown", {
              id,
            });
            breakdown = await updateManpowerBreakdown(id, data);
          } else {
            logger?.info("➕ [ROI API] Creating new manpower breakdown", {
              id,
            });
            breakdown = await createManpowerBreakdown({
              ...data,
              initiative_id: id,
            });
          }
          ...
          safeLogger.error("Error saving manpower breakdown:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info("📊 [ROI API] Adding platform cost(s)", {
            initiativeId: id,
          });

          const { createPlatformCost, initROITables, getROIInitiativeById } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          logger?.info("✅ [ROI API] Platform cost(s) added", {
            initiativeId: id,
            count: createdCosts.length,
          });
          return c.json({ success: true, platformCosts: createdCosts });
        } catch (error) {
          safeLogger.error("Error adding platform cost:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("📊 [ROI API] Listing platform costs", {
            initiativeId: id,
          });

          const { listPlatformCosts, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();

          const platformCosts = await listPlatformCosts(id);

          logger?.info("✅ [ROI API] Platform costs fetched", {
            initiativeId: id,
            count: platformCosts.length,
          });
          return c.json({ platformCosts });
        } catch (error) {
          safeLogger.error("Error listing platform costs:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { deletePlatformCost, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error deleting platform cost:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info("📊 [ROI API] Creating/updating error costs", {
            initiativeId: id,
          });
          ...
          safeLogger.error("Error saving error costs:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info("📊 [ROI API] Creating/updating revenue impact", {
            initiativeId: id,
          });
          ...
          safeLogger.error("Error saving revenue impact:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info(
            "📊 [ROI API] Creating/updating implementation breakdown",
            { initiativeId: id },
          );

          const {
            createImplementationBreakdown,
            updateImplementationBreakdown,
            getImplementationBreakdown,
            initROITables,
            getROIInitiativeById,
          } = await import("../../utils/roiDatabase");
          await initROITables();
          ...
          if (existing) {
            logger?.info(
              "📝 [ROI API] Updating existing implementation breakdown",
              { id },
            );
            implementation = await updateImplementationBreakdown(id, data);
          } else {
            logger?.info("➕ [ROI API] Creating new implementation breakdown", {
              id,
            });
            implementation = await createImplementationBreakdown({
              ...data,
              initiative_id: id,
            });
          }
          ...
          safeLogger.error("Error saving implementation breakdown:", error);
          return c.json(
            { error: "Failed to save implementation breakdown" },
            500,
          );
        }
      };
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const data = await c.req.json();
          logger?.info("📊 [ROI API] Creating/updating risk inputs", {
            initiativeId: id,
          });
          ...
          if (existing) {
            ...
            riskInputs = await updateRiskInputs(
              id,
              data,
              initiative.npv || undefined,
            );
          } else {
          ...
          safeLogger.error("Error saving risk inputs:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("📊 [ROI API] Fetching AI validation logs", {
            initiativeId: id,
          });

          const { listAIValidationLogs, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();

          const validationLogs = await listAIValidationLogs(id);

          logger?.info("✅ [ROI API] AI validation logs fetched", {
            initiativeId: id,
            count: validationLogs.length,
          });
          return c.json({ validationLogs });
        } catch (error) {
          safeLogger.error("Error fetching validation logs:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          logger?.info("🔍 [ROI API] Running AI validation on initiative", {
            initiativeId: id,
          });

          const {
            getFullInitiativeDetails,
            createAIValidationLog,
            initROITables,
          } = await import("../../utils/roiDatabase");
          await initROITables();
          ...
          if (fullDetails.manpowerBreakdown) {
            ...
            if (salary < 3000) {
              const log = await createAIValidationLog({
                ...
                reason:
                  "Salary below minimum threshold of 3000 SAR - this may indicate data entry error or need for verification",
                confidence_score: 85,
                accepted: false,
              });
              validationResults.push({
                ...
                message: "Salary below 3000 SAR",
                log,
              });
              logger?.info("⚠️ [ROI API] Validation warning: Low salary", {
                salary,
              });
            }
            if (salary > 100000) {
              const log = await createAIValidationLog({
                ...
                reason:
                  "Salary exceeds typical maximum of 100000 SAR - verify this is accurate",
                confidence_score: 80,
                accepted: false,
              });
              validationResults.push({
                type: "warning",
                field: "avg_monthly_salary",
                message: "Salary exceeds 100000 SAR",
                log,
              });
              logger?.info("⚠️ [ROI API] Validation warning: High salary", {
                salary,
              });
            }
          }
          ...
          if (fullDetails.errorCosts) {
            ...
            if (errorRate > 50) {
              const log = await createAIValidationLog({
                ...
                suggested_value: "50",
                reason:
                  "Error rate exceeds 50% - this is unusually high and should be verified",
                confidence_score: 90,
                accepted: false,
              });
              validationResults.push({
              ...
              logger?.info("⚠️ [ROI API] Validation warning: High error rate", {
                errorRate,
              });
            }
          }
          ...
          const implCost =
            fullDetails.implementationBreakdown
              ?.calculated_total_implementation ||
            fullDetails.implementation_cost ||
            0;
          const expectedSavings =
            (fullDetails.expected_savings_monthly || 0) *
            (fullDetails.project_duration_months || 12);
          ...
          if (implCost > 0 && expectedSavings > 0) {
            ...
            if (ratio > 2) {
              ...
              logger?.info(
                "⚠️ [ROI API] Validation warning: High implementation to savings ratio",
                { ratio },
              );
            }
          }
          ...
          if (
            ...
            const log = await createAIValidationLog({
              ...
              suggested_value: "> 0",
              reason:
                "Negative NPV indicates the initiative may not be financially viable",
              confidence_score: 95,
              accepted: false,
            });
            validationResults.push({
              type: "critical",
              field: "npv",
              message: "Negative NPV",
              log,
            });
            logger?.info("🚨 [ROI API] Validation critical: Negative NPV", {
              npv: fullDetails.npv,
            });
          }
          ...
          return c.json({
            ...
            summary: {
              ...
              warnings: validationResults.filter((r) => r.type === "warning")
                .length,
              critical: validationResults.filter((r) => r.type === "critical")
                .length,
            },
          });
        } catch (error) {
          safeLogger.error("Error running validation:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { getROIInitiativeById, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error fetching ROI initiative:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { updateROIInitiative, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error updating ROI initiative:", error);
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        try {
          ...
          const { deleteROIInitiative, initROITables } =
            await import("../../utils/roiDatabase");
          await initROITables();
          ...
          safeLogger.error("Error deleting ROI initiative:", error);
    ...
    createHandler: async () => {
      ...
      return async (c: any) => {
        try {
          ...
          safeLogger.error(
            "ROI dashboard not found in any path:",
            possiblePaths,
          );
          return c.text("ROI dashboard not found", 404);
        } catch (error) {
          safeLogger.error("Error serving ROI dashboard:", error);
