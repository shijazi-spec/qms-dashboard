...
async function ensure() {
  ...
  await fs.mkdir(INTAKE_UPLOAD_DIR, { recursive: true }).catch(() => {
    /* best effort */
  });
  ready = true;
}
...
async function saveIntakeFile(file: File): Promise<{
  ...
  if (!ALLOWED_INTAKE_EXT.has(ext)) {
    throw new Error(
      `File type not allowed: ${ext}. Allowed: ${[...ALLOWED_INTAKE_EXT].join(", ")}`,
    );
  }
  if (file.size > MAX_INTAKE_FILE_SIZE) {
    throw new Error(
      `File too large: ${(file.size / 1024 / 1024).toFixed(1)} MB (max 30 MB)`,
    );
  }
...
async function extractFindingsFromText(text: string): Promise<any[]> {
  ...
  return findings.map((f: any) => ({
    ...
    severity: ["critical", "major", "minor", "observation"].includes(f.severity)
      ? f.severity
      : "unknown",
    category: f.category ?? null,
    responsible_party: f.responsible_party ?? null,
    due_date:
      f.due_date && /^\d{4}-\d{2}-\d{2}$/.test(f.due_date)
        ? new Date(f.due_date)
        : null,
    source_page: Number.isFinite(f.source_page) ? f.source_page : null,
    source_excerpt:
      typeof f.source_excerpt === "string"
        ? f.source_excerpt.slice(0, 2000)
        : null,
  ...
  /* ---------------- GET /api/manual-audit-intake ------------------------ */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ManualIntake] list error", err);
        return c.json(
          { error: "Failed to list intakes", details: err.message },
          500,
        );
      }
    },
   ...
   * ---------------------------------------------------------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        const source = String(
          body["audit_source_party"] || "",
        ) as AuditSourceParty;
        if (!source)
          return c.json({ error: "audit_source_party is required" }, 400);
        ...
        const intake = await createIntake({
          ...
          auditor_name: body["auditor_name"]
            ? String(body["auditor_name"])
            : undefined,
          audit_date: body["audit_date"]
            ? new Date(String(body["audit_date"]))
            : null,
        ...
        await logEvent({
          ...
          userRole: user.role,
          actionType: "CREATE",
          entityType: "manual_audit_intake",
          entityId: String(intake.id),
          entityName: intake.intake_code,
        ...
        logger.error("[ManualIntake] upload error", err);
        return c.json(
          { error: "Failed to create intake", details: err.message },
          500,
        );
      }
    },
  ...
  /* ---------------- GET /api/manual-audit-intake/:id -------------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ManualIntake] detail error", err);
        return c.json(
          { error: "Failed to fetch intake", details: err.message },
          500,
        );
      }
    },
  ...
  /* ---------------- GET /api/manual-audit-intake/:id/file --------------- */
  {
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        logger.error("[ManualIntake] file download error", err);
        return c.json(
          { error: "Failed to load file", details: err.message },
          500,
        );
      }
    },
      ...
      try {
        ...
        if (!text) {
          return c.json(
            {
              error:
                "No text to extract. Provide `text` in the body or upload a .txt/.md file. " +
        ...
        try {
          ...
          await logEvent({
            ...
            severity: "INFO",
            aiInvolved: true,
            correlationId: intake.intake_code,
          }).catch(() => {
            /* non-fatal */
          });
        ...
        logger.error("[ManualIntake] extract error", err);
        return c.json(
          { error: "Failed to extract", details: err.message },
          500,
        );
      }
    },
    ...
    createHandler: async () => async (c: any) => {
      try {
        ...
        await logEvent({
          ...
          userRole: user.role,
          actionType: "UPDATE",
          entityType: "manual_audit_finding",
        ...
        logger.error("[ManualIntake] review error", err);
        return c.json(
          { error: "Failed to review finding", details: err.message },
          500,
        );
      }
    },
      ...
      try {
        ...
        if (accepted.length === 0) {
          return c.json(
            {
              error:
                "No accepted findings to promote. Accept at least one finding first.",
            },
            400,
          );
        }
        ...
        const audit = await createAudit({
          ...
          description:
            `Imported via Manual Audit Intake ${intake.intake_code}. ` +
        ...
        await updateIntakeStatus(id, "accepted", {
          ...
          findings_rejected: findings.filter((f) => f.status === "rejected")
            .length,
        ...
        logger.error("[ManualIntake] finalize error", err);
        return c.json(
          { error: "Failed to finalize", details: err.message },
          500,
        );
      }
    },
