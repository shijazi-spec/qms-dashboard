...
for (const { label, value } of SECRET_LIKE_STRINGS) {
  ...
  await upsertOidcUser({
    sub: `oidc-sub-${label}`,
    email: `user-${label.replace(/\W+/g, "")}@example.com`,
    name: `Display Name ${value}`,
    picture: `https://cdn.example.com/avatar.png?ref=${value}`,
  });
...
for (const [keyLabel, rawSecret] of Object.entries(SECRETS)) {
  ...
  await upsertOidcUser({
    ...
    email: `user-${keyLabel}@example.com`,
...
for (const { label, value } of SECRET_LIKE_STRINGS) {
  ...
  await upsertOidcUser({
    ...
    email: `existing-${label.replace(/\W+/g, "")}@example.com`,
    name: `Display Name ${value}`,
    picture: `https://cdn.example.com/avatar.png?ref=${value}`,
  });
  const params = lastWriteParams();
...
// Anti-tautology: an innocuous profile is preserved verbatim.
{
  ...
  if (params) {
    const flat = JSON.stringify(params);
    assert(
      flat.includes("Alice Anderson") && flat.includes("alice@walaplus.example"),
