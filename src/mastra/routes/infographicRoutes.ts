      ...
      finish(() =>
        reject(
          err && (err as any).code === "ENOENT"
            ? new Error(
                "ImageMagick (`magick`) is not installed on this server",
              )
            : err,
        ),
      ),
    ...
    proc.on("close", (code) =>
      finish(() => {
        if (code === 0) resolve(Buffer.concat(chunks));
          ...
          reject(
            new Error(
              `magick exited ${code}: ${Buffer.concat(errChunks).toString().slice(0, 200)}`,
            ),
          );
      }),
    );
      ...
      return async (c: any) => {
        ...
        if (!user)
          return forbiddenResponse(
            c,
            "Insufficient permissions for infographic catalog",
          );
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        ...
        if (!user)
          return forbiddenResponse(
            c,
            "Insufficient permissions to share infographic",
          );
          ...
          try {
            ...
            if (
              ...
              const blocks = [
                {
                  type: "header",
                  text: { type: "plain_text", text: meta.title },
                },
                {
                  type: "section",
                  text: {
                ...
                },
                {
                  ...
                  elements: [
                    {
                      type: "mrkdwn",
                      text: `📎 _A PNG attachment couldn't be uploaded — Slack bot is missing the *files:write* scope. Open the WalaPlus dashboard to view or download._`,
                    },
                  ],
                },
                { type: "divider" },
                {
                  ...
                  elements: [
                    {
                      type: "mrkdwn",
                      text: `🕒 Generated ${new Date().toISOString().slice(0, 16).replace("T", " ")} UTC · WalaPlus QMS`,
                    },
                  ],
                },
              ];
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        ...
        const recipients: string[] = (
          Array.isArray(toRaw) ? toRaw : String(toRaw || "").split(/[,;\s]+/)
        )
          .map((s: string) => s.trim())
          .filter(Boolean);
        ...
        if (recipients.length > 20)
          return c.json({ error: "Maximum 20 recipients per send" }, 400);
        ...
        try {
          ...
          logger?.info(
            `📧 [Infographic] Emailing ${section} to ${recipients.length} recipient(s)`,
          );
          const svg = await buildSectionInfographic(section);
          const png = await svgToPng(svg);
          ...
          const subjectFinal =
            subject || `${meta.title} — WalaPlus snapshot (${stamp})`;
          ...
          const fromEmail =
            ...
            "WalaPlus QMS <onboarding@resend.dev>";
          ...
          if (error) {
            safeLogger.error("[Infographic] Email failed:", error);
            return c.json(
              {
          ...
          safeLogger.error("[Infographic] Email share failed:", error);
          return c.json(
            {
              error: "Failed to send email",
    ...
    createHandler: async ({ mastra }: any) => {
      return async (c: any) => {
        ...
        if (!user)
          return forbiddenResponse(
            c,
            "Insufficient permissions for infographic",
          );
        const logger = mastra?.getLogger();
        const raw = c.req.param("section") || "";
        const url = new URL(c.req.url);
        const wantPng =
          raw.endsWith(".png") || url.searchParams.get("format") === "png";
        const wantDownload = url.searchParams.get("download") === "1";
        const section = raw.replace(/\.(svg|png)$/, "") as InfographicSection;

        if (!VALID_SECTIONS.has(section)) {
          return c.json(
            { error: "Unknown section", valid: Array.from(VALID_SECTIONS) },
            404,
          );
        }
        ...
        try {
          logger?.info(
            `🎨 [Infographic] Rendering ${section} (${wantPng ? "png" : "svg"})`,
          );
          const svg = await buildSectionInfographic(section);
          const stamp = new Date().toISOString().slice(0, 10);
          ...
          safeLogger.error("[Infographic] Render failed:", error);
          return c.json(
            { error: "Failed to render infographic", detail: error?.message },
            500,
          );
        }
      };
