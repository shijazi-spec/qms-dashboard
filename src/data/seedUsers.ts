// Auto-generated from CRM_Users_Complete_117_Updated (2026-04-18). Do not edit by hand;
...
export const SEED_USERS: SeedUser[] = [
  ...
  { name: 'Abdalrzaq Alshamari', team: 'Unassigned', status: 'Inactive', totalRecords: 30, modules: ['Contacts', 'Accounts'] },
  ...
  { name: 'Mohammed Qasem', team: 'CRM Admin', status: 'Active', totalRecords: 3, modules: ['Leads'] },
  { name: 'Mohammed Ridha', team: 'Unassigned', status: 'Inactive', totalRecords: 3, modules: ['Contacts', 'Accounts'] },
  ...
  { name: 'عبدالمجيد الشبيلي', team: 'Unassigned', status: 'Inactive', totalRecords: 193, modules: ['Contacts', 'Accounts'] },
...
export const SEED_USERS_BY_NAME: Record<string, SeedUser> = SEED_USERS.reduce((acc, u) => {
  acc[u.name.trim().replace(/\s+/g, ' ').toLowerCase()] = u;
  return acc;
}, {} as Record<string, SeedUser>);

export function findSeedUser(name: string | undefined | null): SeedUser | undefined {
  if (!name) return undefined;
  return SEED_USERS_BY_NAME[name.trim().replace(/\s+/g, ' ').toLowerCase()];
}
