...
interface FinalKpi {
  ...
  unit: string; // "%" | "days"
  target: number;
  /** Weightage % from the Excel WT column (share of its owner's scorecard). */
  weight: number;
...
export const FINAL_KPIS: FinalKpi[] = [
  ...
  // QM-KPI-016 — the 5-stage Pilot Validation plan, given its own code
  // (Sarah 2026-09-10).
  //
  // It used to live under QM-KPI-008. When that became BU Coverage Rate the
  // plan was left keyed to a KPI whose value comes from somewhere else, and
  // with calc_mode no longer "checklist" kpis.html stopped offering the
  // "Manage Checklist" button at all — real per-BU tick progress, still in the
  // database, with no way to reach it.
  //
  // Name, description and formula are the ORIGINAL QM-KPI-008 wording: this is
  // the metric that row always described, moved to a code that computes it.
  //
  // weight 0 deliberately. QM-KPI-008 keeps the 20 it already had, and
  // splitting one KPI into two must not quietly hand Quality 20 more points of
  // scorecard. Assigning a real share is Sarah's call, not a side effect.
  { code: "QM-KPI-016", name: "BU Pilot Validation Completion Rate", owner_type: "quality_manager", owner_name: "Sarah Hijazi", category: "governance", unit: "%", target: 100, weight: 0, direction: "higher_is_better", frequency: "quarterly", calc_mode: "checklist", description: "Measures the percentage of planned pilot-ready business units that completed pilot validation, including pilot execution, reporting, and action planning", formula: "Planned Pilot-Ready BUs Completing Pilot Validation ÷ Total Quarterly Pilot-Planned BUs × 100", data_source: "QMS Platform" },
  ...
  { code: "GRC-KPI-014", name: "Third-Party Assessment SLA", owner_type: "grc_manager", owner_name: "Maram AlHarbi", category: "vendor", unit: "%", target: 90, weight: 10, direction: "higher_is_better", frequency: "quarterly", calc_mode: "manual", description: "Measures the percentage of in-scope third-party, client, and partner security/risk assessments completed within the defined SLA", formula: "Assessments Completed Within SLA ÷ Total In-Scope Assessment Requests × 100", data_source: "Third-Party Assessment Log" },
  ...
  // ───────────── Legal — Ali Fahad (5) ─────────────
  { code: "LEG-KPI-01", name: "Legal Governance Score", owner_type: "legal_specialist", owner_name: "Ali Fahad", category: "compliance", unit: "%", target: 95, weight: 100, direction: "higher_is_better", frequency: "quarterly", calc_mode: "auto", north_star: true, description: "Measures overall legal operational performance across contract review timeliness, contract clause compliance, contract archiving discipline, and legal action closure", formula: "Equal-weighted average of achievement % of Legal KPIs #2–#5 (each weighted 1/4)", data_source: "KPI Dashboard" },
  { code: "LEG-KPI-02", name: "Contract Review SLA", owner_type: "legal_specialist", owner_name: "Ali Fahad", category: "compliance", unit: "%", target: 95, weight: 25, direction: "higher_is_better", frequency: "monthly", calc_mode: "manual", description: "Measures the percentage of in-scope contracts reviewed within the defined SLA, while controlling overdue review backlog", formula: "Contracts Reviewed Within SLA ÷ Total Contracts Due for Review × 100", data_source: "Contract Register" },
  { code: "LEG-KPI-06", name: "Contract Compliance Coverage", owner_type: "legal_specialist", owner_name: "Ali Fahad", category: "compliance", unit: "%", target: 100, weight: 25, direction: "higher_is_better", frequency: "quarterly", calc_mode: "manual", description: "Measures the percentage of reviewed contracts containing all mandatory standard legal clauses required by the organization", formula: "Contracts With Required Clauses ÷ Total Contracts Reviewed × 100", data_source: "Contract Register" },
  { code: "LEG-KPI-08", name: "Contract Archiving Compliance", owner_type: "legal_specialist", owner_name: "Ali Fahad", category: "compliance", unit: "%", target: 100, weight: 25, direction: "higher_is_better", frequency: "monthly", calc_mode: "manual", description: "Measures the percentage of finalized contracts that are archived, indexed, and retrievable in the approved contract repository within the required timeframe", formula: "Archived Finalized Contracts ÷ Total Finalized Contracts Requiring Archive × 100", data_source: "Archive Folder" },
  { code: "LEG-KPI-09", name: "Legal Action Closure Rate", owner_type: "legal_specialist", owner_name: "Ali Fahad", category: "compliance", unit: "%", target: 90, weight: 25, direction: "higher_is_better", frequency: "quarterly", calc_mode: "manual", description: "Measures the percentage of due legal actions closed within the agreed timeline", formula: "Legal Actions Closed On Time ÷ Total Due Legal Actions × 100", data_source: "Legal Action Log" },

  // ───────────── Shared — GRQ Team (3) ─────────────
  { code: "GRQ-KPI-01", name: "GRQ Health Score", owner_type: "shared", owner_name: "GRQ Team", category: "governance", unit: "%", target: 90, weight: 2.5, direction: "higher_is_better", frequency: "quarterly", calc_mode: "auto", north_star: true, description: "Measures the overall operational health and performance of the Governance, Risk, Quality, and Legal functions through a consolidated achievement score", formula: "(Quality KPI Achievement Score × 35% + GRC KPI Achievement Score × 35% + GRQ Specialist KPI Achievement Score × 15% + Legal KPI Achievement Score × 15%)", data_source: "KPI Dashboard" },
  { code: "GRQ-KPI-02", name: "Quality ↔ GRC Handoff Effectiveness", owner_type: "shared", owner_name: "GRQ Team", category: "governance", unit: "%", target: 95, weight: 2.5, direction: "higher_is_better", frequency: "monthly", calc_mode: "auto", description: "Measures the percentage of handoffs between Quality and GRC that are submitted on time, correctly logged, accepted, and transferred into the required workflow without rework or delay", formula: "Successful Handoffs ÷ Total Handoffs × 100", data_source: "Handoff Emails" },
];
    ...
    await pool.query(
      `INSERT INTO kpi_definitions
         (kpi_name, kpi_code, description, owner_type, owner_name, category, formula, data_source, unit, frequency,
          threshold_green, threshold_amber, threshold_red, threshold_direction, target_value, weight, is_active, is_north_star, calc_mode)
