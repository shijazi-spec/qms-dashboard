...
export async function initOnboardingTables(): Promise<void> {
  ...
  try {
    ...
    const stepCheck = await client.query(
      "SELECT COUNT(*) FROM onboarding_tour_steps",
    );
    ...
    const tooltipCheck = await client.query(
      "SELECT COUNT(*) FROM tooltip_definitions",
    );
    if (parseInt(tooltipCheck.rows[0].count) === 0) {
      await seedDefaultTooltips(client);
    }

    logger.info("✅ Onboarding tables initialized successfully");
  } finally {
    client.release();
  }
}
...
async function seedDefaultTourSteps(client: any): Promise<void> {
  const defaultSteps = [
    {
      step_number: 1,
      step_name: "dashboard_overview",
      target_role: "all",
      title: "Dashboard Overview",
      description:
        "This is your main dashboard showing key quality metrics, KPIs, and system health. Use the navigation bar to access different modules.",
      target_element: "#main-dashboard",
      position: "center",
    },
    {
      step_number: 2,
      step_name: "projects_governance",
      target_role: "all",
      title: "Projects & Governance",
      description:
        "Track quality projects, manage PMP portfolios, and monitor governance compliance. Each project includes risks, milestones, and stakeholders.",
      target_element: 'a[href="/projects"]',
      position: "bottom",
    },
    {
      step_number: 3,
      step_name: "roi_calculator",
      target_role: "all",
      title: "ROI / NPV Calculator",
      description:
        "Submit quality improvement initiatives here. Enter costs, expected savings, and let the system calculate ROI, NPV, and payback period with AI recommendations.",
      target_element: 'a[href="/roi"]',
      position: "bottom",
    },
    {
      step_number: 4,
      step_name: "event_logs",
      target_role: "all",
      title: "Event Logs & Audit Trail",
      description:
        "All system actions are logged here for compliance and transparency. Filter by date, action type, or module to trace any activity.",
      target_element: 'a[href="/logs"]',
      position: "bottom",
    },
    {
      step_number: 5,
      step_name: "ai_assistance",
      target_role: "all",
      title: "AI Assistance",
      description:
        "Our Agentic AI validates your inputs, provides recommendations, and helps ensure data quality. Look for the purple AI badges throughout the system.",
      target_element: ".ai-badge",
      position: "left",
    },
    {
      step_number: 6,
      step_name: "submission_flow",
      target_role: "bu_user",
      title: "Submission & Review Flow",
      description:
        "After submitting an initiative, Quality and Finance teams review it. You can track status in your dashboard. Approved initiatives proceed to implementation.",
      target_element: "#submission-status",
      position: "center",
    },
  ];
  ...
  logger.info("✅ Default tour steps seeded");
}

async function seedDefaultTooltips(client: any): Promise<void> {
  const defaultTooltips = [
    {
      field_id: "expected_savings_monthly",
      module: "roi",
      field_label: "Expected Monthly Savings (SAR)",
      tooltip_text:
        "Enter the estimated monthly cost savings this initiative will generate once fully implemented.",
      calculation_hint:
        "Calculate by: (Current monthly cost - Expected monthly cost after implementation)",
      data_source_hint:
        "Finance or Operations department can provide current cost data.",
    },
    {
      field_id: "implementation_cost",
      module: "roi",
      field_label: "Implementation Cost (SAR)",
      tooltip_text:
        "Total one-time cost to implement this initiative, including technology, training, and change management.",
      calculation_hint:
        "Sum of: Technology costs + Training costs + Consulting fees + Contingency",
      data_source_hint: "IT, HR Training, and Procurement departments.",
    },
    {
      field_id: "project_duration_months",
      module: "roi",
      field_label: "Project Duration (Months)",
      tooltip_text:
        "How long the project will run before full benefits are realized.",
      calculation_hint:
        "Include planning, implementation, and stabilization phases.",
      data_source_hint: "Project Management or department leads.",
    },
    {
      field_id: "discount_rate",
      module: "roi",
      field_label: "Discount Rate (%)",
      tooltip_text:
        "The rate used to discount future cash flows to present value. Default is 10% per company policy.",
      calculation_hint:
        "Typically matches company cost of capital or required rate of return.",
      data_source_hint: "Finance department sets the standard rate.",
    },
    {
      field_id: "error_rate_percent",
      module: "roi",
      field_label: "Current Error Rate (%)",
      tooltip_text:
        "Percentage of transactions or processes that currently result in errors requiring rework.",
      calculation_hint:
        "Calculate: (Number of errors / Total transactions) × 100",
      data_source_hint: "Quality or Operations team tracking data.",
    },
    {
      field_id: "headcount_affected",
      module: "roi",
      field_label: "Headcount Affected",
      tooltip_text:
        "Number of employees whose work will be impacted or made more efficient by this initiative.",
      calculation_hint: "Count all FTEs directly affected by process changes.",
      data_source_hint: "HR or department managers.",
    },
    {
      field_id: "risk_score",
      module: "pmp",
      ...
      tooltip_text:
        "Calculated as Probability × Impact. Higher scores indicate risks requiring more attention.",
      calculation_hint:
        "Score = Probability (1-5) × Impact (1-5). Max score is 25.",
      data_source_hint: "Project team risk assessment workshop.",
    },
  ];
  ...
  for (const tooltip of defaultTooltips) {
    await client.query(
      ...
      [
        ...
        tooltip.calculation_hint,
        tooltip.data_source_hint,
      ],
    );
  }
  logger.info("✅ Default tooltips seeded");
}
...
export async function createOrUpdateOnboardingStatus(data: {
  ...
  if (existing) {
    ...
    if (data.video_watched !== undefined) {
      updates.push(
        `video_watched = $${idx}, video_watched_at = ${data.video_watched ? "NOW()" : "NULL"}`,
      );
    ...
    if (data.tour_completed !== undefined) {
      updates.push(
        `tour_completed = $${idx}, tour_completed_at = ${data.tour_completed ? "NOW()" : "NULL"}`,
      );
...
export async function validateDemoLink(
  linkCode: string,
...
export async function listDemoLinks(): Promise<any[]> {
  const result = await pool.query(
    "SELECT * FROM demo_links ORDER BY created_at DESC",
  );
  return result.rows;
}
...
export async function deactivateDemoLink(linkCode: string): Promise<void> {
  await pool.query(
    "UPDATE demo_links SET is_active = false WHERE link_code = $1",
    [linkCode],
  );
}
