 ...
 *
 * Decision tree:
 *   1. Flag off → skipped_reason='flag_disabled'
 *   2. No Lead/Deal → skipped_reason='no_crm_linkage'
 *   3. Empty patch → skipped_reason='no_writable_fields'
 *   4. PUT runs; on failure → skipped_reason='zoho_error' (and logged)
 */
export async function syncEvaluationToZohoStructuredFields(
  ...
  if (!isFlagEnabled("zoho_structured_fields", identity)) {
    return { synced: false, skipped_reason: "flag_disabled" };
  }

  const { lead_id, deal_id } = callRecord;
  if (!lead_id && !deal_id) {
    return { synced: false, skipped_reason: "no_crm_linkage" };
  }

  const { patch, fieldsIncluded } = buildEvaluationPatch(evaluation);
  if (fieldsIncluded.length === 0) {
    return { synced: false, skipped_reason: "no_writable_fields" };
  }
