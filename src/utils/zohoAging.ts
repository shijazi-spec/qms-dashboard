/**
 * Zoho Lead Status & Deal Stage Aging (Task #825)
 * ===============================================
 *
 * Derives "how long has this record been sitting in its current
 * Lead_Status / Stage?" from Zoho's modification history rather than the
 * record's overall `Modified_Time`.
 *
 *   - Deals: pulled from Zoho's built-in `Stage_History` related list.
 ...
 * UTC days.
 *
 * The Zoho-fetching path goes through `makeZohoRequest` in
 * `src/utils/zohoCRM.ts`, so OAuth caching, the 401-then-refresh retry,
 ...
 * can exercise the spec edge cases without a live Zoho client.
 */
...
import {
  ...
  type ZohoCRMRecord,
  type ZohoChangelogRow,
  type ZohoStageHistoryRow,
} from "./zohoCRM";
...
export function getTerminalDealStages(): string[] {
  return parseListEnv("ZOHO_TERMINAL_DEAL_STAGES", DEFAULT_TERMINAL_DEAL_STAGES);
}
export function getTerminalLeadStatuses(): string[] {
  return parseListEnv("ZOHO_TERMINAL_LEAD_STATUSES", DEFAULT_TERMINAL_LEAD_STATUSES);
}
...
export function getLeadAgingThreshold(): number {
  const n = parseInt(process.env.ZOHO_LEAD_AGING_THRESHOLD_DAYS || "", 10);
  return Number.isFinite(n) && n > 0 ? n : DEFAULT_LEAD_THRESHOLD_DAYS;
}
export function getDealAgingThreshold(): number {
  const n = parseInt(process.env.ZOHO_DEAL_AGING_THRESHOLD_DAYS || "", 10);
...
//
// Routes and unit tests inject these so we don't hit the real Zoho client.
// The default implementation just delegates to the helpers in zohoCRM.ts.
...
export interface AgingFetchers {
  ...
  listDealsPage(page: number, perPage: number): Promise<ZohoCRMRecord[]>;
  listLeadsPage(page: number, perPage: number): Promise<ZohoCRMRecord[]>;
}

async function defaultDealRecord(dealId: string) {
  const recs = await fetchAllZohoRecords("Deals", {
...
async function defaultLeadRecord(leadId: string) {
  const recs = await fetchAllZohoRecords("Leads", {
...
// places (see `cachedAccessToken` in zohoCRM.ts). This is intentionally minimal
...
async function fanoutDeals(ids: string[], fetchers: AgingFetchers): Promise<DealStageAging[]> {
  return mapWithConcurrency(ids, FANOUT_CONCURRENCY, async (id) => {
    ...
    try {
      ...
      logger.warn(`[ZohoAging] Deal ${id} failed: ${err?.message || err}`);
      return {
...
async function fanoutLeads(ids: string[], fetchers: AgingFetchers): Promise<LeadStatusAging[]> {
  return mapWithConcurrency(ids, FANOUT_CONCURRENCY, async (id) => {
    ...
    try {
      ...
      logger.warn(`[ZohoAging] Lead ${id} failed: ${err?.message || err}`);
      return {
...
//
// Caller does NOT need to supply ids. We page Zoho ourselves (sorted by
...
// remaining items by `agingDays desc`. The cursor is the next Zoho page
// number; an empty cursor means we've reached the end.

export interface ListAgingOptions {
  /** Number of records per Zoho page to scan (default 50, max 200). */
  limit?: number;
  /** Zoho page number (1-based). Use the `nextCursor` from the previous response. */
