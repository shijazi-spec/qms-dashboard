 ...
 * Digest returns { sent: false, skipped_reason: 'flag_disabled' }
...
export function renderDigestText(d: WeeklyDigest): string {
  const lines: string[] = [];
  lines.push(`WalaPlus QMS — Weekly Call Evaluation Digest`);
...
export function renderDigestHtml(d: WeeklyDigest): string {
  ...
  return `<div style="font-family:Calibri,Arial,sans-serif;color:#222;max-width:700px">
    <h2 style="color:#047857">WalaPlus QMS — Weekly Call Evaluation Digest</h2>
...
export async function sendWeeklyDigest(
  ...
  if (emailRecipients.length > 0) {
    ...
    try {
      ...
      const send = await sendResendEmail({
        to: emailRecipients,
        subject: `WalaPlus QMS — Weekly Call Eval Digest (${window.label})`,
  ...
  if (!result.sent && !result.skipped_reason) {
    result.skipped_reason = "all_channels_failed_or_unconfigured";
  }
  return result;
}
