// =======================================================================
// Raw-fetch OpenAI Chat Completions helper — bypass `@ai-sdk/openai`.
//
// Why this exists: `@ai-sdk/openai@3.x` returns model objects with
// `specVersion: "v3"` from BOTH `openai.responses(...)` AND
// `openai.chat(...)`. The runtime `generateText` shipped in `ai@5.x`
// only accepts `specVersion: "v2"`, so every call dies inside the JS
// layer with "Unsupported model version v3 for provider openai.chat
// and model …" before ever reaching OpenAI. Prior hotfix (PR #35)
...
export interface GenerateChatTextOptions {
  ...
  maxTokens?: number;
  /** Optional temperature (defaults to OpenAI's default of 1). */
  temperature?: number;
...
export interface GenerateChatTextResult {
  ...
  text: string;
  /** Raw OpenAI response body for debugging / token-usage inspection. */
  raw: any;
}
...
function chatBaseUrl(): string {
  ...
  // configured AND paired with a valid key (the openaiCredentials guard
  // handles that), else fall back to public OpenAI. Avoids the modelfarm
  // proxy + dummy-key Russian-doll bug from earlier sessions.
  return getOpenAIBaseUrl() || "https://api.openai.com/v1";
}
...
/**
 * Call OpenAI Chat Completions and return the assistant's text. Drop-in
 * replacement for `generateText({ model: openai.chat(name), ... })` —
 * same return shape, zero `@ai-sdk/openai` dependency. Throws on HTTP
 * failure with the OpenAI error body in the message so existing error
...
export async function generateChatText(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      `OpenAI /chat/completions ${res.status}: ${errBody.slice(0, 800)}`,
    );
  }
