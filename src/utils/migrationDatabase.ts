...
async function seedMigrationTemplates(): Promise<void> {
  ...
  const templates = [
    ...
    // ----------------------------------------------------------------------
    {
      ...
      target_module: 'capa',
      description: 'Import Corrective & Preventive Actions from existing CAPA logs (WP-SOP-009).',
      field_mapping: JSON.stringify({
        required: ['capa_code', 'title', 'root_cause', 'due_date'],
        optional: ['description', 'owner_email', 'status', 'severity', 'related_finding_code'],
        mappings: {
          'CAPA ID':           'capa_code',
          'CAPA Code':         'capa_code',
          'Action ID':         'capa_code',
    ...
    },
    {
      ...
      description: 'Import historic nonconformities from paper / Excel logs (WP-SOP-009, ISO 9001 §10.2).',
      field_mapping: JSON.stringify({
        required: ['nc_code', 'title', 'severity', 'detected_at'],
        optional: ['description', 'source', 'process', 'reporter_email', 'status'],
        mappings: {
          'NC ID':            'nc_code',
          'NC Code':          'nc_code',
          'Nonconformity':    'title',
    ...
    },
    {
      ...
      description: 'Import training & competency records for auditor qualification (ISO 19011:2018 §7).',
      field_mapping: JSON.stringify({
        required: ['employee_email', 'training_name', 'completed_at'],
        optional: ['provider', 'hours', 'certificate_ref', 'expires_at', 'role_scope'],
        mappings: {
    ...
    },
    {
      ...
      description: 'Import historical audit findings from past internal / external audits (WP-SOP-042).',
      field_mapping: JSON.stringify({
        required: ['finding_code', 'title', 'severity', 'audit_reference'],
        optional: ['description', 'category', 'control_reference', 'responsible_party', 'status', 'detected_at'],
        mappings: {
    ...
    },
    {
      ...
      description: 'Import historical deal / sales evaluation scores for quality trend analysis.',
      field_mapping: JSON.stringify({
