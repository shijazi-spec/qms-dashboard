/**
 * Google Drive call-source helper.
 *
 * Auth resolution order (first available wins):
 *   1. Replit Connector (REPLIT_CONNECTORS_HOSTNAME + repl identity)  — same pattern as googleCalendar.ts
 *   2. Service Account JWT (GOOGLE_DRIVE_CLIENT_EMAIL + GOOGLE_DRIVE_PRIVATE_KEY)
 *   3. OAuth refresh token (GOOGLE_OAUTH_CLIENT_ID + GOOGLE_OAUTH_CLIENT_SECRET + GOOGLE_OAUTH_REFRESH_TOKEN)
 *
...
export type DriveAuthMode = "replit_connector" | "service_account" | "oauth_refresh" | "none";
...
async function getReplitConnectorToken(): Promise<string | null> {
...
export async function resolveDriveAuth(): Promise<DriveAuthResult> {
  ...
  const attempts: Array<{ mode: DriveAuthMode; fn: () => Promise<string | null> }> = [
    { mode: "replit_connector", fn: getReplitConnectorToken },
...
export async function listDriveAudioFiles(opts: {
  ...
  const folderId = opts.folder_id || process.env.GOOGLE_DRIVE_CALLS_FOLDER_ID;
