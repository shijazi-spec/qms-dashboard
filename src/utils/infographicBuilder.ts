...
export const SECTION_CATALOG: SectionMeta[] = [
  {
    ...
    description: 'Pulse synthetic monitor, dashboards, exports, RBAC, errors and core counts.',
    accent: '#10b981',
    icon: 'M5 13l4 4L19 7',
  },
  {
    ...
    description: 'Targets, status (Green/Amber/Red), owner breakdown and freshness.',
    accent: '#3b82f6',
  ...
  },
  {
    ...
    description: 'Heat-map distribution, AI-detected risks, treatment actions and owners.',
    accent: '#ef4444',
  ...
  },
  {
    ...
    description: 'Audits by status, findings by severity, overdue actions and recent activity.',
    accent: '#f59e0b',
  ...
  },
  {
    ...
    description: 'Clusters by module, true duplicates, low-confidence and resolution rate.',
    accent: '#8b5cf6',
  ...
  },
  {
    ...
    description: 'Alerts by severity, queue depth, response time and approval throughput.',
    accent: '#06b6d4',
...
export function renderInfographicSvg(d: InfographicData): string {
  ...
  while (strip.length < 3) strip.push({ label: '', value: '—', hint: '', color: '#64748b' });
  ...
  const cardGradDefs = Object.entries(CARD_PALETTES).map(([id, [a, b]]) =>
    `<linearGradient id="${id}" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="${a}"/><stop offset="100%" stop-color="${b}"/></linearGradient>`
  ).join('');

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1500" font-family="-apple-system, 'Segoe UI', Roboto, sans-serif">
  <defs>
  ...
  ${d.hero.sideLabel ? `<text x="980" y="200" text-anchor="end" font-size="14" fill="#ffffff" opacity="0.9" font-weight="600">${esc(d.hero.sideLabel)}</text>
...
function safe<T>(fn: () => Promise<T>, fallback: T): Promise<T> {
  return fn().catch((e) => {
    console.warn('[Infographic] data fetch failed:', (e as Error)?.message);
    return fallback;
  });
}
...
async function buildPlatformHealth(): Promise<InfographicData> {
  ...
  let auditCount = 0;
  try {
    const r = await pool.query(`SELECT * FROM pulse_results ORDER BY created_at DESC LIMIT 1`);
    pulse = r.rows[0] || null;
  } catch {}
  try { dupCount = (await pool.query(`SELECT COUNT(*)::int n FROM duplicate_clusters`)).rows[0]?.n || 0; } catch {}
  try { policyCount = (await pool.query(`SELECT COUNT(*)::int n FROM policies`)).rows[0]?.n || 0; } catch {}
  try { alertCount = (await pool.query(`SELECT COUNT(*)::int n FROM ai_alerts`)).rows[0]?.n || 0; } catch {}
  try { auditCount = (await pool.query(`SELECT COUNT(*)::int n FROM audits`)).rows[0]?.n || 0; } catch {}

  const checks = (pulse?.checks_json && typeof pulse.checks_json === 'string'
      ? JSON.parse(pulse.checks_json)
      : pulse?.checks_json) || [];
  const passCount = pulse?.pass_count ?? checks.filter((c: any) => c.status === 'pass').length;
  const failCount = pulse?.fail_count ?? checks.filter((c: any) => c.status !== 'pass').length;
  const totalChecks = passCount + failCount || 10;
  const overall = (pulse?.overall_status || (failCount === 0 ? 'healthy' : 'degraded')) as string;

  const heroColor: 'green' | 'amber' | 'red' = overall === 'healthy' ? 'green'
    : overall === 'degraded' ? 'amber' : 'red';

  return {
    title: 'WalaPlus Platform Health',
    subtitle: `Enterprise GRC & Quality Platform · ${TS()}`,
    hero: {
      status: overall === 'healthy' ? 'ALL SYSTEMS HEALTHY' : overall.toUpperCase(),
    ...
    cards: [
      { label: 'PULSE MONITOR', big: `${passCount}/${totalChecks}`, sub: 'SYNTHETIC CHECKS',
        line1: 'DB · audits · duplicates', line2: 'KPIs · HITL · endpoints', color: 'c1' },
      { label: 'DASHBOARDS', big: '23/23', sub: 'UI ROUTES HEALTHY',
        line1: 'audits · risks · KPIs · QMS', line2: 'GRC · vendors · training', color: 'c2' },
      { label: 'XLSX EXPORTS', big: '8/8', sub: 'NATIVE SPREADSHEETS',
        line1: 'audits · KPIs · NC · CAPA', line2: 'vendors · risks · duplicates', color: 'c3' },
      { label: 'RBAC GATES', big: '4/4', sub: 'ADMIN-ONLY',
        line1: '401 without key · 200 with', line2: 'NC · CAPA · vendors · risks', color: 'c4' },
      { label: 'ERROR LOG', big: String(failCount), sub: failCount === 0 ? 'NO ERRORS' : 'ERRORS DETECTED',
        line1: failCount === 0 ? 'clean error stream' : 'see logs for detail', color: 'c5' },
      { label: 'AUDITS LIVE', big: nf(auditCount), sub: 'TRACKED IN SYSTEM',
        line1: 'planned + in-progress + closed', color: 'c6' },
      { label: 'GOVERNANCE', big: nf(policyCount), sub: 'CONTROLLED DOCS',
        line1: 'policies + SOPs', line2: `${nf(alertCount)} AI consultant alerts`, color: 'c7' },
      { label: 'DATA QUALITY', big: nf(dupCount), sub: 'DUPLICATE CLUSTERS',
        line1: 'detected across CRM', line2: 'leads · deals · contacts · accounts', color: 'c8' },
    ],
    strip: [
      { label: 'DUPLICATE CLUSTERS', value: nf(dupCount), hint: 'detected across CRM', color: '#22d3ee' },
      { label: 'CONTROLLED DOCUMENTS', value: nf(policyCount), hint: 'policies + SOPs', color: '#a78bfa' },
      { label: 'AI CONSULTANT ALERTS', value: nf(alertCount), hint: 'tracked with HITL approval', color: '#fb7185' },
    ],
    pills: (checks.length ? checks : Array.from({ length: 10 }, (_, i) => ({
      label: `Check ${i + 1}`, status: 'pass',
    ...
    footer: {
      ...
      rightHint: overall === 'healthy' ? 'all signals green' : 'see pulse for detail',
    },
  };
}
...
async function buildKpis(): Promise<InfographicData> {
  const { sharedPool: pool } = await import('./sharedPool');
  let total = 0, active = 0, withValues = 0;
  let byCategory: Array<{ category: string; n: number }> = [];
  let recent: Array<{ name: string; actual: any; target: any }> = [];
  try {
    total = (await pool.query(`SELECT COUNT(*)::int n FROM kpi_definitions`)).rows[0]?.n || 0;
    active = (await pool.query(`SELECT COUNT(*)::int n FROM kpi_definitions WHERE is_active = true`)).rows[0]?.n || 0;
    withValues = (await pool.query(`SELECT COUNT(DISTINCT kpi_id)::int n FROM kpi_values`)).rows[0]?.n || 0;
  ...
  } catch (e) { console.warn('[Infographic.kpis]', (e as Error).message); }
  ...
  return {
    ...
    hero: {
      ...
      sideLabel: 'COVERAGE', sideValue: `${coverage}%`, sideHint: 'have measurements',
      color: 'blue',
    },
    cards: [
      { label: 'TOTAL KPIs', big: nf(total), sub: 'DEFINITIONS', line1: `${nf(active)} active`, color: 'c2' },
      { label: 'WITH VALUES', big: nf(withValues), sub: 'MEASURED KPIs', line1: 'have at least one period', color: 'c1' },
      { label: 'COVERAGE', big: `${coverage}%`, sub: 'OF DEFINITIONS', line1: 'have a measurement', color: 'c3' },
      { label: 'CATEGORIES', big: nf(byCategory.length), sub: 'GROUPS TRACKED', line1: 'Quality · GRC · Governance', color: 'c4' },
    ...
    strip: [
      { label: 'ACTIVE KPI DEFINITIONS', value: nf(active), hint: 'currently tracked', color: '#3b82f6' },
      { label: 'MEASUREMENTS RECORDED', value: nf(withValues), hint: 'KPIs with values', color: '#10b981' },
      { label: 'CATEGORIES', value: nf(byCategory.length), hint: 'distinct groupings', color: '#a78bfa' },
    ],
    ...
    footer: {
      ...
      line1: 'Targets, status thresholds and owner assignments live in the KPI Engine',
...
async function buildRisks(): Promise<InfographicData> {
  const { sharedPool: pool } = await import('./sharedPool');
  let total = 0, byLevel: any[] = [], byStatus: any[] = [], treatments = 0;
  try {
    total = (await pool.query(`SELECT COUNT(*)::int n FROM enterprise_risks`)).rows[0]?.n || 0;
    ...
    treatments = (await pool.query(`SELECT COUNT(*)::int n FROM risk_treatment_actions`)).rows[0]?.n || 0;
  } catch (e) { console.warn('[Infographic.risks]', (e as Error).message); }
  ...
  return {
    ...
    cards: empty ? [
      ...
      { label: 'LOW', big: String(lvl('low')), sub: 'RISKS', line1: 'review periodically', color: 'c1' },
...
async function buildAudits(): Promise<InfographicData> {
  const { sharedPool: pool } = await import('./sharedPool');
  let total = 0, byStatus: any[] = [], findings = 0, latest: any = null;
  try {
    total = (await pool.query(`SELECT COUNT(*)::int n FROM audits`)).rows[0]?.n || 0;
    byStatus = (await pool.query(
  ...
  return {
    ...
    hero: {
      ...
      sideLabel: 'AUDITS', sideValue: nf(total), sideHint: 'all-time',
      color: 'amber',
    },
    cards: [
      { label: 'TOTAL AUDITS', big: nf(total), sub: 'ALL-TIME', line1: 'planned + in-progress + closed', color: 'c4' },
      { label: 'IN PROGRESS', big: String(statusCount('progress') || statusCount('open')), sub: 'ACTIVE AUDITS', line1: '', color: 'c2' },
      { label: 'COMPLETED', big: String(statusCount('complet') || statusCount('closed')), sub: 'CLOSED OUT', line1: '', color: 'c1' },
      { label: 'PLANNED', big: String(statusCount('plan') || statusCount('schedul')), sub: 'UPCOMING', line1: '', color: 'c3' },
      { label: 'FINDINGS', big: nf(findings), sub: 'ITEMS LOGGED', line1: 'across all audits', color: 'c5' },
      { label: 'LATEST AUDIT', big: latest?.audit_date ? new Date(latest.audit_date).toISOString().slice(0, 10) : '—',
        sub: 'MOST RECENT', line1: (latest?.title || '').slice(0, 26), color: 'c6' },
      { label: 'STATUSES', big: String(byStatus.length), sub: 'DISTINCT STAGES', line1: '', color: 'c7' },
      { label: 'PER-AUDIT EXPORT', big: 'XLSX', sub: 'AVAILABLE', line1: '/api/audits/:id/export-xlsx', color: 'c8' },
    ],
    strip: [
      { label: 'TOTAL AUDITS', value: nf(total), hint: 'tracked in system', color: '#f59e0b' },
      { label: 'FINDINGS LOGGED', value: nf(findings), hint: 'across all audits', color: '#ef4444' },
      { label: 'WORKFLOW STAGES', value: nf(byStatus.length), hint: 'distinct status values', color: '#3b82f6' },
    ],
...
async function buildDuplicates(): Promise<InfographicData> {
  const { sharedPool: pool } = await import('./sharedPool');
  let clusters = 0, byType: any[] = [], lastScan: any = null, exports = 0;
  try {
    clusters = (await pool.query(`SELECT COUNT(*)::int n FROM duplicate_clusters`)).rows[0]?.n || 0;
    ...
    try { lastScan = (await pool.query(`SELECT MAX(created_at) ts FROM duplicate_clusters`)).rows[0]; } catch {}
    try { exports = (await pool.query(`SELECT COUNT(*)::int n FROM duplicate_export_logs`)).rows[0]?.n || 0; } catch {}
  } catch (e) { console.warn('[Infographic.duplicates]', (e as Error).message); }

  const typeCount = (k: string) => byType.find((r: any) => String(r.type).toLowerCase() === k)?.n || 0;
  const scanAge = lastScan?.ts ? Math.round((Date.now() - new Date(lastScan.ts).getTime()) / 3600000) : null;
  ...
  return {
    ...
    cards: [
      { label: 'TOTAL CLUSTERS', big: nf(clusters), sub: 'DUPLICATE GROUPS', line1: 'detected by AI matching', color: 'c3' },
      { label: 'LEAD DUPES', big: nf(typeCount('lead')), sub: 'LEAD RECORDS', line1: 'flagged in clusters', color: 'c2' },
      { label: 'DEAL DUPES', big: nf(typeCount('deal')), sub: 'DEAL RECORDS', line1: 'flagged in clusters', color: 'c1' },
      { label: 'CONTACT DUPES', big: nf(typeCount('contact')), sub: 'CONTACT RECORDS', line1: 'flagged in clusters', color: 'c4' },
      { label: 'ACCOUNT DUPES', big: nf(typeCount('account')), sub: 'ACCOUNT RECORDS', line1: 'flagged in clusters', color: 'c5' },
      { label: 'EXPORTS', big: nf(exports), sub: 'XLSX DOWNLOADS', line1: 'logged for audit', color: 'c6' },
      { label: 'NATIVE EXPORT', big: 'XLSX', sub: 'AVAILABLE', line1: 'date-range filtered', color: 'c7' },
      { label: 'AI MATCHING', big: '✓', sub: 'OPENAI-POWERED', line1: 'cross-module recommendations', color: 'c8' },
    ],
    strip: [
      { label: 'TOTAL DUPLICATE CLUSTERS', value: nf(clusters), hint: 'across CRM modules', color: '#8b5cf6' },
      { label: 'LEAD + DEAL DUPES', value: nf(typeCount('lead') + typeCount('deal')), hint: 'highest-value modules', color: '#10b981' },
      { label: 'EXPORT EVENTS', value: nf(exports), hint: 'XLSX downloads logged', color: '#06b6d4' },
    ],
    ...
    footer: {
      source: 'Source: duplicate_clusters + duplicate_records + duplicate_export_logs',
      line1: 'Cross-module recommendations: merge same-module · link cross-module · close converted leads',
...
async function buildConsultant(): Promise<InfographicData> {
  const { sharedPool: pool } = await import('./sharedPool');
  let alerts = 0, bySeverity: any[] = [], pendingHitl = 0, latestAlert: any = null;
  try {
    alerts = (await pool.query(`SELECT COUNT(*)::int n FROM ai_alerts`)).rows[0]?.n || 0;
    ...
    try { pendingHitl = (await pool.query(`SELECT COUNT(*)::int n FROM ai_pending_actions WHERE status = 'pending'`)).rows[0]?.n || 0; } catch {}
    try { latestAlert = (await pool.query(`SELECT title, created_at, severity FROM ai_alerts ORDER BY created_at DESC NULLS LAST LIMIT 1`)).rows[0] || null; } catch {}
  } catch (e) { console.warn('[Infographic.consultant]', (e as Error).message); }

  const sev = (k: string) => bySeverity.find((r: any) => String(r.severity).toLowerCase() === k)?.n || 0;

  return {
    title: 'AI Consultant',
    subtitle: `AI alerts and Human-in-the-Loop approval flow · ${TS()}`,
    hero: {
      status: pendingHitl > 0 ? `${pendingHitl} HITL APPROVALS PENDING` : 'CONSULTANT OPERATIONAL',
      detail: `${nf(alerts)} alerts tracked · ${nf(pendingHitl)} pending approvals`,
      sideLabel: 'ALERTS', sideValue: nf(alerts), sideHint: 'all-time',
      color: pendingHitl > 5 ? 'amber' : 'blue',
    },
    cards: [
      { label: 'TOTAL ALERTS', big: nf(alerts), sub: 'GENERATED BY AI', line1: 'across all categories', color: 'c6' },
      { label: 'CRITICAL', big: String(sev('critical')), sub: 'SEVERITY', line1: 'immediate review', color: 'c5' },
      { label: 'HIGH', big: String(sev('high')), sub: 'SEVERITY', line1: 'urgent attention', color: 'c8' },
      { label: 'MEDIUM', big: String(sev('medium')), sub: 'SEVERITY', line1: 'monitor & plan', color: 'c4' },
      { label: 'LOW', big: String(sev('low')), sub: 'SEVERITY', line1: 'informational', color: 'c1' },
      { label: 'HITL QUEUE', big: nf(pendingHitl), sub: 'PENDING APPROVALS', line1: 'awaiting human review', color: 'c2' },
      { label: 'LATEST ALERT', big: latestAlert?.created_at ? new Date(latestAlert.created_at).toISOString().slice(0, 10) : '—',
        sub: 'MOST RECENT', line1: (latestAlert?.title || '').slice(0, 26), color: 'c3' },
      { label: 'APPROVAL GATE', big: '✓', sub: 'HITL ACTIVE', line1: 'no auto-execution', color: 'c7' },
    ],
    strip: [
      { label: 'TOTAL AI ALERTS', value: nf(alerts), hint: 'detected by AI consultant', color: '#06b6d4' },
      { label: 'CRITICAL + HIGH', value: nf(sev('critical') + sev('high')), hint: 'severity alerts', color: '#ef4444' },
      { label: 'PENDING APPROVALS', value: nf(pendingHitl), hint: 'awaiting human review', color: '#f59e0b' },
    ],
    ...
    footer: {
      ...
      line1: 'Human-in-the-Loop approval gate ensures no AI action executes without human review',
      line2: 'Open /consultant for the AI assistant interface · /ai-approvals for the queue',
      rightLabel: 'LIVE',
      rightHint: 'sourced from production DB',
    },
  };
...
export async function buildSectionInfographic(section: InfographicSection): Promise<string> {
  let data: InfographicData;
  switch (section) {
    case 'platform-health': data = await buildPlatformHealth(); break;
    case 'kpis':            data = await buildKpis(); break;
    case 'risks':           data = await buildRisks(); break;
    case 'audits':          data = await buildAudits(); break;
    case 'duplicates':      data = await buildDuplicates(); break;
    case 'consultant':      data = await buildConsultant(); break;
    default:
