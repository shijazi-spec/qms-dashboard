...
async function checkMissedKPIs(result: ScanResult): Promise<void> {
  const rows = await safeQuery(`
    SELECT kd.id, kd.kpi_name AS name, kd.target_value AS target, kv.actual_value AS value, kv.period_end
    FROM kpi_definitions kd
    JOIN LATERAL (
      SELECT actual_value, period_end FROM kpi_values WHERE kpi_id = kd.id ORDER BY period_end DESC LIMIT 1
    ) kv ON true
    WHERE kv.actual_value < kd.target_value
  `);
  result.checksPerformed++;
