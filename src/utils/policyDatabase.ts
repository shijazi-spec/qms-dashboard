...
export async function initPolicyTables(): Promise<void> {
  console.log('📋 [PolicyDB] Initializing policy governance tables...');
...
export async function createPolicy(policy: Policy): Promise<Policy> {
  console.log('📝 [PolicyDB] Creating new policy:', policy.title);
  ...
  `, [result.rows[0].id, policy.version || '1.0', policy.content_text, policy.file_path, 'Initial version', policy.created_by || 'System', policy.status || 'draft']);

  console.log('✅ [PolicyDB] Policy created with ID:', result.rows[0].id);
  return result.rows[0];
}

export async function updatePolicy(id: number, policy: Partial<Policy>, updatedBy: string): Promise<Policy> {
  console.log('📝 [PolicyDB] Updating policy ID:', id);
  ...
  if (policy.version && policy.version !== existing.version) {
    await pool.query(`
      INSERT INTO policy_versions (policy_id, version, content_text, file_path, change_summary, changed_by, status_at_version)
      VALUES ($1, $2, $3, $4, $5, $6, $7)
    `, [id, policy.version, policy.content_text || existing.content_text, policy.file_path || existing.file_path, policy.change_summary || 'Version update', updatedBy, policy.status || existing.status]);
  }

  console.log('✅ [PolicyDB] Policy updated:', id);
  return result.rows[0];
}
...
export async function getAllPolicies(filters?: {
  ...
  console.log('📊 [PolicyDB] Fetching policies with filters:', filters);
  ...
  const countResult = await pool.query(`SELECT COUNT(*) FROM policies ${whereClause}`, values);
  ...
  console.log('✅ [PolicyDB] Found', result.rows.length, 'policies');
  return { policies: result.rows, total };
}

export async function getPolicySummaryStats(): Promise<any> {
  console.log('📊 [PolicyDB] Generating policy summary statistics...');
  ...
  console.log('✅ [PolicyDB] Summary statistics generated');
  return {
    ...stats.rows[0],
...
export async function getPolicyVersions(policyId: number): Promise<PolicyVersion[]> {
  const result = await pool.query(`
...
export async function acknowledgePolicy(ack: PolicyAcknowledgment): Promise<PolicyAcknowledgment> {
  console.log('📝 [PolicyDB] Recording policy acknowledgment for policy:', ack.policy_id);
  ...
  const result = await pool.query(`
    ...
    RETURNING *
  `, [ack.policy_id, ack.user_id, ack.user_name, ack.user_email, ack.department, ack.acknowledgment_method || 'electronic', expiresAt]);

  console.log('✅ [PolicyDB] Acknowledgment recorded');
  return result.rows[0];
}

export async function getPolicyAcknowledgments(policyId: number): Promise<PolicyAcknowledgment[]> {
  const result = await pool.query(`
...
export async function transitionPolicyStatus(id: number, newStatus: string, transitionedBy: string): Promise<Policy> {
  console.log('📝 [PolicyDB] Transitioning policy', id, 'to status:', newStatus);
...
export async function getOverduePolicies(): Promise<Policy[]> {
  console.log('📊 [PolicyDB] Fetching overdue policies...');
  ...
  console.log('✅ [PolicyDB] Found', result.rows.length, 'overdue policies');
  return result.rows;
}

export async function getPendingAcknowledgments(department?: string): Promise<any[]> {
  console.log('📊 [PolicyDB] Fetching policies pending acknowledgment...');
