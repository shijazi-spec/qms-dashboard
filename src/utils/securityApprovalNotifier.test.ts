...
async function testSlackSentOnSuccess(): Promise<void> {
  ...
  _resetSecurityApprovalNotifierThrottleForTests();
  process.env.SECURITY_REVIEWER_SLACK_CHANNEL = "C-SECURITY";
  process.env.SECURITY_REVIEWER_APP_URL = "https://wala.example.com";
  ...
  assert(blockText.includes("alice@example.com"), "blocks include requester email");
...
async function testEmailSentOnSuccess(): Promise<void> {
  ...
  clearEnv();
  _resetSecurityApprovalNotifierThrottleForTests();
  process.env.SECURITY_REVIEWER_EMAIL = "soc@example.com, secops@example.com";
  process.env.SECURITY_REVIEWER_APP_URL = "https://wala.example.com";
  ...
  assert(
    Array.isArray(emailCalls[0].to) &&
      ...
      (emailCalls[0].to as string[])[0] === "soc@example.com" &&
      (emailCalls[0].to as string[])[1] === "secops@example.com",
  ...
  assert(
    !!emailCalls[0].html && emailCalls[0].html!.includes("https://wala.example.com/ai-approvals?code="),
