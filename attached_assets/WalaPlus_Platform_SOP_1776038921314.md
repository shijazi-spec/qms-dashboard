...
| 4.1 | Apr 8, 2026 | Engineering | **GRC Maturity Enhancement.** Fixed 8 SOP inconsistencies (audit pagination 100→50K, risk scale 1-4→1-5, clarified scanner thresholds). **Duplicate Radar async scan fix:** scan-zoho now returns immediately and runs in background; new `/api/duplicates/scan-status` endpoint for live progress polling (fixes Gateway Timeout); default limit reduced to 5K/module (configurable via `DUPLICATE_SCAN_LIMIT`). Added Management Review module (Section 4.26, ISO 9001 §9.3: meetings, decisions, action items, auto-gather QMS inputs). Added Analytics Engine (Section 4.27: process cycle times with avg/median/P90, per-agent compliance reports, CAPA recurrence detection, trend analysis). Added Executive Quality Digest (Section 4.28/8.7: weekly email via Inngest cron). AI Scanner expanded from 12→14 checks (added auto-NC creation from critical SLA breaches, CAPA recurrence detection). New dashboard page: `/reviews`. 3 new backend files, 2 new route files. Database expanded to 105+ tables. |
