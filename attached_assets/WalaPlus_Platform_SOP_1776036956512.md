...
- **Background Scanning:** Scan runs server-side; results returned on completion
...
2. Button shows spinner while scan executes server-side
...
7. On completion, results are returned and dashboard refreshes automatically
...
| `/api/duplicates/scan-zoho` | POST | Start Zoho scan (requires admin, returns on completion) |
...
| Duplicate scan shows "Gateway Timeout" | Scan runs server-side and may take several minutes for large datasets; wait for the response or check `/api/duplicates/logs` for scan history |
