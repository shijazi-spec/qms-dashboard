  ...
  return makeZohoRequest(
    ...
    async (response) => {
      if (response.status === 429) {
        throw new Error(`Zoho CRM API error: 429 - Rate limit exceeded`);
      }
      if (!response.ok) {
...
const ZOHO_PAGE_DELAY_MS = parseInt(process.env.ZOHO_PAGE_DELAY_MS || '150');
const ZOHO_429_BASE_DELAY_MS = 5000;
...
export async function fetchAllZohoRecords(
  ...
  params: {
    ...
    sortOrder?: 'asc' | 'desc';
    maxRecords?: number;
    onProgress?: (fetched: number, maxRecords: number) => void;
  } = {}
): Promise<ZohoCRMRecord[]> {
  ...
  console.log(`[ZohoCRM] Fetching all ${module} records (max: ${maxRecords === Infinity ? 'unlimited' : maxRecords})...`);
  ...
  while (hasMore && allRecords.length < maxRecords) {
    try {
      ...
      if (records.length === 0) {
        ...
        if (page % 10 === 0 || records.length < perPage) {
          console.log(`[ZohoCRM] ${module}: page ${page}, ${allRecords.length} records so far`);
        }
        params.onProgress?.(allRecords.length, maxRecords === Infinity ? allRecords.length + perPage : maxRecords);
        
        if (records.length < perPage) {
          hasMore = false;
        } else {
          page++;
          if (ZOHO_PAGE_DELAY_MS > 0) await sleep(ZOHO_PAGE_DELAY_MS);
        }
      }
    } catch (error: any) {
      if (error.message?.includes('204') || error.message?.includes('No Content')) {
        ...
        retryCount++;
        const delay = ZOHO_429_BASE_DELAY_MS * retryCount;
        console.warn(`[ZohoCRM] ${module}: rate limited (429), retrying in ${delay}ms (attempt ${retryCount}/3)`);
        await sleep(delay);
      } else {
        throw error;
      }
  ...
  console.log(`[ZohoCRM] ${module}: complete — ${allRecords.length} records fetched in ${page} pages`);
  return allRecords;
}
...
/**
 * Fetch records that were deleted in Zoho CRM since a given time.
 *
 * Zoho endpoint: GET /crm/v2/{module}/deleted?type=all
 *   - type=all covers recycle-bin (user-deleted), permanently deleted, and MERGED records.
 *   - If `modifiedSince` is provided, the If-Modified-Since header returns only
 *     deletions that happened after that ISO8601 timestamp.
 *
 * Returns an array of { id, type, deletedTime, mergedIntoId? } — where `mergedIntoId`
 * is the surviving record when `type === 'merged'`. This is how we detect that a
 * user merged duplicates directly inside Zoho CRM.
 */
export async function fetchDeletedZohoRecords(
  module: string,
  options: { modifiedSince?: string; type?: 'all' | 'recycle' | 'permanent' } = {}
): Promise<Array<{ id: string; type: string; deletedTime?: string; mergedIntoId?: string; displayName?: string }>> {
  const type = options.type || 'all';
  const allDeleted: Array<{ id: string; type: string; deletedTime?: string; mergedIntoId?: string; displayName?: string }> = [];
  const perPage = 200;
  let page = 1;
  let hasMore = true;
  ...
  while (hasMore) {
    ...
    const batch = await makeZohoRequest(
      async (config) => {
      ...
      async (response) => {
        if (response.status === 204 || response.status === 304) return { records: [], hasMore: false };
        if (!response.ok) {
          const error = await response.json().catch(() => ({}));
          throw new Error(`Zoho deleted-records error: ${response.status} - ${error.message || response.statusText}`);
        }
        const data = await response.json();
        const records = (data.data || []).map((r: any) => ({
          ...
          mergedIntoId: r.merged_into?.id ? String(r.merged_into.id) : undefined,
          displayName: r.display_name,
        }));
        const more = data.info?.more_records === true;
        return { records, hasMore: more };
      }
    );

    allDeleted.push(...batch.records);
    hasMore = batch.hasMore && batch.records.length === perPage;
    if (hasMore) {
      page++;
      if (ZOHO_PAGE_DELAY_MS > 0) await sleep(ZOHO_PAGE_DELAY_MS);
    }
  ...
  console.log(`[ZohoCRM] ${module}: ${allDeleted.length} deleted records since ${options.modifiedSince || 'beginning'}`);
  return allDeleted;
}
