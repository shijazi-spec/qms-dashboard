    ...
    createHandler: async () => {
      return async (c: any) => {
        try {
          ...
          const { recordCAPAEffectiveness, createCapaRecord } = await import("../../utils/qmsDatabase");
          ...
          if (body.result === 'not_effective') {
            try {
              const reCapa = await createCapaRecord({
                title: `Re-CAPA: ${capa.title} (effectiveness failed)`,
                description: `Auto-generated re-CAPA because CAPA #${capa.capa_number} was found not effective. Original evidence: ${body.evidence || 'N/A'}`,
                capa_type: capa.capa_type || 'corrective',
                severity: capa.severity || 'major',
                source: 'effectiveness_review',
                source_id: String(id),
                assigned_to: capa.assigned_to,
              });
              reCapaId = reCapa?.id || null;
              const { logEvent } = await import("../../utils/eventLogsDatabase");
              await logEvent({ actionType: 'CREATE', entityType: 'CAPA', entityId: String(reCapaId), description: `Auto re-CAPA created from ineffective CAPA #${capa.capa_number}`, module: 'qms', severity: 'WARNING' });
              const { notifyEvent } = await import("../../utils/notificationHub");
              await notifyEvent('CAPA', `Re-CAPA required: ${capa.capa_number}`, `CAPA #${capa.capa_number} effectiveness was "not effective". A new CAPA has been auto-created for re-investigation.`, 'qms', 'quality_manager');
            } catch {}
          }
