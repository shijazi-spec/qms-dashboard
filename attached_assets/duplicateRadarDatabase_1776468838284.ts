...
/**
 * Mark records as stale when the caller has a COMPLETE list of live Zoho IDs.
 *
 * WARNING: Only call this after a FULL sync (every live record was fetched).
 * Using this with an incremental sync would wipe out every record not modified
...
/**
 * Remove specific records from the duplicate DB by their Zoho IDs.
 * Used after an incremental sync to reflect records that were merged or deleted
 * in Zoho CRM (detected via the /deleted endpoint).
 *
 * For merged records we purge rather than mark stale, so they disappear from the
 * duplicate radar (the surviving record is already present or will be re-synced).
 * Returns { removed, moduleBreakdown }.
 */
export async function removeRecordsByZohoIds(
  zohoIds: string[],
  options: { module?: string } = {}
): Promise<{ removed: number; affectedClusters: number[] }> {
  if (zohoIds.length === 0) return { removed: 0, affectedClusters: [] };

  const clusterRes = await pool.query<{ cluster_id: number }>(
    `SELECT DISTINCT cluster_id FROM duplicate_records
      WHERE zoho_record_id = ANY($1) AND cluster_id IS NOT NULL
        ${options.module ? 'AND record_type = $2' : ''}`,
    options.module ? [zohoIds, options.module.toLowerCase()] : [zohoIds]
  );
  const affectedClusters = clusterRes.rows.map(r => r.cluster_id);

  const delRes = await pool.query(
    `DELETE FROM duplicate_records
      WHERE zoho_record_id = ANY($1) AND is_mock_data = false
        ${options.module ? 'AND record_type = $2' : ''}`,
    options.module ? [zohoIds, options.module.toLowerCase()] : [zohoIds]
  );

  return { removed: delRes.rowCount || 0, affectedClusters };
}
...
// Module priority for picking the primary record in a cross-module cluster.
// In Zoho CRM, Contacts/Deals are child records of Accounts (via Account_Name),
// so Accounts are always the preferred primary when present.
function getModulePriority(recordType: string | null | undefined): number {
...
// Zoho does not allow cross-module merges. A Contact cannot be "merged" into an
// Account — it must be LINKED via the Account_Name field. This helper returns
// the correct action + reason based on the primary's module and the record's module.
function getCrossModuleAction(
  ...
  if (p === r) {
    return { action: 'MERGE', reason: `Merge into primary ${p} (same module — Zoho native merge)` };
  }

  if (p === 'account') {
    if (r === 'contact') return { action: 'LINK',  reason: 'Link contact to primary account (set Account Name field on contact)' };
    if (r === 'deal')    return { action: 'LINK',  reason: 'Link deal to primary account (set Account Name field on deal)' };
    if (r === 'lead')    return { action: 'CLOSE', reason: 'Close/convert lead — an account already exists for this company' };
    if (r === 'task')    return { action: 'LINK',  reason: 'Re-parent task to primary account (set Related To / What Id)' };
  }
  if (p === 'contact') {
    if (r === 'lead') return { action: 'CLOSE', reason: 'Close/convert lead — a contact already exists for this person' };
    if (r === 'deal') return { action: 'LINK',  reason: 'Link deal to primary contact (set Contact Name field on deal)' };
    if (r === 'task') return { action: 'LINK',  reason: 'Re-parent task to primary contact (set Related To / Who Id)' };
  }
  if (p === 'deal') {
    if (r === 'lead') return { action: 'CLOSE', reason: 'Close/convert lead — a deal already exists for this company' };
    if (r === 'task') return { action: 'LINK',  reason: 'Re-parent task to primary deal (set Related To / What Id)' };
  }

  return { action: 'REVIEW', reason: `Manual review — a ${r} cannot be natively merged with a ${p} in Zoho CRM` };
}
...
export function generateSmartRecommendation(records: DuplicateRecord[]): {
  ...
  if (records.length === 0) {
    return { primary_id: null, primary_type: null, is_cross_module: false, recommendations: [], summary: 'No records to analyze' };
  }
  ...
  const scored = records.map(r => ({
    ...
    modulePriority: getModulePriority(r.record_type),
  }));

  // Primary selection: module priority first (Account > Contact > Deal > Lead > Task),
  // then active-deal signal, then completeness, then recency, then age.
  scored.sort((a, b) => {
    if (a.modulePriority !== b.modulePriority) return a.modulePriority - b.modulePriority;
  ...
  const best = scored[0];
  const distinctTypes = new Set(records.map(r => (r.record_type || '').toLowerCase()));
  const isCrossModule = distinctTypes.size > 1;
  ...
  const recs = scored.map((r, i) => {
    if (i === 0) {
      return {
        ...
        action: 'KEEP',
        reason: `Primary ${r.record_type} — ${r.completeness}% complete${r.hasActiveDeal ? ', active deal' : ''}`,
        completeness: r.completeness,
      };
    }
    const { action, reason } = getCrossModuleAction(best.record_type, r.record_type);
    return {
      record_id: r.id!,
      record_name: r.record_name,
      record_type: r.record_type,
  ...
  // Action breakdown for summary
  const actionCounts: Record<string, number> = {};
  for (const rec of recs) {
    if (rec.action === 'KEEP') continue;
    actionCounts[rec.action] = (actionCounts[rec.action] || 0) + 1;
  }
  const actionParts = Object.entries(actionCounts)
    .map(([a, c]) => `${c} to ${a.toLowerCase()}`)
    .join(', ');

  const crossModuleNote = isCrossModule
    ? ' Cross-module cluster — only same-module records can be natively merged in Zoho; other records must be linked via Account_Name/Contact_Name.'
    : '';

  return {
    primary_id: best.id || null,
    primary_type: best.record_type || null,
    is_cross_module: isCrossModule,
    recommendations: recs,
    summary: `Keep "${best.record_name}" (${best.record_type}, ${best.completeness}% complete). ${actionParts || 'No duplicate actions needed'}.${crossModuleNote}`,
  };
}
