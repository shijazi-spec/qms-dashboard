...
export interface ReviewInputSummary {
  ...
  capa_summary?: { open: number; closed: number; effective: number; not_effective: number };
  audit_score?: number;
...
async function safeQuery(sql: string, params: any[] = []): Promise<any[]> {
  try {
    ...
    console.error('[MgmtReview DB] Query error:', error instanceof Error ? error.message : error);
    throw error;
  }
}
...
export async function createReview(data: Partial<ManagementReview>): Promise<ManagementReview> {
  ...
  const rows = await safeQuery(
    `INSERT INTO management_reviews (review_number, title, review_date, chair, attendees, status, agenda_items, minutes, decisions, input_summary, output_summary, next_review_date, created_by)
     ...
     RETURNING *`,
    [
      reviewNumber, data.title || 'Management Review', data.review_date,
      data.chair || 'Quality Manager', JSON.stringify(data.attendees || []),
      data.status || 'planned', JSON.stringify(data.agenda_items || []),
      data.minutes || null, JSON.stringify(data.decisions || []),
      JSON.stringify(data.input_summary || {}), data.output_summary || null,
      data.next_review_date || null, data.created_by || null
    ]
  );
  return rows[0];
}

export async function getReviews(filters?: { status?: string; year?: number; limit?: number; offset?: number }): Promise<{ reviews: ManagementReview[]; total: number }> {
  await initManagementReviewTables();
  let where = 'WHERE 1=1';
  const params: any[] = [];
  let idx = 1;
  ...
  if (filters?.year) { where += ` AND EXTRACT(YEAR FROM review_date) = $${idx++}`; params.push(filters.year); }

  const countRows = await safeQuery(`SELECT COUNT(*) as total FROM management_reviews ${where}`, params);
...
export async function getReviewById(id: number): Promise<ManagementReview | null> {
  await initManagementReviewTables();
  const rows = await safeQuery(`SELECT * FROM management_reviews WHERE id = $1`, [id]);
  if (rows.length === 0) return null;
  const review = rows[0];
  const actions = await safeQuery(`SELECT * FROM management_review_actions WHERE review_id = $1 ORDER BY action_number`, [id]);
  review.action_items = actions;
  return review;
}

export async function updateReview(id: number, data: Partial<ManagementReview>): Promise<ManagementReview | null> {
  const fields: string[] = [];
  const params: any[] = [];
  let idx = 1;

  if (data.title !== undefined) { fields.push(`title = $${idx++}`); params.push(data.title); }
  if (data.review_date !== undefined) { fields.push(`review_date = $${idx++}`); params.push(data.review_date); }
  if (data.chair !== undefined) { fields.push(`chair = $${idx++}`); params.push(data.chair); }
  if (data.attendees !== undefined) { fields.push(`attendees = $${idx++}`); params.push(JSON.stringify(data.attendees)); }
  if (data.status !== undefined) { fields.push(`status = $${idx++}`); params.push(data.status); }
  if (data.agenda_items !== undefined) { fields.push(`agenda_items = $${idx++}`); params.push(JSON.stringify(data.agenda_items)); }
  if (data.minutes !== undefined) { fields.push(`minutes = $${idx++}`); params.push(data.minutes); }
  if (data.decisions !== undefined) { fields.push(`decisions = $${idx++}`); params.push(JSON.stringify(data.decisions)); }
  if (data.input_summary !== undefined) { fields.push(`input_summary = $${idx++}`); params.push(JSON.stringify(data.input_summary)); }
  if (data.output_summary !== undefined) { fields.push(`output_summary = $${idx++}`); params.push(data.output_summary); }
  if (data.next_review_date !== undefined) { fields.push(`next_review_date = $${idx++}`); params.push(data.next_review_date); }
...
export async function deleteReview(id: number): Promise<boolean> {
  const result = await pool.query(`DELETE FROM management_reviews WHERE id = $1`, [id]);
  return (result.rowCount || 0) > 0;
}

export async function addReviewAction(reviewId: number, data: Partial<ReviewAction>): Promise<ReviewAction> {
  ...
  const actionNumber = data.action_number || parseInt(countRows[0]?.next || '1');

  const rows = await safeQuery(
    `INSERT INTO management_review_actions (review_id, action_number, description, assigned_to, due_date, status, priority)
     VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
    [reviewId, actionNumber, data.description, data.assigned_to, data.due_date, data.status || 'open', data.priority || 'medium']
  );
  return rows[0];
}

export async function updateReviewAction(actionId: number, data: Partial<ReviewAction>): Promise<ReviewAction | null> {
  const fields: string[] = [];
  const params: any[] = [];
  let idx = 1;
  ...
  if (data.status !== undefined) { fields.push(`status = $${idx++}`); params.push(data.status); }
  if (data.completion_date !== undefined) { fields.push(`completion_date = $${idx++}`); params.push(data.completion_date); }
  if (data.completion_notes !== undefined) { fields.push(`completion_notes = $${idx++}`); params.push(data.completion_notes); }
...
export async function getReviewActionsSummary(): Promise<{ total: number; open: number; overdue: number; completed: number }> {
  await initManagementReviewTables();
  const rows = await safeQuery(`
    SELECT
      COUNT(*) as total,
...
export async function gatherReviewInputs(): Promise<ReviewInputSummary> {
  ...
  try {
    const auditRows = await safeQuery(`SELECT overall_score FROM quality_audits ORDER BY audit_date DESC LIMIT 1`);
