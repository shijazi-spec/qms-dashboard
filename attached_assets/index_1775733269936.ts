        ...
        createHandler: async ({ mastra }) => {
          ...
          return async (c: any) => {
            try {
              ...
              // Try Inngest event dispatch first (works when Inngest dev server is running)
              try {
                await inngest.send({
              ...
              // Fallback: execute the workflow directly (for production or when Inngest is unavailable)
              (async () => {
                try {
                  const run = await qualityAuditWorkflow.createRunAsync();
                  await run.start({ inputData: {} });
                  logger?.info("✅ [API] Direct audit workflow completed");
                } catch (err) {
                  console.error("❌ [API] Direct audit workflow execution failed:", err);
                }
              })();

              logger?.info("✅ [API] Audit triggered via direct workflow execution");
              return c.json({ 
                success: true, 
                message: "Quality audit triggered successfully. Results will be available shortly." 
              });
