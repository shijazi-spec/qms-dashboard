      ...
      fetch('/api/notifications?limit=10&status=unread', { credentials: 'same-origin' })
        ...
        .then(data => {
          ...
          items.forEach(n => {
            ...
            item.className = 'flex items-start space-x-2 p-2 rounded-lg hover:bg-gray-50 cursor-pointer';
            item.addEventListener('click', () => WalaPlusNav.markRead(parseInt(n.id, 10)));
