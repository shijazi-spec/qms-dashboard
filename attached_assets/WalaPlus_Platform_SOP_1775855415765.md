...
- **Linear Webhook Integration:** Issue-created webhook handler (`src/triggers/exampleConnectorTrigger.ts`) with `Linear-Signature` header presence validation when `LINEAR_WEBHOOK_SECRET` is configured. Requests without the expected header are rejected with 401. Full HMAC-SHA256 signature verification with `timingSafeEqual` is planned for a future release.
...
- **Route-level auth enforcement (v3.6):** All API endpoints across RBAC, PDPL, Call Intelligence, and Duplicate Radar modules require authentication — unauthenticated requests return `401 Authentication required`. Specific enforcement levels: RBAC and Duplicate Radar destructive endpoints use `requireAdminOrKey` (admin only); PDPL reads use `requireAuthOrKey` (any authenticated user) and writes use `requireWriteRole` (write-capable roles); Call Intelligence ingest/upload endpoints use `requireAdminOrKey || getSessionUser` (any authenticated user or admin key)
...
| Date | Change | Impact |
|------|--------|--------|
| Apr 9, 2026 | SOP v3.6: 22-fix security hardening — SQL injection (parameterized intervals), path traversal, fake data removal, OIDC nonce (oauth_data cookie, redirect on mismatch), tiered auth guards (admin-only on RBAC/DuplicateRadar; session/admin on CallIntel; auth/write-role on PDPL), webhook validation (Telegram secret + Linear header presence check), audit trigger chain in fallback, error propagation, UUID resolution, dashboard HTML fixes | Protected API endpoints require authentication; webhook endpoints validated; SQL injection vectors eliminated |
