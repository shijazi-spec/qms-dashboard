/**
 * E2E coverage for the WalaPlus dashboard sidebar (Task #824).
 *
 * Background:
 ...
 *     walaplus-nav-pinned:user:admin
 *     walaplus-nav-recent:user:admin
 *
...
const PIN_KEY = 'walaplus-nav-pinned:user:admin';
const RECENT_KEY = 'walaplus-nav-recent:user:admin';
...
async function bootstrap(page: Page, lang: 'en' | 'ar') {
  await page.addInitScript((l: string) => {
    try {
      ...
      localStorage.removeItem('walaplus-nav-pinned');
      localStorage.removeItem('walaplus-nav-recent');
      localStorage.removeItem('walaplus-nav-pinned:user:admin');
      localStorage.removeItem('walaplus-nav-recent:user:admin');
      localStorage.removeItem('walaplus-nav-collapsed');
    } catch (_) { /* noop */ }
  }, lang);
  ...
  // (walaplus-nav-{pinned,recent}:user:admin) stay correct.
