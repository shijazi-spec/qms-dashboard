/**
 * Integration tests for src/mastra/routes/manualAuditRoutes.ts
 *
 * Coverage matrix:
 *   - structural      → every route exposes path/method/createHandler.
 *   - 200/4xx         → handlers return a numeric status.
 *
 * Run:  npx tsx tests/manualAuditRoutes.test.ts
 */

import { manualAuditRoutes } from "../src/mastra/routes/manualAuditRoutes";
import { TestSuite } from "./_helpers/runner";
import { buildHandler, makeContext, type FakeContext } from "./_helpers/fakeContext";

const suite = new TestSuite("manualAuditRoutes");

console.log("\n=== manualAuditRoutes integration tests ===\n");

await suite.test("every route exposes path, method and createHandler", async () => {
  for (const r of manualAuditRoutes) {
  ...
  suite.expect(manualAuditRoutes.length >= 1, "at least 1 route registered");
});

const apiRoutes = manualAuditRoutes.filter((r) => r.path.startsWith("/api/"));

for (const route of apiRoutes) {
  const path = route.path;
  const method = route.method as string;
  await suite.test(`${method} ${path} — handler returns numeric status`, async () => {
    const handler = await buildHandler(manualAuditRoutes, path, method, { mastra: null });
