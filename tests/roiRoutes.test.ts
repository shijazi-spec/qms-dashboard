/**
 * Integration tests for src/mastra/routes/roiRoutes.ts
 *
 * Coverage matrix:
 *   - 401/403         → API endpoints require an authenticated ROI role.
 *   - structural      → every route exposes path/method/createHandler.
 *
 * Run:  npx tsx tests/roiRoutes.test.ts
 */

import { roiRoutes } from "../src/mastra/routes/roiRoutes";
import { TestSuite } from "./_helpers/runner";
import { buildHandler, makeContext, type FakeContext } from "./_helpers/fakeContext";

const suite = new TestSuite("roiRoutes");

console.log("\n=== roiRoutes integration tests ===\n");

await suite.test("every route exposes path, method and createHandler", async () => {
  for (const r of roiRoutes) {
  ...
  suite.expect(roiRoutes.length >= 1, "at least 1 route registered");
});

const apiRoutes = roiRoutes.filter((r) => r.path.startsWith("/api/"));

for (const route of apiRoutes) {
  const path = route.path;
  const method = route.method as string;
  await suite.test(`${method} ${path} — 401/403 without an authenticated ROI role`, async () => {
    const handler = await buildHandler(roiRoutes, path, method, { mastra: null });
