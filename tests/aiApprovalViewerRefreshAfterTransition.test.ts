...
const TEST_PLATFORM_USERS: Record<string, { status: string; role: string }> = {
  'qm1@walaplus.test': { status: 'active', role: 'admin' },
  'qm2@walaplus.test': { status: 'active', role: 'quality_manager' },
...
const REVIEWER_A = {
  userId: 42,
  email: 'qm1@walaplus.test',
  name: 'Reviewer A (admin)',
  role: 'admin',
};
const REVIEWER_B = {
  userId: 43,
  email: 'qm2@walaplus.test',
...
async function fetchListPriorViewers(
  ...
  const res = await callRoute(
    ...
    'GET',
    makeContext({
      url: `https://test.local/api/ai/approvals?status=${encodeURIComponent(status)}`,
      cookie,
    }),
  );
  if (res.status !== 200) {
...
async function viewDetail(actionCode: string, cookie: string): Promise<FakeResponse> {
  return callRoute(
    ...
    'GET',
    makeContext({
      url: `https://test.local/api/ai/approvals/${actionCode}`,
      param: actionCode,
      cookie,
    }),
  );
