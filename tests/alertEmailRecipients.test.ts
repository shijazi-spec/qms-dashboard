...
async function run(): Promise<void> {
  console.log("\nnormaliseEmail");
  {
    assert(normaliseEmail("Foo@Bar.Com") === "foo@bar.com", "lower-cases input");
    assert(normaliseEmail("  ops@x.io  ") === "ops@x.io", "trims whitespace");
    ...
    assert(normaliseEmail("has space@x.io") === null, "rejects whitespace inside");
  ...
  console.log("\nparseRecipientsEnvValue");
  {
    assert(parseRecipientsEnvValue("a@x.io").join(",") === "a@x.io", "single entry");
    assert(
      parseRecipientsEnvValue("a@x.io, b@y.io , c@z.io").join("|") ===
        "a@x.io|b@y.io|c@z.io",
      "multi-entry trim",
    );
    assert(
      parseRecipientsEnvValue("a@x.io, A@X.io, b@y.io").join("|") === "a@x.io|b@y.io",
  ...
  console.log("\nresolveEffectiveRecipients precedence");
  {
    ...
    const r1 = await resolveEffectiveRecipients(
      ...
      {
        list: async () => [
          { email: "db@x.io", added_by: "admin", added_at: new Date() },
        ],
      },
    );
    assert(r1.source === "db", "source=db when DB list non-empty");
    assert(
      r1.recipients.length === 1 && r1.recipients[0] === "db@x.io",
    ...
    const r2 = await resolveEffectiveRecipients(
      "post_restore_sweep",
      "env-only@x.io, second@x.io",
      { list: async () => [] },
    );
    assert(r2.source === "env", "source=env when DB empty + env set");
    assert(
      r2.recipients.length === 2 && r2.recipients[0] === "env-only@x.io",
    ...
    const r4 = await resolveEffectiveRecipients(
      ...
      {
        list: async () => [
          { email: "Admin@X.io", added_by: null, added_at: null },
          { email: "admin@x.io", added_by: null, added_at: null },
          { email: "other@x.io", added_by: null, added_at: null },
        ],
      },
    );
    assert(r4.source === "db", "source=db on duplicates");
    assert(
      r4.recipients.length === 2 && r4.recipients[0] === "Admin@X.io",
    ...
    assert(
      r5.source === "env" && r5.recipients[0] === "ai-oncall@x.io",
