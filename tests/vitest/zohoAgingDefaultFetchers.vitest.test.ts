    ...
    fetchZohoRecordsMock.mockResolvedValueOnce([
      {
        ...
        data: { Full_Name: "Acme", Lead_Status: "New", Owner: { name: "Dan" } },
      },
    ]);
    fetchAllZohoRecordsMock.mockResolvedValueOnce([
      { id: "L1", data: { Full_Name: "Acme", Lead_Status: "New", Owner: { name: "Dan" }, Created_Time: "2026-04-23T00:00:00Z" } },
    ]);
...
describe("getDealStageAging / getLeadStatusAging — single-record live path", () => {
  ...
  test("getLeadStatusAging picks the latest matching Lead_Status entry", async () => {
    fetchAllZohoRecordsMock.mockResolvedValueOnce([
      { id: "L7", data: { Full_Name: "X Co", Lead_Status: "Working", Owner: { name: "Sam" }, Created_Time: "2025-01-01T00:00:00Z" } },
    ]);
