/**
 * Unit tests for the Zoho structured-fields patch builder + sync
 * pipeline. Pure functions + the flagged sync, exercised against
 * a stubbed updateZohoRecord via dynamic import mocking.
 *
 * Run: npx vitest run tests/vitest/zohoStructuredFieldsSync.vitest.test.ts
 */
import { afterEach, beforeEach, describe, expect, test, vi } from "vitest";
import {
  buildEvaluationPatch,
  readFieldNames,
  syncEvaluationToZohoStructuredFields,
} from "../../src/utils/zohoStructuredFieldsSync";

const FLAG_KEY = "ZOHO_STRUCTURED_FIELDS";

afterEach(() => {
  delete process.env[FLAG_KEY];
  delete process.env.ZOHO_FIELD_QA_SCORE;
  delete process.env.ZOHO_FIELD_COMPLIANCE_PASS;
  delete process.env.ZOHO_FIELD_LAST_EVAL_DATE;
  vi.restoreAllMocks();
});
...
describe("readFieldNames", () => {
  ...
  test("env overrides win", () => {
    process.env.ZOHO_FIELD_QA_SCORE = "Custom_QA";
    process.env.ZOHO_FIELD_COMPLIANCE_PASS = "Custom_Compliance";
    process.env.ZOHO_FIELD_LAST_EVAL_DATE = "Custom_Eval_Date";
    expect(readFieldNames()).toEqual({
      qaScore: "Custom_QA",
...
describe("syncEvaluationToZohoStructuredFields — flag gating", () => {
...
describe("syncEvaluationToZohoStructuredFields — skip semantics", () => {
  beforeEach(() => {
    process.env[FLAG_KEY] = "true";
  });
  ...
  test("no_crm_linkage when neither lead nor deal id present", async () => {
    ...
    expect(r.synced).toBe(false);
    expect(r.skipped_reason).toBe("no_crm_linkage");
  });
  ...
  test("no_writable_fields when evaluation has nothing patch-able", async () => {
    ...
    expect(r.synced).toBe(false);
    expect(r.skipped_reason).toBe("no_writable_fields");
  });

  test("prefers Leads over Deals when both are set", async () => {
    // Mock updateZohoRecord to capture which module is targeted
    let capturedModule: string | null = null;
    vi.doMock("../../src/utils/zohoCRM", () => ({
      updateZohoRecord: async (mod: string) => {
        capturedModule = mod;
        return { id: "ok" };
      },
    }));
