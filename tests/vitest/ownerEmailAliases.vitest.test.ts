...
describe("OWNER_EMAIL_ALIASES table", () => {
  test("Rayan's three mailboxes all map to rayan@walaplus.com", () => {
    expect(OWNER_EMAIL_ALIASES["pipedrive@walaplus.com"]).toBe("rayan@walaplus.com");
    expect(OWNER_EMAIL_ALIASES["info@walaplus.com"]).toBe("rayan@walaplus.com");
  });
...
describe("canonicaliseOwnerEmail", () => {
  test("aliases map to canonical", () => {
    expect(canonicaliseOwnerEmail("pipedrive@walaplus.com")).toBe("rayan@walaplus.com");
    expect(canonicaliseOwnerEmail("info@walaplus.com")).toBe("rayan@walaplus.com");
  });
  ...
  test("non-alias email passes through (lower-cased)", () => {
    expect(canonicaliseOwnerEmail("alice@walaplus.com")).toBe("alice@walaplus.com");
    expect(canonicaliseOwnerEmail("Alice@WalaPlus.com")).toBe("alice@walaplus.com");
  });
  ...
  test("canonical email also canonicalises to itself", () => {
    ...
    expect(canonicaliseOwnerEmail("rayan@walaplus.com")).toBe("rayan@walaplus.com");
  });
});
