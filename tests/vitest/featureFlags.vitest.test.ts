...
describe("isFlagEnabled — per-user allowlist", () => {
  test("listed user is enabled even when global is off", () => {
    process.env[USERS_KEY] = "alice@walaplus.com,bob@walaplus.com";
    expect(isFlagEnabled(FLAG, "alice@walaplus.com")).toBe(true);
    expect(isFlagEnabled(FLAG, "bob@walaplus.com")).toBe(true);
  });

  test("unlisted user stays disabled when global is off", () => {
    process.env[USERS_KEY] = "alice@walaplus.com";
    expect(isFlagEnabled(FLAG, "carol@walaplus.com")).toBe(false);
  });

  test("user list is additive — global on still wins for unlisted users", () => {
    process.env[ENV_KEY] = "true";
    process.env[USERS_KEY] = "alice@walaplus.com";
    expect(isFlagEnabled(FLAG, "carol@walaplus.com")).toBe(true);
  });
...
describe("isFlagEnabled — defensive behavior", () => {
  test("unknown flag name returns false (never throws)", () => {
    ...
    expect(isFlagEnabled("" as FlagName, "alice@walaplus.com")).toBe(false);
  });
});
