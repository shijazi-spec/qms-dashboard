...
describe("ownerToConsult", () => {
  test("name + email → 'name <email>'", () => {
    expect(
      ownerToConsult({ owner_name: "Ali", owner_email: "ali@walaplus.com" }),
    ).toBe("Ali <ali@walaplus.com>");
  });
  test("name only → name", () => {
    expect(
      ownerToConsult({ owner_name: "Ali", owner_email: "" }),
    ).toBe("Ali");
  });
  test("email only → email", () => {
    expect(
      ownerToConsult({ owner_name: null, owner_email: "ali@walaplus.com" }),
    ).toBe("ali@walaplus.com");
  });
...
describe("startCluster + rowPlaybook (integration)", () => {
  test("first row of cluster captures primary; subsequent row uses 'Merge into <primary>'", () => {
    ...
    expect(dupPb.owner_to_consult).toBe("Sara <sara@walaplus.com>");
