...
describe("normalizeQuery", () => {
  test("strips protocol, www, path; lowercases", () => {
    expect(normalizeQuery("HTTPS://www.Example.com/path")).toBe("example.com");
  });
  test("preserves Saudi multi-level TLD", () => {
    expect(normalizeQuery("https://alsahab.sa/")).toBe("alsahab.sa");
    expect(normalizeQuery("anb.com.sa")).toBe("anb.com.sa");
  });
