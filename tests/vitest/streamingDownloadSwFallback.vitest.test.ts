...
function loadSW(origin = 'http://localhost'): SWHarness {
...
describe('streaming-download SW — expired/missing-registration fallback page', () => {
  ...
  it('ignores cross-origin fetches (does not respond at all)', () => {
    const sw = loadSW('http://localhost');
    let respondCalled = false;
    const event = {
      request: { url: 'https://evil.example.com/_stream-download/x' },
      respondWith() {
        respondCalled = true;
      },
    };
  ...
  it('ignores requests outside the /_stream-download/ namespace', () => {
    ...
    let respondCalled = false;
    const event = {
      request: { url: 'http://localhost/some/other/path' },
      respondWith() {
        respondCalled = true;
      },
    };
