/**
 * zoho_sync_state — the incremental watermark.
 *
 ...
 * run asked Zoho for "changes since the moment the failed run began" and every
...
const call = () =>
  query.mock.calls.find((c) => /INSERT INTO zoho_sync_state/i.test(String(c[0])));
