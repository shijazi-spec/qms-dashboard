/**
 * Zoho Tasks bulk sync — feeds SDR-KPI-11, SALES-KPI-07 and SALES-KPI-08.
 *
 * Two things this locks in, both learned the hard way elsewhere in this
 * codebase:
 *
 * 1. WINDOWING USES If-Modified-Since, NEVER `criteria`. Zoho honours criteria
...
vi.mock("../../src/utils/zohoCRM", () => ({
  fetchAllZohoRecords: fetchAll,
  getZohoConnectionStatus: connStatus,
}));
vi.mock("../../src/utils/logger", () => ({
  logger: { info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug: vi.fn() },
}));

import { runZohoTasksSync } from "../../src/utils/zohoTasksSync";
...
const task = (over: any = {}) => ({
  ...
  data: {
    ...
    Status: "Not Started",
    Due_Date: "2026-08-20",
    Owner: { name: "Rep", email: "REP@walaplus.com" },
...
describe("runZohoTasksSync — Zoho query shape", () => {
  it("windows by If-Modified-Since and NEVER sends criteria", async () => {
    await runZohoTasksSync({ sinceIso: "2026-05-01T00:00:00Z" });
  ...
  it("bails when Zoho is not configured, without calling Zoho", async () => {
    ...
    const r = await runZohoTasksSync({});
...
describe("runZohoTasksSync — linkage census", () => {
  it("classifies Who / What / both / none", async () => {
    ...
    expect(r.skipped_no_linkage).toBe(1);
  });

  it("counts inserts and updates separately via xmax", async () => {
    fetchAll.mockResolvedValue([task({ id: "T1" }), task({ id: "T2" })]);
    query.mockImplementation(async (sql: string) =>
      /INSERT INTO zoho_tasks/i.test(String(sql))
        ? { rows: [{ inserted: query.mock.calls.filter((c) => /INSERT INTO zoho_tasks/i.test(String(c[0]))).length === 1 }] }
        : { rows: [] },
    );
    const r = await runZohoTasksSync({});
  ...
  it("upserts on zoho_task_id so re-running is safe", async () => {
    fetchAll.mockResolvedValue([task({ id: "T1" })]);
    await runZohoTasksSync({});
    const insert = query.mock.calls
      .map((c) => String(c[0]))
      .find((s) => /INSERT INTO zoho_tasks/i.test(s));
    expect(insert).toMatch(/ON CONFLICT \(zoho_task_id\) DO UPDATE/i);
  });

  it("lowercases the owner email and keeps both lookup ids", async () => {
    fetchAll.mockResolvedValue([task({ id: "T1" })]);
    await runZohoTasksSync({});
    const call = query.mock.calls.find((c) => /INSERT INTO zoho_tasks/i.test(String(c[0])));
    const params = call?.[1] as any[];
    expect(params).toContain("rep@walaplus.com");
  ...
  it("skips a task with no id rather than writing a null key", async () => {
    ...
    expect(query.mock.calls.some((c) => /INSERT INTO zoho_tasks/i.test(String(c[0])))).toBe(false);
  });
});
