...
const TEST_QM_EMAIL = "review-counts-int-qm@walaplus-test.invalid";
const TEST_QM_NAME = "Review-Counts Integration QM";
const TEST_QM_USER_ID = 999_801;

const TEST_BU_EMAIL = "review-counts-int-bu@walaplus-test.invalid";
...
async function seedActions(
  ...
  for (const shape of shapes) {
    const row = await enqueuePendingAction({
      ...
      requestedByEmail: `req-${shape.ownerUserId}@walaplus-test.invalid`,
  ...
  for (const row of seeded) {
    for (const viewerUserId of row.viewerUserIds) {
      await logEvent({
        userId: viewerUserId,
        userEmail: `viewer-${viewerUserId}@walaplus-test.invalid`,
