/**
 * WalaPlus i18n / RTL E2E Tests — Phase 1
 *
 * Verifies:
 ...
 *
 * These tests use the running dev server (http://localhost:5000).
...
test.describe('AI Consultant widget — i18n chrome', () => {
  test('widget chrome is translated to Arabic', async ({ page }) => {
    ...
    await page.waitForFunction(() => {
      ...
      return h4 && h4.textContent && h4.textContent.trim() !== 'WalaPlus QMS Consultant';
    }, { timeout: 6000 }).catch(() => {
    ...
    if (await h4.count() > 0) {
      ...
      expect(h4Text).not.toBe('WalaPlus QMS Consultant');
