# Prompts to Fix QC Failures in Replit

Copy the prompt(s) below into **Replit's AI chat** (or Agent). Use **Prompt 1** first for a full fix; use **Prompt 2** if you want to fix by module in smaller steps.
...
```
I need you to fix the WalaPlus QMS Dashboard so that all of these API endpoints work correctly. Our QC suite runs against this app and currently all 52 checks fail. Each endpoint should return HTTP 200 (or 401 for admin routes when X-Admin-Key is missing).
...
**QMS Dashboard (route /qms)**
- GET /api/qms/dashboard — QMS dashboard data
...
```
Our platform QC suite checks 52 API endpoints. All are failing. Please fix the WalaPlus Mastra app so these endpoints respond correctly:
...
## After Replit applies fixes

1. In Replit: run the app (e.g. **Run** or `npm run dev`) and leave it running.
2. On your PC (PowerShell): run  
   `$env:QC_BASE_URL="https://qms-dashboard.replit.app"; npm run qc`
3. Open `tests/qc/qc-report.md` and check the new Pass/Fail counts and the “Send to Replit – Fix These” section for any remaining failures.
