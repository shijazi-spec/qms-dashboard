/**
 * WalaPlus Duplicate Radar — per-tab Advanced Filters wiring test
 *
...
test.describe('Duplicate Radar — per-tab Advanced Filters', () => {
  ...
  test('Apply filters the active Deals tab in-place with the selected layout', async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      layout.innerHTML = '<option value="WalaPlus">WalaPlus</option>';
    ...
    expect(dealsReq!).toContain('layouts=WalaPlus');
