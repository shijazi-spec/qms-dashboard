/**
 * Integration tests for src/mastra/routes/duplicateRadarRoutes.ts
 *
 * Coverage matrix:
 *   - structural      → every route exposes path/method/createHandler.
 *   - 200/4xx         → handlers return a numeric status; auth varies.
 *
 * Run:  npx tsx tests/duplicateRadarRoutes.test.ts
 */
...
const suite = new TestSuite("duplicateRadarRoutes");

console.log("\n=== duplicateRadarRoutes integration tests ===\n");

await suite.test("every route exposes path, method and createHandler", async () => {
  for (const r of duplicateRadarRoutes) {
  ...
  suite.expect(duplicateRadarRoutes.length >= 1, "at least 1 route registered");
});

const apiRoutes = duplicateRadarRoutes.filter((r) => r.path.startsWith("/api/"));

for (const route of apiRoutes) {
  const path = route.path;
  const method = route.method as string;
  await suite.test(`${method} ${path} — handler returns numeric status`, async () => {
    const handler = await buildHandler(duplicateRadarRoutes, path, method, { mastra: null });
