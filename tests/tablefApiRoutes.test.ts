/**
 * Integration tests for src/mastra/routes/tablefApiRoutes.ts
 *
 * Coverage matrix:
 *   - structural      → every route exposes path/method/createHandler.
 *   - 200/4xx         → handlers return a numeric status.
 *
 * Run:  npx tsx tests/tablefApiRoutes.test.ts
 */
...
const suite = new TestSuite("tablefApiRoutes");

console.log("\n=== tablefApiRoutes integration tests ===\n");

await suite.test("every route exposes path, method and createHandler", async () => {
  for (const r of tablefApiRoutes) {
  ...
  suite.expect(tablefApiRoutes.length >= 1, "at least 1 route registered");
});

const apiRoutes = tablefApiRoutes.filter((r) => r.path.startsWith("/api/"));

for (const route of apiRoutes) {
  const path = route.path;
  const method = route.method as string;
  await suite.test(`${method} ${path} — handler returns numeric status`, async () => {
    const handler = await buildHandler(tablefApiRoutes, path, method, { mastra: null });
    const ctx = makeContext({
      method,
      params: { id: "1", recordId: "1" },
