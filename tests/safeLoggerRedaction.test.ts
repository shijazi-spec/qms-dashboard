...
{
  const result = _sanitiseForTest({ username: 'alice', email: 'alice@example.com' }) as any;
  assert(result.username === 'alice', 'non-sensitive key preserved by sanitise');
  assert(result.email === 'alice@example.com', 'email preserved by sanitise');
}

{
  const result = _sanitiseForTest({ username: 'bob', password_hash: '$2b$12$secret', api_key: 'sk-abc' }) as any;
...

{
  const result = _sanitiseForTest({
    provider: 'zoho',
    access_token: 'ya29.a0AfH6SMB_ACCESS',
...
{
  const result = _sanitiseForTest({
    user: { email: 'x@y.com', password_hash: '$2b$12$hash', mfa_secret: 'TOTP_SECRET' },
    meta: { module: 'auth' },
  }) as any;
  ...
  assert(result.user.email === 'x@y.com', 'nested email preserved');
...
{
  const result = _mergePayloadsForTest([
    { userId: 10, access_token: 'ya29.secret_TOKEN' },
    { provider: 'google' },
  ]);
...
{
  const result = _mergePayloadsForTest([
    { config: { api_key: 'sk-PLAINTEXTKEY', timeout: 5000 } },
  ]);
...
{
  const ghp = 'ghp_AAAABBBBCCCCDDDDEEEEFFFFGGGGHHHH1234';
